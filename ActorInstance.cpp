...
#include "RaceData.h"
#include "../SpeedTreeLib/SpeedTreeForestD3D9Ex.h"
...
void CActorInstance::OnUpdate()
{
	...
	if ((m_eRace >= 34000 && m_eRace <= 34999) || (m_eRace >= 65744 && m_eRace <= 65747) || (m_eRace >= 65748 && m_eRace <= 65755) 
		 || (m_eRace >= 65764 && m_eRace <= 65777) || m_eRace == 8941 || m_eRace == 54086)
		m_bRenderActor = CPythonGraphicOnOff::Instance().CanRenderPet();

	if (GetActorType() == EType::TYPE_HORSE || GetActorType() == EType::TYPE_MOUNT)
		m_bRenderActor = CPythonGraphicOnOff::Instance().CanRenderMount();
	...
	if (m_bRenderActor)
	{
		if (CPythonGraphicOnOff::Instance().CanRenderEffect(GetActorType(), FALSE != m_isMain, GetRank()) && (!m_bIsInvisibleActor || m_isMain))
		{
			if (!m_bRenderEffect)
			{
...
#ifdef ENABLE_ANTIKNOCK_BOSSES
bool CActorInstance::IsNoKnockback()
{
...
void CActorInstance::__CreateTree(const char * c_szFileName)
{
	...
	CSpeedTreeForestD3D9Ex& rkForest=CSpeedTreeForestD3D9Ex::Instance();
...
void CActorInstance::__DestroyTree()
{
	...
	CSpeedTreeForestD3D9Ex::Instance().DeleteInstance(m_pkTree);
}
...
void CActorInstance::__Initialize()
{
	...
	m_pkHorse=NULL;
	m_pkTree=NULL;
	m_awWeaponType = -1; // GRM Update 16112023
