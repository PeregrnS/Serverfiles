...
int						CGraphicBase::ms_nonPow2Support = 0;
...
HRESULT CGraphicBase::ResetDeviceEx(D3DPRESENT_PARAMETERS* pPP)
{
	if (!ms_lpd3dDevice || !pPP)
		return E_FAIL;

	D3DDISPLAYMODEEX fullMode = {};
	D3DDISPLAYMODEEX* pFull = NULL;
	if (!pPP->Windowed)
	{
		fullMode.Size = sizeof(fullMode);
		fullMode.Width = pPP->BackBufferWidth;
		fullMode.Height = pPP->BackBufferHeight;
		fullMode.RefreshRate = pPP->FullScreen_RefreshRateInHz;
		fullMode.Format = pPP->BackBufferFormat;
		fullMode.ScanLineOrdering = D3DSCANLINEORDERING_UNKNOWN;
		pFull = &fullMode;
	}
	return ms_lpd3dDevice->ResetEx(pPP, pFull);
}
