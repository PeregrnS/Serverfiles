...
static void Grp_CopyD3DDisplayModeExToDisplayMode(D3DDISPLAYMODE* pOut, const D3DDISPLAYMODEEX& ex)
{
	ZeroMemory(pOut, sizeof(D3DDISPLAYMODE));
	pOut->Width = ex.Width;
	pOut->Height = ex.Height;
	pOut->RefreshRate = ex.RefreshRate;
	pOut->Format = ex.Format;
}
...
VOID D3D_CAdapterDisplayModeList::Build(IDirect3DEmbedded& rkD3D, D3DFORMAT eD3DFmtDefault, UINT iD3DAdapterInfo)
{
	...
	modeFilter.Format = eD3DFmtDefault;
	modeFilter.ScanLineOrdering = D3DSCANLINEORDERING_UNKNOWN;

	UINT uAdapterModeNum = rkD3D.GetAdapterModeCountEx(iD3DAdapterInfo, &modeFilter);
	for (UINT iD3DAdapterInfoMode = 0; iD3DAdapterInfoMode < uAdapterModeNum; iD3DAdapterInfoMode++)
	{
		D3DDISPLAYMODEEX kD3DDMCurEx;
		ZeroMemory(&kD3DDMCurEx, sizeof(kD3DDMCurEx));
		kD3DDMCurEx.Size = sizeof(D3DDISPLAYMODEEX);
		if (FAILED(rkD3D.EnumAdapterModesEx(iD3DAdapterInfo, &modeFilter, iD3DAdapterInfoMode, &kD3DDMCurEx)))
			continue;

		D3DDISPLAYMODE kD3DDMCur;
		Grp_CopyD3DDisplayModeExToDisplayMode(&kD3DDMCur, kD3DDMCurEx);
...
BOOL D3D_CDeviceInfo::FindDepthStencilFormat(IDirect3DEmbedded& rkD3D, UINT iD3DAdapterInfo, D3DDEVTYPE DeviceType, D3DFORMAT TargetFormat, D3DFORMAT* pDepthStencilFormat)
{
...
BOOL D3D_CDeviceInfo::Build(IDirect3DEmbedded& rkD3D, UINT iD3DAdapterInfo, UINT iDevType, D3D_CAdapterDisplayModeList& rkD3DADMList, BOOL(*pfnConfirmDevice)(D3DCAPS9& rkD3DCaps, UINT uBehavior, D3DFORMAT eD3DFmt))
{
...
BOOL D3D_CAdapterInfo::Build(IDirect3DEmbedded& rkD3D, UINT iD3DAdapterInfo, PFNCONFIRMDEVICE pfnConfirmDevice)
{
	D3DDISPLAYMODE& rkD3DDMDesktop = m_kD3DDMDesktop;
	D3DDISPLAYMODEEX desktopEx;
	ZeroMemory(&desktopEx, sizeof(desktopEx));
	desktopEx.Size = sizeof(D3DDISPLAYMODEEX);
	D3DDISPLAYROTATION rotation = D3DDISPLAYROTATION_IDENTITY;
	if (FAILED(rkD3D.GetAdapterDisplayModeEx(iD3DAdapterInfo, &desktopEx, &rotation)))
		return false;
	Grp_CopyD3DDisplayModeExToDisplayMode(&rkD3DDMDesktop, desktopEx);
...
BOOL D3D_CDisplayModeAutoDetector::Build(IDirect3DEmbedded& rkD3D, PFNCONFIRMDEVICE pfnConfirmDevice)
{
	m_uD3DAdapterInfoCount = 0;
