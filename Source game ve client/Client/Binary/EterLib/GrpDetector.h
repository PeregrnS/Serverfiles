...
// IDirect3D9Ex only (see ENABLE_DX9EX guard in EterLib/StdAfx.h).
typedef IDirect3D9Ex IDirect3DEmbedded;
...
class D3D_CAdapterDisplayModeList
{
	public:
		...
		~D3D_CAdapterDisplayModeList() {}
		VOID Build(IDirect3DEmbedded& rkD3D, D3DFORMAT eD3DFmtDefault, UINT iAdapter);
...
class D3D_CDeviceInfo
{
	public:
		D3D_CDeviceInfo() {}
		~D3D_CDeviceInfo() {}
		BOOL Build(IDirect3DEmbedded& rkD3D, UINT iAdapter, UINT iDevType, D3D_CAdapterDisplayModeList& rkD3DADMList, PFNCONFIRMDEVICE pfnConfirmDevice);
		...
		BOOL FindDepthStencilFormat(IDirect3DEmbedded& rkD3D, UINT iAdapter, D3DDEVTYPE DeviceType, D3DFORMAT TargetFormat, D3DFORMAT* pDepthStencilFormat);
...
class D3D_CAdapterInfo
{
	public:
		...
		BOOL Build(IDirect3DEmbedded& rkD3D, UINT iAdapter, PFNCONFIRMDEVICE pfnConfirmDevice);
		VOID GetString(std::string* pstEnumList);
...
class D3D_CDisplayModeAutoDetector
{
	public:
		...
		BOOL Build(IDirect3DEmbedded& rkD3D, PFNCONFIRMDEVICE pfnConfirmDevice);
