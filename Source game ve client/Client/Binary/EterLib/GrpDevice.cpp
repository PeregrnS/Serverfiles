...
void CGraphicDevice::EnableWebBrowserMode(const RECT& c_rcWebPage)
{
	...
	DestroyDefaultIndexBuffersBeforeReset();
	HRESULT hr = CGraphicBase::ResetDeviceEx(&rkD3DPP);
	if (FAILED(hr))
	{
		CreateDefaultIndexBuffersAfterReset();
		return;
	}
	if (!CreateDefaultIndexBuffersAfterReset())
		return;
...
CGraphicDevice::EDeviceState CGraphicDevice::GetDeviceState()
{
	...
	if (ms_hWnd)
	{
		const HRESULT hrCS = ms_lpd3dDevice->CheckDeviceState(ms_hWnd);
		if (hrCS == D3DERR_DEVICELOST)
			return DEVICESTATE_BROKEN;
		if (hrCS == D3DERR_DEVICENOTRESET)
			return DEVICESTATE_NEEDS_RESET;
	}
...
{
	DestroyDefaultIndexBuffersBeforeReset();
	HRESULT hr = CGraphicBase::ResetDeviceEx(&ms_d3dPresentParameter);
	if (FAILED(hr))
	{
		CreateDefaultIndexBuffersAfterReset();
		return false;
...
int CGraphicDevice::Create(HWND hWnd, int iHres, int iVres, bool Windowed, int /*iBit*/, int iReflashRate)
{
	...
	ms_hWnd		= hWnd;
	ms_hDC		= GetDC(hWnd);
	if (FAILED(Direct3DCreate9Ex(D3D_SDK_VERSION, &ms_lpd3d)))
		return CREATE_NO_DIRECTX;

	if (!ms_kD3DDetector.Build(*ms_lpd3d, EL3D_ConfirmDevice))
	{
		TraceError("adapter error 0");
		return CREATE_ENUM;
	}
	...
	if (!pkD3DAdapterInfo)
	{
		TraceError("adapter %d is EMPTY", ms_iD3DAdapterInfo);
		return CREATE_DETECT;
	}
	...
	if (!pkD3DModeInfo)
	{
		TraceError("device %d, mode %d is EMPTY", ms_iD3DDevInfo, ms_iD3DModeInfo);
		return CREATE_DETECT;
	}
...
RETRY:
	...
	D3DDISPLAYMODEEX kFullscreenMode = {};
	D3DDISPLAYMODEEX* pkFullscreenMode = NULL;
	if (!Windowed)
	{
		kFullscreenMode.Size = sizeof(kFullscreenMode);
		kFullscreenMode.Width = ms_d3dPresentParameter.BackBufferWidth;
		kFullscreenMode.Height = ms_d3dPresentParameter.BackBufferHeight;
		kFullscreenMode.RefreshRate = ms_d3dPresentParameter.FullScreen_RefreshRateInHz;
		kFullscreenMode.Format = ms_d3dPresentParameter.BackBufferFormat;
		kFullscreenMode.ScanLineOrdering = D3DSCANLINEORDERING_UNKNOWN;
		pkFullscreenMode = &kFullscreenMode;
	}
				...
				&ms_lpd3dDevice)))
	{
		switch (ms_hLastResult)
		{
			case D3DERR_INVALIDCALL:
				TraceError("IDirect3DDevice9Ex.CreateDeviceEx - ERROR D3DERR_INVALIDCALL\nThe method call is invalid. For example, a method's parameter may have an invalid value.");
				break;
			case D3DERR_NOTAVAILABLE:
				TraceError("IDirect3DDevice9Ex.CreateDeviceEx - ERROR D3DERR_NOTAVAILABLE\nThis device does not support the queried technique. ");
				break;
			case D3DERR_OUTOFVIDEOMEMORY:
				TraceError("IDirect3DDevice9Ex.CreateDeviceEx - ERROR D3DERR_OUTOFVIDEOMEMORY\nDirect3D does not have enough display memory to perform the operation");
				break;
			default:
				TraceError("IDirect3DDevice9Ex.CreateDeviceEx - ERROR %d", ms_hLastResult);
				break;
		}
	...
	{
		TraceError("IDirect3DDevice9Ex.GetDeviceCaps - ERROR %d", ms_hLastResult);
...
bool CGraphicDevice::__CreateDefaultIndexBuffer(UINT eDefIB, UINT uIdxCount, const WORD* c_awIndices)
{
	...
	const D3DPOOL ePool = D3DPOOL_DEFAULT;
	const DWORD dwIBUsage = D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC;
	...
	WORD* dstIndices;
	if (FAILED(
		ms_alpd3dDefIB[eDefIB]->Lock(0, 0, (void**)&dstIndices, dwLockFlags)
	)) return false;
...
{
	return __FillDefaultIndexBufferList();
}

bool CGraphicDevice::__FillDefaultIndexBufferList()
{
...
bool CGraphicDevice::CreateDefaultIndexBuffersAfterReset()
{
	return __FillDefaultIndexBufferList();
}

void CGraphicDevice::DestroyDefaultIndexBuffersBeforeReset()
{
	for (UINT i = 0; i < DEFAULT_IB_NUM; ++i)
	{
		if (ms_alpd3dDefIB[i])
...
void CGraphicDevice::ReleaseAccumulatedDriverMemory()
{
	if (ms_lpd3dDevice)
		ms_lpd3dDevice->EvictManagedResources();
