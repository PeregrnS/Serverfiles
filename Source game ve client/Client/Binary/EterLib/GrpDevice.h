...
public:
	...
	// After map warp / heavy unload: hint driver to drop managed VRAM and tighten working set.
	static void		ReleaseAccumulatedDriverMemory();

	// DEFAULT pool IBs must be released before IDirect3DDevice9Ex::ResetEx.
	static void		DestroyDefaultIndexBuffersBeforeReset();
	static bool		CreateDefaultIndexBuffersAfterReset();
...
protected:
	...
	static bool __FillDefaultIndexBufferList();
	static bool __CreateDefaultIndexBuffer(UINT eDefIB, UINT uIdxCount, const WORD* c_awIndices);
