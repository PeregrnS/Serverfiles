...
#ifdef ENABLE_INGAME_WIKI
	SetExpandedRenderingRect(fLeft, fLeft, fTop, fTop, fRight, fRight, fBottom, fBottom, true);
#else
	m_RenderingRect.left = fWidth * fLeft;
	m_RenderingRect.top = fHeight * fTop;
	m_RenderingRect.right = fWidth * fRight;
...
#ifdef ENABLE_INGAME_WIKI
void CGraphicExpandedImageInstance::SetExpandedRenderingRect(float fLeftTop, float fLeftBottom, float fTopLeft, float fTopRight, float fRightTop, float fRightBottom, float fBottomLeft, float fBottomRight, bool bScale)
{
	if (IsEmpty())
		return;
	...
	if (bScale)
	{
		fWidth = float(GetWidth()) * m_v2Scale.x;
		fHeight = float(GetHeight()) * m_v2Scale.y;
	}
	else
	{
