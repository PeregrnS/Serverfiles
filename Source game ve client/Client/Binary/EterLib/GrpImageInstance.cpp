...
void CGraphicImageInstance::DisplayImageProcent(float fProcent)
{
	fProcent = (100.0f - fProcent) / 100.0f;

	if (fProcent < 0.0f || fProcent > 1.0f)
	{
		fProcent = 1.0f;
	}
	...
	int iTriCount = int(8.0f - 8.0f * fProcent);
	float fLastPercentage = (8.0f - 8.0f * fProcent) - iTriCount;
	...
	if (iTriCount > 0)
	{
		...
		{
			vertex.position = TPosition(fxCenter + (s_v2BoxPos[j].x * fimgWidthHalf) - 0.5f, fyCenter + (s_v2BoxPos[j].y * fimgHeightHalf) - 0.5f, 0.0f);
			vertex.texCoord = TTextureCoordinate(v2TexPos[j & (c_iTriangleCountPerBox - 1)].x, v2TexPos[j & (c_iTriangleCountPerBox - 1)].y);
			vertex.diffuse = m_DiffuseColor;
			vertices.push_back(vertex);
		}
	}
	...
	if (fLastPercentage > 0.0f)
	{
		...
		float fx = (pv2LastPos->x - pv2Pos->x) * fLastPercentage + pv2Pos->x;
		float fy = (pv2LastPos->y - pv2Pos->y) * fLastPercentage + pv2Pos->y;
		vertex.position = TPosition(fimgWidthHalf * fx + fxCenter - 0.5f, fimgHeightHalf * fy + fyCenter - 0.5f, 0.0f);
		vertex.texCoord = TTextureCoordinate(euh * fx + euh, evh * fy + evh);
		vertex.diffuse = m_DiffuseColor;
		vertices.push_back(vertex);
		++iTriCount;
	}
