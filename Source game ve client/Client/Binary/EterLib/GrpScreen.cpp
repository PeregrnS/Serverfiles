...
public:
	...
	{
		IDirect3DVertexShader9* m_dwVS = nullptr;
...
BOOL CScreen::IsLostDevice()
{
	...
	if (ms_hWnd)
	{
		...
		if (hrCS == D3DERR_DEVICELOST || hrCS == D3DERR_DEVICENOTRESET)
			return TRUE;
	}
	...
	if (FAILED(hrTestCooperativeLevel))
	{
		...
		if (D3DERR_DEVICENOTRESET == hrTestCooperativeLevel)
		{
			...
			if (FAILED(rkD3D.GetAdapterDisplayModeEx(iD3DAdapterInfo, &desktopEx, &rotation)))
				return FALSE;
			rkD3DDMDesktop.Width = desktopEx.Width;
			rkD3DDMDesktop.Height = desktopEx.Height;
			rkD3DDMDesktop.RefreshRate = desktopEx.RefreshRate;
			rkD3DDMDesktop.Format = desktopEx.Format;

			rkD3DPP.BackBufferFormat = rkD3DDMDesktop.Format;

			CGraphicDevice::DestroyDefaultIndexBuffersBeforeReset();
			HRESULT hrReset = CGraphicBase::ResetDeviceEx(&rkD3DPP);
			if (FAILED(hrReset))
			{
#ifdef ENABLE_BETTER_RENDERING_WHILE_CLIENT_IS_MINIMIZED
...
#endif
				CGraphicDevice::CreateDefaultIndexBuffersAfterReset();
				return FALSE;
			}
			if (!CGraphicDevice::CreateDefaultIndexBuffersAfterReset())
				return FALSE;
...
void CScreen::Show(HWND hWnd)
{
	...
	{
		HRESULT hr = ms_lpd3dDevice->PresentEx(NULL, NULL, hWnd, NULL, 0);
