...
#ifdef ENABLE_PREMIUM_PRIVATE_SHOP
	if (m_bInputMode == MODE_CURRENCY)
	{
		// Accept digits + k/K
		if (!iswdigit(c) && c != 'k' && c != 'K')
			return;
		...
		GetText(strCurrentText);

		// @note: 'K" amplifier can only be inserted after a number and a number can only be inserted before amplifier
		if (iswdigit(c))
		{
			// We're writing a number after an amplifier
			if (ms_curpos && !iswdigit(strCurrentText[ms_curpos - 1]))
				return;
		}
		else
		{
			// We're writing an amplifier before a number
			if (ms_curpos < strCurrentText.size() && strCurrentText[ms_curpos + 1])
				return;
		}
	}
#endif
...
bool CIME::IsMax(const wchar_t* wInput, int len)
{
	...
	if (m_bInputMode == MODE_CURRENCY)
	{
		...
		for (const char c : strCurrentText)
		{
			if (c == 'K')
				textLen += 2; // We add 2, not 3, as one character is already recognized from the 'K' itself
		}
