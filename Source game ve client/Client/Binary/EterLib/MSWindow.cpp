...
static void ApplyWindowTitleBarTheme(HWND hWnd)
{
    if (!hWnd)
        return;

    BOOL dark = IsWindowsDarkModeEnabled() ? TRUE : FALSE;
...
LRESULT CMSWindow::WindowProcedure(HWND hWnd, UINT uiMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uiMsg)
    {
        ...
        case WM_SETTINGCHANGE:
            if (lParam && !lstrcmpiA((LPCSTR)lParam, "ImmersiveColorSet"))
                ApplyWindowTitleBarTheme(hWnd);
            break;
...
#ifdef ENABLE_WIN_DARKMODE
    ApplyWindowTitleBarTheme(m_hWnd);
#endif
...
const char * CMSWindow::RegisterWindowClass(DWORD style, int brush, WNDPROC pfnWndProc, HICON hIcon, int iCursorResource)
{
	...
	sprintf(szClassName, "eter - s%x:b%x:p:%x:%d", style, brush, (DWORD)pfnWndProc, random_range(2, 99955599));
