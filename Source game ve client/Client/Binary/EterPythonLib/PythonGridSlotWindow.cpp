	...
	if (GetPickedSlotList(byWidth, byHeight, &SlotList))
	{
		...
		if (m_isUseMode)
		{
			...
			if (pCenterSlot->isItem)
			{
				...
				RECT Rect;
				Rect.left = m_rect.left + pCenterSlot->ixPosition;
				Rect.top = m_rect.top + pCenterSlot->iyPosition;
				Rect.right = m_rect.left + pCenterSlot->ixPosition + pCenterSlot->byxPlacedItemSize * ITEM_WIDTH;
				Rect.bottom = m_rect.top + pCenterSlot->iyPosition + pCenterSlot->byyPlacedItemSize * ITEM_HEIGHT;
