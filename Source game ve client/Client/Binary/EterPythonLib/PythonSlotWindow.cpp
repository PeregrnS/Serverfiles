...
void CSlotWindow::SetSlotCoolTime(DWORD dwIndex, float fCoolTime, float fElapsedTime)
{
	...
	void CSlotWindow::ActivateSlot(DWORD dwIndex, BYTE byColorType)
#else
...
void CSlotWindow::ActivateSlotEffect(DWORD dwIndex, BYTE byColorType)
{
	TSlot * pSlot;
	if (!GetSlotPointer(dwIndex, &pSlot))
	{
	...
	pSlot->bActiveNewEffect = TRUE;
	pSlot->byToggleColorType = byColorType;

	if (!m_apSlotActiveNewEffect[0] || !m_apSlotActiveNewEffect[1] || !m_apSlotActiveNewEffect[2])
	{
		__CreateSlotEnableNewEffect();
	}
}

void CSlotWindow::DeactivateSlotEffect(DWORD dwIndex)
{
	TSlot * pSlot;
	if (!GetSlotPointer(dwIndex, &pSlot))
	{
...
#ifdef ENABLE_HIGHLIGHT_SLOT_NEW_EFFECT
		if (m_apSlotActiveNewEffect[i])
		{
			m_apSlotActiveNewEffect[i]->Update();
		}
#endif
	}
#else
...
void CSlotWindow::OnRender()
{
	...
	for (itor = m_SlotList.begin(); itor != m_SlotList.end(); ++itor)
	{
		...
		if (IS_SET(rSlot.dwState, SLOT_STATE_UNUSABLE_EX))
		{
			CPythonGraphic::Instance().SetDiffuseColor(1.0f, 1.0f, 1.0f, 0.3f);
			CPythonGraphic::Instance().RenderBar2d(m_rect.left + rSlot.ixPosition,
												   m_rect.top + rSlot.iyPosition,
...
#ifdef ENABLE_HIGHLIGHT_SLOT_SYSTEM_EX
				m_apSlotActiveEffect[iSize]->SetDiffuseColorByType(rSlot.byToggleColorType);
#endif
...
#ifdef ENABLE_HIGHLIGHT_SLOT_NEW_EFFECT
		if (rSlot.bActiveNewEffect)
		{
			int iSize = ((int)rSlot.byyPlacedItemSize) - 1;
			if (iSize >= 0 && iSize < 3 && m_apSlotActiveNewEffect[iSize])
			{
				int ix = m_rect.left + rSlot.ixPosition;
				int iy = m_rect.top + rSlot.iyPosition;
				m_apSlotActiveNewEffect[iSize]->SetPosition(ix, iy);
				m_apSlotActiveNewEffect[iSize]->SetDiffuseColorByType(rSlot.byToggleColorType);
				m_apSlotActiveNewEffect[iSize]->Render();
			}
		}
#endif
	}
...
#ifdef ENABLE_HIGHLIGHT_SLOT_NEW_EFFECT
void CSlotWindow::__CreateSlotEnableNewEffect()
{
	__DestroySlotEnableNewEffect();
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		if (i == 0)
		{
			...
			{
				char buf[74 + 1];
				sprintf_s(buf, "d:/ymir work/ui/public/newslotactiveeffect/newitemeffect_0000%02d.sub", (j + 1));
				// TraceError("trying to load %s", buf);
				pEff->AppendImage(buf);
			}
		}
		else if (i == 1)
		...
		m_apSlotActiveNewEffect[i] = pEff;
	}
}
#endif
...
#ifdef ENABLE_HIGHLIGHT_SLOT_NEW_EFFECT
void CSlotWindow::__DestroySlotEnableNewEffect()
{
	for (int i = 0; i < 3; ++i)
	{
		if (m_apSlotActiveNewEffect[i])
		{
			delete m_apSlotActiveNewEffect[i];
			m_apSlotActiveNewEffect[i] = NULL;
		}
	}
}
#endif
...
#ifdef ENABLE_ITEM_MANAGER
void CSlotWindow::SetUsableSlotOnTopWndEx(DWORD dwIndex)
{
	TSlot* pSlot;
	if (!GetSlotPointer(dwIndex, &pSlot))
	{
	...
	REMOVE_BIT(pSlot->dwState, SLOT_STATE_UNUSABLE_EX);
	//pSlot->dwState |= SLOT_STATE_UNUSABLE_EX;
}

void CSlotWindow::SetUnusableSlotOnTopWndEx(DWORD dwIndex)
{
	TSlot* pSlot;
	if (!GetSlotPointer(dwIndex, &pSlot))
	{
	...
	SET_BIT(pSlot->dwState, SLOT_STATE_UNUSABLE_EX);
	//pSlot->dwState ^= SLOT_STATE_UNUSABLE_EX;
}
#endif
