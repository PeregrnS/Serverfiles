...
PyObject* wndMgrIsDisable(PyObject* poSelf, PyObject* poArgs)
{
	UI::CWindow* pWindow;
	if (!PyTuple_GetWindow(poArgs, 0, &pWindow))
		return Py_BuildException();
...
PyObject* wndMgrSetButtonDiffuseColor(PyObject* poSelf, PyObject* poArgs)
{
	UI::CWindow* pWindow;
	if (!PyTuple_GetWindow(poArgs, 0, &pWindow))
		return Py_BuildException();
...
PyObject* wndMgrIsInWindowRect(PyObject* poSelf, PyObject* poArgs)
{
	UI::CWindow* pWindow;
	if (!PyTuple_GetWindow(poArgs, 0, &pWindow))
		return Py_BuildException();

	return Py_BuildValue("b", pWindow->IsIn());
}
...
PyObject * wndMgrActivateSlotEffect(PyObject * poSelf, PyObject * poArgs)
{
	...
	UI::CSlotWindow * pSlotWin = (UI::CSlotWindow *)pWin;
	pSlotWin->ActivateSlotEffect(iSlotIndex, iColorType);
	return Py_BuildNone();
}
...
PyObject* wndMgrSetUsableSlotOnTopWndEx(PyObject* poSelf, PyObject* poArgs)
{
	UI::CWindow* pWnd;
	if (!PyTuple_GetWindow(poArgs, 0, &pWnd))
	{
	...
	UI::CSlotWindow* pSlotWnd = (UI::CSlotWindow*)pWnd;
	pSlotWnd->SetUsableSlotOnTopWndEx(iSlotIndex);
	return Py_BuildNone();
}

PyObject* wndMgrSetUnusableSlotOnTopWndEx(PyObject* poSelf, PyObject* poArgs)
{
	UI::CWindow* pWnd;
	if (!PyTuple_GetWindow(poArgs, 0, &pWnd))
	{
	...
	UI::CSlotWindow* pSlotWnd = (UI::CSlotWindow*)pWnd;
	pSlotWnd->SetUnusableSlotOnTopWndEx(iSlotIndex);
	return Py_BuildNone();
}
#endif
...
PyObject* wndMgrGetMouseLocalPositionByPickedWin(PyObject* poSelf, PyObject* poArgs)
{
	UI::CWindow* pPickedWin = UI::CWindowManager::Instance().GetPointWindow();
	if (!pPickedWin)
	{
	...
	long lx, ly;
	pPickedWin->GetMouseLocalPosition(lx, ly);
...
void initwndMgr()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "IsDisable",					wndMgrIsDisable,					METH_VARARGS },
		{ "SetButtonDiffuseColor",		wndMgrSetButtonDiffuseColor,		METH_VARARGS },
		{ "SetButtonScale",				wndMgrSetButtonScale,				METH_VARARGS },
		{ "IsInWindowRect",				wndMgrIsInWindowRect,				METH_VARARGS },
...
#ifdef ENABLE_ITEM_MANAGER
		{ "SetUsableSlotOnTopWndEx",	wndMgrSetUsableSlotOnTopWndEx,		METH_VARARGS },
		{ "SetUnusableSlotOnTopWndEx",	wndMgrSetUnusableSlotOnTopWndEx,	METH_VARARGS },
#endif
		...
		{ "GetMouseLocalPositionByPickedWin",			wndMgrGetMouseLocalPositionByPickedWin,			METH_VARARGS},
...
//	PyObject * poSlotModule = Py_InitModule("wndSlot", s_methods);
	...
	PyModule_AddIntConstant(poModule, "COLOR_TYPE_VIOLET",				UI::COLOR_TYPE_VIOLET);
