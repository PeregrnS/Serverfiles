...
float CActorInstance::__GetInvisibleTimeAdjust(const UINT uiSkill, const NRaceData::TAttackData& c_rAttackData) {
	...
	const double dValueScale = 1.1; // edit this with lower values and u can test which is ok value

	static const int shamanw = 3, shamanm = 7;

	if ((GetRace() != shamanw && GetRace() != shamanm) ||
		uiSkill != 0 ||
		m_fAtkSpd < dValueScale)
		return 0.0f;

	const auto scale = (m_fAtkSpd - dValueScale) / dValueScale;
