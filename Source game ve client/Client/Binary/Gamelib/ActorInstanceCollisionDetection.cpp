...
extern DWORD g_dwCurrentTargetVID;
...
{
	const bool bIsNewRotationMaker = (IsPC() && !rVictim.IsPC() && !rVictim.IsStone() && m_pkHorse && (CItemData::WEAPON_DAGGER == m_awWeaponType || CItemData::WEAPON_BELL == m_awWeaponType));
	...
	for (; itorHitData != pad->HitDataContainer.end(); ++itorHitData)
	{
		...
		// START_OF_FIX_ATTACK_DUALSWORD
		if (bIsNewRotationMaker)
		{
			NRaceData::THitTimePositionMap::const_iterator range_start, range_end;
			range_start = c_rHitData.mapHitPosition.lower_bound(motiontime - CTimer::Instance().GetElapsedSecond());
			range_end = c_rHitData.mapHitPosition.upper_bound(motiontime);
			...
			for (; range_start != range_end; ++range_start)
			{
				...
				CDynamicSphereInstance dsi;
				dsi = dsiSrc;
				dsi.fRadius = 110.0f; // Radius attack

				// Set last|current position of hitPos to be your current position *with radius he hit all monster in that range.
				dsi.v3Position = GetPosition();
				dsi.v3LastPosition = GetPosition();
...
*/
	...
	if (rVictim.IsStone() && m_isMain && g_dwCurrentTargetVID != rVictim.GetVirtualID())
	{
		CPythonCharacterManager& rkChrMgr = CPythonCharacterManager::Instance();
		auto pkInstance = rkChrMgr.GetInstancePtr(rVictim.GetVirtualID());
		if (pkInstance && pkInstance->FindContainerEffect(CInstanceBase::EFFECT_REFINED + CInstanceBase::EFFECT_METIN_QUEUE))
			return FALSE;
	}
