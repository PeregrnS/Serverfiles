...
void CActorInstance::SetShape(DWORD eShape, float fSpecular)
{
	...
	float fNewSpecular = 0.0f;
	if (CPythonNonPlayer::Instance().GetMobSpecular(m_eRace, fNewSpecular))
	{
		if (fNewSpecular > 0.0f)
		{
			fSpecular += fNewSpecular;
		}
	}
...
DWORD * CActorInstance::GetSkillColorByEffectID(DWORD id)
{
	...
	if (motion_index > CInstanceBase::SKILL_COSTUME_EFFECT_START)
	{
		motion_index -= CInstanceBase::SKILL_COSTUME_EFFECT_START;
		while (motion_index > ((CPythonSkill::SKILL_GRADEGAP * CPythonSkill::SKILL_GRADE_COUNT) + 21 + CRaceMotionData::NAME_SKILL))
			motion_index -= 100;
	}
#endif
