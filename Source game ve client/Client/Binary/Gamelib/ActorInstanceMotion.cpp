...
BOOL CActorInstance::IsUsingSkill()
{
	...
	if ((dwCurMotionIndex >= CRaceMotionData::NAME_SKILL_COSTUME && dwCurMotionIndex < CRaceMotionData::NAME_SKILL_COSTUME_END)
		|| (dwCurMotionIndex >= 5000/*CInstanceBase::SKILL_COSTUME_EFFECT_START*/ && dwCurMotionIndex < 5100/*CInstanceBase::SKILL_COSTUME_EFFECT_END*/))
	{
		return TRUE;
	}
#endif
...
DWORD CActorInstance::__SetMotion(const SSetMotionData& c_rkSetMotData, DWORD dwRandMotKey)
{
	...
	if (GetRace() == 9573) //Sunset Big Boss
	{
		if (uNextMot == CRaceMotionData::NAME_RUN)
		{
			SetScreenEffectWaving(50.00f, 300); // max 800
		}
		else
		{
	...
	else if (GetRace() == 60006) //Fire Big Boss
	{
		if (uNextMot == CRaceMotionData::NAME_RUN)
		{
			SetScreenEffectWaving(50.00f, 200); // Meh Wave
		}
		else
		{
			SetScreenEffectWavingEnd();
...
#if defined(ENABLE_RENDER_TARGET) || defined(ENABLE_EMOTION_HIDE_WEAPON)
WORD CActorInstance::NextValidMotion()
{
	assert(m_pkCurRaceData);

	WORD i = 0;
	for (i = __GetCurrentMotionIndex() + 1; i; ++i)
	{
		if (i > CRaceMotionData::NAME_SPECIAL_6)
		{
			i = CRaceMotionData::NAME_WAIT;
		}

		MOTION_KEY MotionKey;
		if (m_pkCurRaceData->GetMotionKey(m_wcurMotionMode, i, &MotionKey))
		{
			break;
		}
	}
