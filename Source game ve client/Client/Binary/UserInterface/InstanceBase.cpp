...
const float c_fDefaultHorseRotationSpeed = 1500.0f;	// default 300.0f
...
BOOL CInstanceBase::IsInBlock()
{
	const TPixelPosition& c_rkPPosCur = m_GraphicThingInstance.NEW_GetCurPixelPositionRef();
	CPythonBackground& background = CPythonBackground::Instance();
	if (background.isAttrOn(c_rkPPosCur.x, c_rkPPosCur.y, CTerrainImpl::ATTRIBUTE_BLOCK) || background.isAttrOn(c_rkPPosCur.x, c_rkPPosCur.y, (1 << 7)) || background.isAttrOn(c_rkPPosCur.x, c_rkPPosCur.y, CTerrainImpl::ATTRIBUTE_WATER))
		return TRUE;
...
bool CInstanceBase::Create(const SCreateData& c_rkCreateData)
{
	...
	if (IsPC())
	{
...
#ifdef ENABLE_SHINING_ITEM_SYSTEM
		AttachShiningEffect(c_rkCreateData.m_dwWeapon);
#endif
		...
		ChangeSkillMotion(c_rkCreateData.m_bSkillMotion);
#endif

		m_byTitleIndex = c_rkCreateData.m_byTitleIndex;
...
BYTE CInstanceBase::GetSpecialTitle() const
{
	return m_byTitleIndex;
}

void CInstanceBase::SetSpecialTitle(BYTE byIndex)
{
	m_byTitleIndex = byIndex;
	RefreshSpecialTitleTextTail();
}
...
#if defined(__BL_GRAPHIC_ON_OFF__)
	if (!IsMainInstance() && HasPrivateShopSign())
...
void CInstanceBase::RefreshStoneScale()
{
	...
	float fRatio = 2.0f + CPythonSystem::Instance().GetStoneScale(); 
	
	if (GetRace() == 11505 || GetRace() == 11506 || GetRace() == 11507 || GetRace() == 11508)
	{
		fRatio = 1.0f; // Erzwingt die Originalgröße (100%)
	}
	
	if (fRatio >= 2.0f) {
		m_GraphicThingInstance.SetScale(2.0f, 2.0f, 2.0f, true);
	}
	else if (fRatio <= 1.0f) {
		m_GraphicThingInstance.SetScale(1.0f, 1.0f, 1.0f, true);
	}
	else {
		m_GraphicThingInstance.SetScale(fRatio, fRatio, fRatio, true);
	}
...
BOOL CInstanceBase::IsBoss()
{
	...
	// if (pkTab != nullptr)
	// {
	// 	if (pkTab->bRank == 5 && !IsStone() && pkTab->bType == 0)
	// 		return TRUE;
	// }
...
DWORD CInstanceBase::GetAura()
{
	return GetPart(CRaceData::PART_AURA);
}
#endif
#endif
...
void CInstanceBase::SetShining(BYTE num, DWORD dwShining)
{
	...
	if (pItemData && pItemData->GetType() == CItemData::ITEM_TYPE_SHINING || dwShining == 0)
	{
		...
		// TraceError("SetShining::num: %d", num);
		AttachShiningEffect(m_awPart[CRaceData::PART_WEAPON]);
	}
}
#endif
				...
				if (pItem)
				{
					if (pItem->GetType() == CItemData::ITEM_TYPE_WEAPON)
					{
						...
						case CItemData::WEAPON_TWO_HANDED:
							m_swordShiningRight[i] = EFFECT_SHINING_WEAPON + EFFECT_SHINING_WEAPON_TWO_HANDED + __GetShiningEffect(i);
							break;
...
UINT CInstanceBase::__GetRefinedEffect(CItemData* pItem)
{
	...
	{
		AttachShiningEffect(pItem->GetIndex());
	}
	else
	{
		AttachShiningEffect(m_awPart[CRaceData::PART_WEAPON]);
...
void CInstanceBase::__GetShiningEffect(CItemData* pItem)
{
	...
	else if (pItem->GetType() == CItemData::ITEM_TYPE_COSTUME)
	{
		...
		{
			if (pItem->GetIndex() == 86133 || pItem->GetIndex() == 86134 ||
				pItem->GetIndex() == 86135 || pItem->GetIndex() == 86136 ||
				pItem->GetIndex() == 86137 || pItem->GetIndex() == 86138 ||
				pItem->GetIndex() == 86139 || pItem->GetIndex() == 86140 ||
				pItem->GetIndex() == 86141 || pItem->GetIndex() == 86142 ||
				pItem->GetIndex() == 86143 || pItem->GetIndex() == 86144 ||
				pItem->GetIndex() == 86145 || pItem->GetIndex() == 86146 ||
				pItem->GetIndex() == 86147 || pItem->GetIndex() == 86148 ||
				pItem->GetIndex() == 86149 || pItem->GetIndex() == 86160 ||
				pItem->GetIndex() == 86161 || pItem->GetIndex() == 86162 ||
				pItem->GetIndex() == 86163 || pItem->GetIndex() == 86164 ||
				pItem->GetIndex() == 86165 || pItem->GetIndex() == 86166 ||
				pItem->GetIndex() == 86167 || pItem->GetIndex() == 86168 ||
				pItem->GetIndex() == 86169 ||
				pItem->GetIndex() == 86188
				)
			{
				for (int i = 0; i < CItemData::ITEM_SHINING_MAX_COUNT; i++)
				{
...
void CInstanceBase::__AttachWeaponShiningEffect(int effectIndex, const char* effectFileName, const char* boneName)
{
	...
	{
		Tracef("Invalid partname for getting attaching weapon bone name. %s - %s", effectFileName, boneName);
	}
}
...
#ifdef ENABLE_SKILL_COSTUME_SYSTEM
void CInstanceBase::ChangeSkillMotion(const uint8_t* bSkillMotion)
{
	memcpy(m_bSkillMotion, bSkillMotion, sizeof(m_bSkillMotion));
}

uint8_t CInstanceBase::GetSkillMotion(uint8_t bIndex)
{
	return m_bSkillMotion[bIndex];
}

uint32_t CInstanceBase::CheckSkillMotion(uint32_t uMot)
{
	uint8_t bIndex = (uMot % CPythonSkill::SKILL_GRADEGAP);
	const uint8_t bRealIndex = bIndex;
	if (bIndex >= 15)
		bIndex -= 15;

	const uint8_t bSkillMotionValue = GetSkillMotion(bIndex - 1);
	if (0 == bSkillMotionValue)
		return CRaceMotionData::NAME_SKILL + uMot;

	return CRaceMotionData::NAME_SKILL_COSTUME + bRealIndex + (100 * (bSkillMotionValue - 1));
}

uint8_t CInstanceBase::GetSkillIndexByID(uint8_t bSkillID)
{
	switch (bSkillID)
	{
