...
class CInstanceBase
{
	...
	public:
		...
		enum
		{
			...
			AFFECT_RESIST_BUFF,	  			// 42
			AFFECT_CRITICAL_BUFF, 			// 43
			AFFECT_REFLECT_BUFF,  			// 44
			AFFECT_MOV_SPEED_BUFF,			// 45
			AFFECT_ATT_GRADE_BUFF,			// 46
#endif
		...
		enum
		{
			...
			NEW_AFFECT_BIOLOG = 305,
#endif
#ifdef ENABLE_METIN_STONE_QUEUE
			NEW_AFFECT_METIN_QUEUE = 306,
#endif
#ifdef RENEWAL_PICKUP_AFFECT
			NEW_AFFECT_PICKUP_ENABLE = 307,
			NEW_AFFECT_PICKUP_DEACTIVE = 308,
#endif
#ifdef ENABLE_VOTE_4_BUFF
			NEW_AFFECT_VOTE_4_BUFF = 309,
#endif
			NEW_AFFECT_BLEND = 531,
			NEW_AFFECT_FISH_POTION = 547,
#ifdef ENABLE_EXTENDED_BLEND
			NEW_AFFECT_MALL_EX = 548,
			NEW_AFFECT_WATER = 549,
			NEW_AFFECT_BLEND_EX = 550,
#endif
#ifdef ENABLE_BATTLE_PASS
			NEW_AFFECT_BATTLE_PASS = 551,
#endif
		...
		enum
		{
			...
			EFFECT_SHINING_WEAPON_SWORD_END = EFFECT_SHINING_WEAPON_SWORD + 49,
			EFFECT_SHINING_WEAPON_TWO_HANDED,
			EFFECT_SHINING_WEAPON_TWO_HANDED_END = EFFECT_SHINING_WEAPON_TWO_HANDED + 49,
			EFFECT_SHINING_WEAPON_BOW,
			EFFECT_SHINING_WEAPON_BOW_END = EFFECT_SHINING_WEAPON_BOW + 49,
			EFFECT_SHINING_WEAPON_FAN,
			EFFECT_SHINING_WEAPON_FAN_END = EFFECT_SHINING_WEAPON_FAN + 49,
			EFFECT_SHINING_WEAPON_BELL,
			EFFECT_SHINING_WEAPON_BELL_END = EFFECT_SHINING_WEAPON_BELL + 49,
			EFFECT_SHINING_WEAPON_DAGGER_R,
			EFFECT_SHINING_WEAPON_DAGGER_R_END = EFFECT_SHINING_WEAPON_DAGGER_R + 49,
			EFFECT_SHINING_WEAPON_DAGGER_L,
			EFFECT_SHINING_WEAPON_DAGGER_L_END = EFFECT_SHINING_WEAPON_DAGGER_L + 49,
...
#endif
#ifdef ENABLE_PREMIUM_PRIVATE_SHOP
			EFFECT_SELECT_PRIVATE_SHOP,
#endif
#ifdef ENABLE_SHINING_ITEM_SYSTEM
			EFFECT_SHINING_WEAPON,
			EFFECT_SHINING_WEAPON_END = EFFECT_SHINING_WEAPON + EFFECT_SHINING_WEAPON_NUM,
			EFFECT_SHINING_ARMOR,
			EFFECT_SHINING_ARMOR_END = EFFECT_SHINING_ARMOR + 250,
			EFFECT_SHINING_SPECIAL,
			EFFECT_SHINING_SPECIAL_END = EFFECT_SHINING_SPECIAL + 50,
#endif

#ifdef ENABLE_SKILL_COSTUME_SYSTEM
			SKILL_COSTUME_EFFECT_START = 5000,
			SKILL_COSTUME_EFFECT_END = SKILL_COSTUME_EFFECT_START + (100 * (SKILL_COSTUME_EFFECT_COUNT)),
#endif
	...
	public:
		...
		static void RegisterSpecialTitle(int iIndex, const char* c_szTitleName, const DWORD dwColor);
	...
	public:
		...
		void	SetSpecialTitle(BYTE byIndex);
		BYTE	GetSpecialTitle() const;
		void	RefreshSpecialTitleTextTail();
		BYTE	m_byTitleIndex;
	...
	public:
		bool FindContainerEffect(DWORD eEffect)
		{
			auto it = m_kEffectContainer.m_kDct_dwEftID.find(eEffect);
			if (it != m_kEffectContainer.m_kDct_dwEftID.end())
				return true;
	...
	public:
		...
		void SetMetinQueueEffect(bool bStatus);
#endif

#ifdef ENABLE_SKILL_COSTUME_SYSTEM
	public:
		void ChangeSkillMotion(const uint8_t* bSkillMotion);
		uint8_t GetSkillMotion(uint8_t bIndex);
		uint32_t CheckSkillMotion(uint32_t uMot);
		uint8_t GetSkillIndexByID(uint8_t bSkillID);

		uint8_t m_bSkillMotion[ESkillMotionLength::MAX_SKILL_COUNT + ESkillMotionLength::MAX_BUFF_COUNT];
		// #ifdef ENABLE_RENDER_TARGET
		// public:
		// 	void SetSkillPreview(DWORD uSkill, BYTE bSkillSet);
		// #endif
#endif
};
