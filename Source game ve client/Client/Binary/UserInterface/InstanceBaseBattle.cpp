...
bool CInstanceBase::NEW_UseSkill(UINT uSkill, UINT uMot, UINT uMotLoopCount, bool isMovingSkill)
{
	...
	m_GraphicThingInstance.InterceptOnceMotion(CheckSkillMotion(uMot), 0.1f, uSkill, 1.0f);
#else
...
BOOL CInstanceBase::CheckAdvancing()
{
	...
	for(CPythonCharacterManager::CharacterIterator i = rkChrMgr.CharacterInstanceBegin(); i!=rkChrMgr.CharacterInstanceEnd();++i)
	{
		...
		if (!bAttacking && rkActorEach.IsStone())
			continue;
	...
	if (__IsMainInstance() && g_dwCurrentTargetVID != 0)
	{
		CPythonCharacterManager& rkChrMgr = CPythonCharacterManager::Instance();
...
void CInstanceBase::Die()
{
    ...
    std::vector<int> BuffAffectList = {
									...
									, AFFECT_RESIST_BUFF, AFFECT_CRITICAL_BUFF, AFFECT_REFLECT_BUFF
									, AFFECT_MOV_SPEED_BUFF, AFFECT_ATT_GRADE_BUFF
#endif
									};
