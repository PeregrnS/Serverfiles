...
std::map<int, std::pair<std::string, DWORD>> g_SpecialTitles;
...
void CInstanceBase::MyBalls()
{
	...
	*/
	...
	while(damage>0)
	{
		if(index > 7)
		{
			TraceError("MyBalls are itching");
			break;
		}
		num = damage%10;
		damage /= 10;
...
void CInstanceBase::__AttachEmpireEffect(DWORD eEmpire)
{
	...
	if (IsWarp() || IsObject() || IsFlag() || IsResource() || IsNPC()
			|| IsMount()
	   )
	{
		return;
...
#ifdef TEAM_BEFORE_NAME
	if (IsGameMaster()) // GameMaster Text
		snprintf(szText, sizeof(szText), "|cffff0000[Team]|r  |cff98FF33Lv %d |r ", level);
	else
		snprintf(szText, sizeof(szText), "|cff98FF33Lv %u |r ", level);
#else
...
void CInstanceBase::RefreshTextTail()
{
	...
	SetSpecialTitle(m_byTitleIndex);
}
...
void CInstanceBase::RegisterSpecialTitle(int mapIdx, const char* c_SpecialTitle, const DWORD c_Color)
{
	g_SpecialTitles.emplace(mapIdx, std::make_pair(c_SpecialTitle, c_Color));
}

void CInstanceBase::RefreshSpecialTitleTextTail()
{
	if (!IsPC())
		return;

	const int iSpecialTitle = GetSpecialTitle();

	if (iSpecialTitle == 0)
	{
		CPythonTextTail::Instance().DetachSpecialTitle(GetVirtualID());
	}
	else
	{
		auto it = g_SpecialTitles.find(iSpecialTitle);
		if (it != g_SpecialTitles.end())
		{
			CPythonTextTail::Instance().AttachSpecialTitle(GetVirtualID(), it->second.first.c_str(), it->second.second);
...
bool CInstanceBase::IsPossibleEmoticon()
{
	...
	{
		Tracenf("ELTimer_GetMSec() - m_dwEmoticonTime");
		return false;
	}
...
DWORD CInstanceBase::__AttachEffect(UINT eEftType)
{
	...
	case EFFECT_WEAPON + WEAPON_TWOHAND:
		...
		// TraceError("bEffectIndex: %d", bEffectIndex);
	default:
		break;
	}
...
void CInstanceBase::SetMetinQueueEffect(bool bStatus)
{
	if (bStatus)
		__EffectContainer_AttachEffect(EFFECT_REFINED + EFFECT_METIN_QUEUE);
	else
		__EffectContainer_DetachEffect(EFFECT_REFINED + EFFECT_METIN_QUEUE);
}
#endif
