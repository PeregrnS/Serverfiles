...
void SNetworkActorData::__copy__(const SNetworkActorData& src)
{
	...
	memcpy(m_bSkillMotion, src.m_bSkillMotion, sizeof(m_bSkillMotion));
#endif
}
...
CInstanceBase* CNetworkActorManager::__FindActor(SNetworkActorData& rkNetActorData, LONG lDstX, LONG lDstY)
{
	...
	if (!pkInstFind)
	{
		if (__IsVisiblePos(lDstX, lDstY, rkNetActorData))
...
bool CNetworkActorManager::__IsVisibleActor(const SNetworkActorData& c_rkNetActorData)
{
	...
	if (__IsVisiblePos(c_rkNetActorData.m_lCurX, c_rkNetActorData.m_lCurY, c_rkNetActorData))
		return true;
...
bool CNetworkActorManager::__IsVisiblePos(LONG lPosX, LONG lPosY, const SNetworkActorData& c_rkNetActorData)
{
	...
	extern int STONE_STAGE_VIEW_BOUND;

	auto maxDistanceView = CHAR_STAGE_VIEW_BOUND;
	if (c_rkNetActorData.m_bType == CActorInstance::TYPE_STONE)
		maxDistanceView = STONE_STAGE_VIEW_BOUND;

	if (len < maxDistanceView)
		return true;
...
CInstanceBase* CNetworkActorManager::__AppendCharacterManagerActor(SNetworkActorData& rkNetActorData)
{
	...
	kCreateData.m_byTitleIndex = rkNetActorData.byTitleIndex;
...
#endif
#ifdef ENABLE_SKILL_COSTUME_SYSTEM
	memcpy(kCreateData.m_bSkillMotion, rkNetActorData.m_bSkillMotion, sizeof(kCreateData.m_bSkillMotion));
#endif
...
#endif
#ifdef ENABLE_SKILL_COSTUME_SYSTEM
			pMainInstance->ChangeSkillMotion(c_rkNetActorData.m_bSkillMotion);
#endif
		}
...
void CNetworkActorManager::UpdateActor(const SNetworkUpdateActorData& c_rkNetUpdateActorData)
{
	...
	if (pkInstFind)
	{
		...
		pkInstFind->SetSpecialTitle(c_rkNetUpdateActorData.byTitleIndex);
#ifdef ENABLE_SKILL_COLOR_SYSTEM
		pkInstFind->ChangeSkillColor(*c_rkNetUpdateActorData.m_dwSkillColor);
#endif
#ifdef ENABLE_SKILL_COSTUME_SYSTEM
		pkInstFind->ChangeSkillMotion(c_rkNetUpdateActorData.m_bSkillMotion);
#endif
	}
	...
	rkNetActorData.byTitleIndex = c_rkNetUpdateActorData.byTitleIndex;
#ifdef ENABLE_ACCE_COSTUME_SYSTEM
	...
	for (int i = 0; i < CHR_SHINING_NUM; i++)
	{
		rkNetActorData.m_dwShining[i] = c_rkNetUpdateActorData.m_dwShining[i];
	}
#endif
#ifdef ENABLE_QUIVER_SYSTEM
...
#endif
#ifdef ENABLE_SKILL_COSTUME_SYSTEM
	memcpy(rkNetActorData.m_bSkillMotion, c_rkNetUpdateActorData.m_bSkillMotion, sizeof(rkNetActorData.m_bSkillMotion));
#endif
}
