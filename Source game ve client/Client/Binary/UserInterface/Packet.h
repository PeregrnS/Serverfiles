...
{
	MA_LENGTH = 255,
	MULTIPLE_FARM_MAX_ACCOUNT = 2, //number max of accounts per computer, we can put what we want here, but pay attention, need to be >= 1
};
#endif
...
enum
{
...
#ifdef ENABLE_LONGER_GUILD_NAME
	GUILD_NAME_MAX_LEN = 18,
#else
	...
	SHOP_TAB_COUNT_MAX = 4, // default 3
...
#ifdef ENABLE_TRASH_SYSTEM
	TRASH_X_SLOT_COUNT	= 6,
	TRASH_Y_SLOT_COUNT	= 6,
	TRASH_SLOT_MAX_NUM	= TRASH_X_SLOT_COUNT * TRASH_Y_SLOT_COUNT,
#endif
};
...
typedef struct SPacketGCUpdateCostumeVisible
{
	BYTE bHeader;
	bool type[ECostumeHideParts::HIDE_COSTUME_COUNT];
} TPacketGCUpdateCostumeVisible;
#endif
...
enum
{
	GUILDSTORAGE_GC_RECV_GSITEMS,
	GUILDSTORAGE_GC_RECV_OPEN,
	GUILDSTORAGE_GC_REMOVE_ITEM,
	GUILDSTORAGE_GC_ADD_ITEM,
	GUILDSTORAGE_GC_RECV_CLOSE,
	GUILDSTORAGE_GC_GOLD_UPDATE,
	GUILDSTORAGE_GC_RECV_LOG,
	GUILDSTORAGE_GC_SEND_LOGENTRY,


	GUILDSTORAGE_CG_REQUEST_CHECKOUT = 0,
	GUILDSTORAGE_CG_REQUEST_CHECKIN,
	GUILDSTORAGE_CG_REQUEST_MOVE,
	GUILDSTORAGE_CG_REQUEST_CLOSE,
	GUILDSTORAGE_CG_REQUEST_GOLDIN,
	GUILDSTORAGE_CG_REQUEST_GOLDOUT,
};
...
typedef struct
{
	uint16_t slot;
	bool bullshit;
} TPacketGCGuildStorageRemoveItem;


typedef struct
{
	TPlayerItem item;
} TPacketGCGuildStorageAddItem;

typedef struct
{
	long long gold;
} TPacketGuildStorageGoldUpdate;
...
typedef struct
{
	bool clear;
	uint16_t item_count;
} TPacketGCGuildStorageOpenItems;

typedef struct
{
	bool clear;
	uint16_t log_count;
} TPacketGCGuildStorageLog;
...
#endif
	BYTE pos;
	long sockets[ITEM_SOCKET_SLOT_MAX_NUM];
	TPlayerItemAttribute attr[ITEM_ATTRIBUTE_SLOT_MAX_NUM];
} TGuildSafeboxItem;
