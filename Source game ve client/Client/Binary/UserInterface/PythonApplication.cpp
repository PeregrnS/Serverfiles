...
#ifdef ENABLE_RAINBOW_WEAPON_TRACE
...
bool CPythonApplication::Process()
{
	...
	CFratelloSecurity::Instance().Update();
	if (!CFratelloSecurity::Instance().IsHealthy())
		return false;
#endif
...
#ifdef ENABLE_BETTER_RENDERING_WHILE_CLIENT_IS_MINIMIZED
	{
		static char i = 0;
...
#ifdef ENABLE_RAINBOW_WEAPON_TRACE
		CWeaponTrace::ProcessRainbowColor();
#endif
...
bool CPythonApplication::CreateDevice(int width, int height, int Windowed, int bit /* = 32*/, int frequency /* = 0*/)
{
	...
	case CGraphicDevice::CREATE_NO_DIRECTX:
		...
		TraceError("CreateDevice: Direct3D 9Ex (Direct3DCreate9Ex / CreateDeviceEx) not available — check GPU driver and Windows version");
		return false;
...
bool LoadLocaleData(const char* localePath)
{
...
#endif
#ifdef ENABLE_SHINING_TABLE
	snprintf(szShiningTable, sizeof(szShiningTable), "locale/common/ShiningTable.txt");
#endif
#ifdef ENABLE_CHEST_DROP_INFO
	snprintf(szChestDrop, sizeof(szChestDrop), "locale/common/chest_drop");
#endif
#ifdef ENABLE_EVENT_MANAGER
	snprintf(szRewardList, sizeof(szRewardList), "locale/common/event_reward_list.txt");
#endif
#ifdef ENABLE_PREMIUM_PRIVATE_SHOP
	snprintf(szDecorationPath, sizeof(szDecorationPath), "%s/shop_deco.txt", localePath);
#endif
#ifdef ENABLE_EXTENDED_BLEND
	snprintf(szBlendFileName, sizeof(szBlendFileName), "locale/common/blend.txt");
#endif
	snprintf(szMobSpecular, sizeof(szMobSpecular), "locale/common/mob_specular.txt");
#if defined(__BL_CLIENT_LOCALE_STRING__)
...
#ifdef ENABLE_PREMIUM_PRIVATE_SHOP
	if (!rkPrivateShop.LoadDecoration(szDecorationPath))
	{
		TraceError("LoadLocaleData - LoadDecoration(%s) Error", szDecorationPath);
		return false;
	}
#endif
	...
	if (!rkNPCMgr.LoadMobSpecular(szMobSpecular))
	{
		TraceError("LoadLocaleData - LoadMobSpecular(%s) Error", szMobSpecular);
		return false;
	}
...
#ifdef ENABLE_FRATELLO_SECURITY
	if (!CFratelloSecurity::Instance().InitializePostStart())
	{
...
void CPythonApplication::Destroy()
{
	...
	//CSpeedTreeForestD3D9Ex::Instance().Clear();
