...
float CPythonBackground::GetObjectDistance()
{
	return CPythonSystem::instance().GetObjectDistance();
}
#endif
...
void CPythonBackground::__CreateProperty()
{
	...
	{
		m_PropertyManager.Initialize("main/data/property");
	}
}
...
bool CPythonBackground::CheckAdvancing(CInstanceBase * pInstance)
{
	...
	if (pInstance && pInstance->IsMainInstance() && g_dwCurrentTargetVID != 0)
	{
		CPythonCharacterManager& rkChrMgr = CPythonCharacterManager::Instance();
		auto pkInstance = rkChrMgr.GetInstancePtr(g_dwCurrentTargetVID);
		if (pkInstance && pkInstance->IsStone() && pkInstance->FindContainerEffect(CInstanceBase::EFFECT_REFINED + CInstanceBase::EFFECT_METIN_QUEUE))
			return false;
	}
...
void CPythonBackground::RenderOXEventArea(OXArea eArea)
{
	if (m_strMapName != "season1/metin2_map_oxevent")
		return;
	...
	switch (eArea)
	{
	case OXArea::OXEVENT_AREA_O:
		RegisterGuildArea(26199, 22899, 29800, 26400);
		VisibleGuildArea();
		break;
	case OXArea::OXEVENT_AREA_X:
		RegisterGuildArea(22199, 22899, 25900, 26400);
		VisibleGuildArea();
		break;
	}
}
