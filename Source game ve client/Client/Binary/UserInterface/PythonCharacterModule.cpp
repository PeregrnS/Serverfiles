...
PyObject* chrSetCanSelect(PyObject* poSelf, PyObject* poArgs)
{
	int iRace;
	...
	pkInst->SetCanSelect(iRace > 0);

	return Py_BuildNone();
}

PyObject* chrEffectUpdate(PyObject* poSelf, PyObject* poArgs)
{
	CEffectManager::Instance().Update();
	return Py_BuildNone();
}

PyObject* chrRenderAllAttachingEffect(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase* pkInst = CPythonCharacterManager::Instance().GetSelectedInstancePtr();
	if (!pkInst)
		return Py_BuildNone();

	pkInst->RenderAllAttachingEffect();
...
#if defined(ENABLE_GUILD_DRAGONLAIR_SYSTEM) || defined(ENABLE_DROP_INFO)
...
void initchr()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "EffectUpdate",				chrEffectUpdate,					METH_VARARGS },
		{ "RenderAllAttachingEffect",	chrRenderAllAttachingEffect,		METH_VARARGS },
		{ "SetCanSelect",				chrSetCanSelect,					METH_VARARGS },
	...
	PyModule_AddIntConstant(poModule, "MOTION_SKILL_COSTUME", CRaceMotionData::NAME_SKILL_COSTUME);
	...
	PyModule_AddIntConstant(poModule, "AFFECT_MOV_SPEED_BUFF",				CInstanceBase::AFFECT_MOV_SPEED_BUFF);
	PyModule_AddIntConstant(poModule, "AFFECT_ATT_GRADE_BUFF",				CInstanceBase::AFFECT_ATT_GRADE_BUFF);
#endif

	PyModule_AddIntConstant(poModule, "NEW_AFFECT_BLEND", CInstanceBase::NEW_AFFECT_BLEND);
#ifdef ENABLE_EXTENDED_BLEND
	PyModule_AddIntConstant(poModule, "NEW_AFFECT_MALL_EX", CInstanceBase::NEW_AFFECT_MALL_EX);
	PyModule_AddIntConstant(poModule, "NEW_AFFECT_WATER", CInstanceBase::NEW_AFFECT_WATER);
	PyModule_AddIntConstant(poModule, "NEW_AFFECT_BLEND_EX", CInstanceBase::NEW_AFFECT_BLEND_EX);
#endif

#ifdef RENEWAL_PICKUP_AFFECT
	PyModule_AddIntConstant(poModule, "NEW_AFFECT_PICKUP_ENABLE",			CInstanceBase::NEW_AFFECT_PICKUP_ENABLE);
	PyModule_AddIntConstant(poModule, "NEW_AFFECT_PICKUP_DEACTIVE",			CInstanceBase::NEW_AFFECT_PICKUP_DEACTIVE);
#endif

#ifdef ENABLE_HIDE_COSTUME_SYSTEM
	PyModule_AddIntConstant(poModule, "HIDE_COSTUME_HAIR", ECostumeHideParts::HIDE_COSTUME_HAIR);
	PyModule_AddIntConstant(poModule, "HIDE_COSTUME_BODY", ECostumeHideParts::HIDE_COSTUME_BODY);
#ifdef ENABLE_WEAPON_COSTUME_SYSTEM
	PyModule_AddIntConstant(poModule, "HIDE_COSTUME_WEAPON", ECostumeHideParts::HIDE_COSTUME_WEAPON);
#endif
#ifdef ENABLE_ACCE_COSTUME_SYSTEM
	PyModule_AddIntConstant(poModule, "HIDE_COSTUME_ACCE", ECostumeHideParts::HIDE_COSTUME_ACCE);
#endif
#ifdef ENABLE_AURA_COSTUME_SYSTEM
	PyModule_AddIntConstant(poModule, "HIDE_COSTUME_AURA", ECostumeHideParts::HIDE_COSTUME_AURA);
#endif
	PyModule_AddIntConstant(poModule, "HIDE_COSTUME_COUNT", ECostumeHideParts::HIDE_COSTUME_COUNT);
#endif
	...
	PyModule_AddIntConstant(poModule, "NEW_AFFECT_VOTE_4_BUFF", CInstanceBase::NEW_AFFECT_VOTE_4_BUFF);
#endif

#ifdef ENABLE_METIN_STONE_QUEUE
	PyModule_AddIntConstant(poModule, "INSTANCE_TYPE_STONE", CActorInstance::TYPE_STONE);
	PyModule_AddIntConstant(poModule, "NEW_AFFECT_METIN_QUEUE", CInstanceBase::NEW_AFFECT_METIN_QUEUE);
#endif
	...
	PyModule_AddIntConstant(poModule, "NEW_AFFECT_FISH_POTION", CInstanceBase::NEW_AFFECT_FISH_POTION);
}
