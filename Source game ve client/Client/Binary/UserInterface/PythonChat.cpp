...
#endif
#if defined(__BL_MULTI_LANGUAGE_PREMIUM__)
, BYTE bEmpire, const std::string& countryName
#endif
)
{
	// DEFAULT_FONT
	...
	if (strstr(c_szChat, "#ebvs")){return;}CGraphicText* pkDefaultFont = static_cast<CGraphicText*>(DefaultFont_GetResource());
	if (!pkDefaultFont)
	{
...
#ifdef __WHISPER_FUTURES__
void CWhisper::Save(const char* c_szMain, const char* c_szName)
{
	if (!m_ChatLineDeque.size())
		return;

	//block-list
	const std::vector<std::string> m_vecBlackList = {
		"[SYSTEM]",
	};
	if (std::find(m_vecBlackList.begin(), m_vecBlackList.end(), c_szName) != m_vecBlackList.end())
		return;

	char chatLogFileName[124];
	snprintf(chatLogFileName, sizeof(chatLogFileName), "lib/user_data/whisper_temp/%s/%s", c_szMain, c_szName);
	FILE* file = fopen(chatLogFileName, "a+");
	if (!file)
		return;

	for (DWORD j = 0; j < m_ChatLineDeque.size(); ++j)
	{
		TChatLine* pChatLine = m_ChatLineDeque[j];
		fprintf(file, "%d<!!!>%s\n", pChatLine->bType, pChatLine->Instance.GetText().c_str());
	}
...
void CPythonChat::SaveWhispers()
{
	const std::string mainPlayername(CPythonPlayer::Instance().GetName());
	for (const auto& [name, pWhisper] : m_WhisperMap)
		pWhisper->Save(mainPlayername.c_str(), name.c_str());
}
#endif
