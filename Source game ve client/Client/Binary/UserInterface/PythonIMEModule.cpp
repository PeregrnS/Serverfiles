...
PyObject* imeSetCurrencyMode(PyObject* poSelf, PyObject* poArgs)
{
	CPythonIME::Instance().SetCurrencyMode();
	return Py_BuildNone();
}
#endif
...
void initime()
{
	static PyMethodDef s_methods[] =
	{
		...
		#ifdef ENABLE_PREMIUM_PRIVATE_SHOP
		{ "SetCurrencyMode",			imeSetCurrencyMode,				METH_VARARGS },
#endif

		{ NULL,						NULL,						NULL		 },
	};

	PyObject* poModule = Py_InitModule("ime", s_methods);

#ifdef ENABLE_PREMIUM_PRIVATE_SHOP
	PyModule_AddIntConstant(poModule, "MODE_STRING", CIME::MODE_STRING);
	PyModule_AddIntConstant(poModule, "MODE_NUMBER", CIME::MODE_NUMBER);
	PyModule_AddIntConstant(poModule, "MODE_CURRENCY", CIME::MODE_CURRENCY);
#endif
}
