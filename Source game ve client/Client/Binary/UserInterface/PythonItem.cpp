...
#if defined(__BL_CLOSE_ITEM_TEXT_TAIL_COLOR__)
bool CPythonItem::CanPickGroundItem(DWORD dwVirtualID) const
{
	TGroundItemInstanceMap::const_iterator itor = m_GroundItemInstanceMap.find(dwVirtualID);
	if (m_GroundItemInstanceMap.end() == itor)
		return false;

	CInstanceBase* mainInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();
	if (!mainInstance)
		return false;

	const TGroundItemInstance* itemInstance = itor->second;
	const D3DXVECTOR3& endPosition = itemInstance->v3EndPosition;

	const DWORD pickableDistance = CPythonPlayer::Instance().__GetPickableDistance();

#if 1
	/*CPythonItem::GetCloseItem*/
	TPixelPosition pixelPos;
	mainInstance->NEW_GetPixelPosition(&pixelPos);

	DWORD dwxDistance = DWORD(pixelPos.x - endPosition.x);
	DWORD dwyDistance = DWORD(pixelPos.y - (-endPosition.y));
	DWORD dwDistance = DWORD(dwxDistance * dwxDistance + dwyDistance * dwyDistance);

	return (dwDistance <= float(pickableDistance) * float(pickableDistance));
#else
	return (mainInstance->GetDistance(endPosition) <= static_cast<float>(pickableDistance));
#endif
}
#endif
