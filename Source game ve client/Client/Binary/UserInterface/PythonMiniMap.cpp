...
void CPythonMiniMap::Update(float fCenterX, float fCenterY)
{
	...
	for(i = rkChrMgr.CharacterInstanceBegin(); i!=rkChrMgr.CharacterInstanceEnd(); ++i)
	{
		...
		else if (pkInstEach->IsStone())
		{
			...
			m_StonesPositionVector.push_back(aMarkPosition);
		}
#endif
	}
...
void CPythonMiniMap::Render(float fScreenX, float fScreenY)
{
	...
	if (m_fScale >= 2.0f)
	{
		...
		for (const auto& it : m_BossPositionVector)
		{
			m_BossMark.SetPosition(it.m_fX, it.m_fY);
			m_BossMark.Render();
		}

		// Stones
		STATEMANAGER.SetRenderState(D3DRS_TEXTUREFACTOR, CInstanceBase::GetIndexedNameColor(CInstanceBase::NAMECOLOR_STONES));
		for (const auto& it : m_StonesPositionVector)
		{
			m_WhiteMark.SetPosition(it.m_fX, it.m_fY);
			m_WhiteMark.Render();
		}
#endif
	}
...
#endif
#ifdef ENABLE_ATLASS_EXTENDED
	const std::string strBossMark = strImageRoot + "minimap/field_boss_mark.tga";
#endif
...
#ifdef ENABLE_ATLASS_EXTENDED
void CPythonMiniMap::ClearAtlasBossInfo()
{
	m_AtlasBossInfoVector.clear();
}
#endif
...
#ifdef ENABLE_ATLASS_EXTENDED
void CPythonMiniMap::RegisterAtlasBoss(const char* c_szName, long lx, long ly, DWORD dwTime)
{
	TAtlasMarkInfo aAtlasMarkInfo;
	...
	aAtlasMarkInfo.m_fY = float(ly);
	aAtlasMarkInfo.m_strText = c_szName;
	aAtlasMarkInfo.m_respTime = dwTime;
...
)
{
	...
	while (m_AtlasMarkInfoVectorIterator != m_AtlasBossInfoVector.end())
	{
			...
			rAtlasMarkInfo.m_fY - fCheckWidth / 2 < fRealY && rAtlasMarkInfo.m_fY + fCheckHeight > fRealY)
		{
			...
			*pTime = rAtlasMarkInfo.m_respTime;
			return true;
		}
		++m_AtlasMarkInfoVectorIterator;
	}
