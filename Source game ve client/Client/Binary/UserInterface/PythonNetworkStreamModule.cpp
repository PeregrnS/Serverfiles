...
{
	unsigned long long ullElk = 0;
	if (!PyTuple_GetUnsignedLongLong(poArgs, 0, &ullElk))
	{
		return Py_BuildException();
	}

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendExchangeElkAddPacket(ullElk);
	return Py_BuildNone();
}
#else
PyObject* netSendExchangeElkAddPacket(PyObject* poSelf, PyObject* poArgs)
...
PyObject* netSendSkillGroupPacket(PyObject* poSelf, PyObject* poArgs)
{
	BYTE bGroup;
	if (!PyTuple_GetByte(poArgs, 0, &bGroup))
	{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendSkillGroupPacket(bGroup);
...
PyObject* netSendHwidBanPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendHwidBanPacket(bMode, szPlayer, szReason);
	return Py_BuildNone();
}
#endif
...
PyObject* netSendHuntingAction(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream::Instance().SendHuntingAction(bAction, (DWORD)dValue);
	return Py_BuildNone();
}
#endif
...
PyObject * netChangeCostumeVisible(PyObject * poSelf, PyObject * poArgs)
{
	BYTE type = 0;
	if (!PyTuple_GetByte(poArgs, 0, &type))
	{
		return Py_BuildException();
	}

	bool status;
	if (!PyTuple_GetBoolean(poArgs, 1, &status))
	{
		return Py_BuildException();
	}

	CPythonNetworkStream & rkNetStream = CPythonNetworkStream::instance();
	rkNetStream.ChangeCostumeVisible(type, status);
	return Py_BuildNone();
}
#endif

#ifdef ENABLE_SPLIT_BY_COUNT
PyObject* netSendSplitItemByCount(PyObject* poSelf, PyObject* poArgs)
{
	TItemPos Cell;
	if (!PyTuple_GetByte(poArgs, 0, &Cell.window_type))
		return Py_BuildException();
	...
	CPythonNetworkStream & rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendSplitItemByCount(Cell, (short)iCount);
	return Py_BuildNone();
}
#endif
...
PyObject* netSendBattlePassAction(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream::Instance().SendBattlePassAction(bAction);
	return Py_BuildNone();
}
#endif
...
{
	int iGold;
	if (!PyTuple_GetInteger(poArgs, 0, &iGold))
		return Py_BuildException();

	CPythonNetworkStream& rns = CPythonNetworkStream::Instance();
	rns.SendGuildSafeboxGiveGoldPacket(iGold);
...
PyObject* netSendGuildSafeboxTakeGoldPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rns = CPythonNetworkStream::Instance();
	rns.SendGuildSafeboxTakeGoldPacket(iGold);
...
#ifdef ENABLE_SKILL_GROUP_GUI
		{ "SendSkillGroupPacket",					netSendSkillGroupPacket,					METH_VARARGS },
#endif
...
#ifdef ENABLE_HIDE_COSTUME_SYSTEM
		{"SendChangeCostumeVisibility", 			netChangeCostumeVisible, 					METH_VARARGS },
#endif

#ifdef ENABLE_SPLIT_BY_COUNT
		{ "SendSplitItemByCount",					netSendSplitItemByCount,					METH_VARARGS },
#endif
