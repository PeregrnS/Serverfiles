...
bool CPythonNetworkStream::__RecvPlayerDragonPoints()
{
	TPacketGCDragonPoints DragonPointsPacket;

	if (!Recv(sizeof(TPacketGCDragonPoints), &DragonPointsPacket))
	{
		return false;
	}

	CPythonPlayer::Instance().SetCash(DragonPointsPacket.dwCash);
	CPythonPlayer::Instance().SetCoins(DragonPointsPacket.dwCoins);
