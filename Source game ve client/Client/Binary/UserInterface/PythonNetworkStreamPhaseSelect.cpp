...
void CPythonNetworkStream::SetSelectPhase()
{
...
#endif
#if defined(ENABLE_LOADING_PERFORMANCE)
	CPythonLoading::Instance().JoinThreadLoading();
#endif
	...
	if (__DirectEnterMode_IsSet())
	{
		...
		{
			PyCallClassMemberFunc(m_poHandler, "SetWarpPhase", Py_BuildValue("()"));
			return;
		}
#endif
...
void CPythonNetworkStream::SelectPhase()
{
	...
	switch (header)
	{
		...
		case HEADER_GC_CHARACTER_DRAGON_POINTS_CHANGE:
			...
			Recv(sizeof(TPacketGCDragonPointsChange), &DragonPointsChange);
			return;
			break;
