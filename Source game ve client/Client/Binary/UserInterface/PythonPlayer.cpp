...
void CPythonPlayer::SPlayerStatus::SetCash(DWORD dwCash)
{
	m_dwCash = dwCash;
}

DWORD CPythonPlayer::SPlayerStatus::GetCash()
{
	return m_dwCash;
}

void CPythonPlayer::SPlayerStatus::SetCoins(DWORD dwCoins)
{
	m_dwCoins = dwCoins;
}

DWORD CPythonPlayer::SPlayerStatus::GetCoins()
{
	return m_dwCoins;
}
...
void CPythonPlayer::Update()
{
	...
	CPythonPlayer::Instance().SetMetinstonesQueueCount2(3);
	MetinstonesQueueLoop();
#endif
...
DWORD CPythonPlayer::GetArmor()
{
	...
	return pkInstMain->GetArmor();
}

DWORD CPythonPlayer::GetWeapon()
{
	CInstanceBase* pkInstMain = NEW_GetMainActorPtr();
	...
	return pkInstMain->GetWeapon();
}

DWORD CPythonPlayer::GetHair()
{
	CInstanceBase* pkInstMain = NEW_GetMainActorPtr();
	...
	return pkInstMain->GetHair();
}

#ifdef ENABLE_ACCE_COSTUME_SYSTEM
DWORD CPythonPlayer::GetAcce()
{
	CInstanceBase* pkInstMain = NEW_GetMainActorPtr();
	...
	return pkInstMain->GetAcce();
}
#endif

#ifdef ENABLE_AURA_COSTUME_SYSTEM
DWORD CPythonPlayer::GetAura()
{
	CInstanceBase* pkInstMain = NEW_GetMainActorPtr();
...
int CPythonPlayer::GetSkillRemainBook(DWORD dwSlotIndex)
{
	if (dwSlotIndex >= SKILL_MAX_NUM)
		return 0;

	return m_playerStatus.aSkill[dwSlotIndex].iBooks;
}
#endif
...
#ifdef ENABLE_SKILL_DESCRIPTION_RENEWAL
void CPythonPlayer::SetSkillLevel_(DWORD dwSkillIndex, DWORD dwSkillGrade, DWORD dwSkillLevel, DWORD dwBooks)
#else
...
#endif
{
	...
	switch (dwSkillGrade)
	{
		case 0:
			...
			m_playerStatus.aSkill[dwSlotIndex].iLevel = dwSkillLevel;
#ifdef ENABLE_SKILL_DESCRIPTION_RENEWAL
			m_playerStatus.aSkill[dwSlotIndex].iBooks = 0;
#endif
			break;
		case 1:
			m_playerStatus.aSkill[dwSlotIndex].iGrade = dwSkillGrade;
...
#ifdef ENABLE_SKILL_DESCRIPTION_RENEWAL
			m_playerStatus.aSkill[dwSlotIndex].iBooks = dwBooks;
#endif
			break;
		case 2:
			m_playerStatus.aSkill[dwSlotIndex].iGrade = dwSkillGrade;
...
#ifdef ENABLE_GOLD_MAX_EXTENDED
void CPythonPlayer::SetGold(unsigned long long value)
{
	m_playerStatus.SetGold(value);
}

unsigned long long CPythonPlayer::GetGold()
{
	return m_playerStatus.GetGold();
}
#endif

void CPythonPlayer::SetCash(DWORD dwCash)
{
	m_playerStatus.SetCash(dwCash);
}

DWORD CPythonPlayer::GetCash()
{
	return m_playerStatus.GetCash();
}

void CPythonPlayer::SetCoins(DWORD dwCoins)
{
	m_playerStatus.SetCoins(dwCoins);
}

DWORD CPythonPlayer::GetCoins()
{
	return m_playerStatus.GetCoins();
}
...
int CPythonPlayer::GetAffectDuration(DWORD dwType)
{
	int ret = 0, i = 0;
	for (TAffectDataVector::iterator it = m_mapAffectData.begin(); it != m_mapAffectData.end(); ++it, ++i)
	{
