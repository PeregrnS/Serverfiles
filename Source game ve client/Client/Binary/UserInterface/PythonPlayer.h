...
#ifndef ENABLE_RENDER_TARGET_PREVIEW
enum
{
	MAIN_RACE_WARRIOR_M,
...
class CPythonPlayer : public CSingleton<CPythonPlayer>, public IAbstractPlayer
{
...
#ifdef ENABLE_GOLD_MAX_EXTENDED
		void 	SetGold(unsigned long long value);
		unsigned long long GetGold();
#endif
...
#ifdef ENABLE_SKILL_DESCRIPTION_RENEWAL
		int		GetSkillRemainBook(DWORD dwSlotIndex);
#endif
...
#ifdef ENABLE_SKILL_DESCRIPTION_RENEWAL
		void	SetSkillLevel_(DWORD dwSkillIndex, DWORD dwSkillGrade, DWORD dwSkillLevel, DWORD dwBooks);
#else
	...
	public:
		void MetinstonesQueueLoop();
		bool SetMetinstonesQueue(bool isAdd, DWORD dwVirtualID);
		DWORD GetMetinstonesQueueTarget();
		void SetMetinstonesQueueCount(BYTE bCount) { bQueueCount = bCount; }
		void SetMetinstonesQueueCount2(BYTE bCount) { if (bQueueCount < bCount) bQueueCount = bCount; }
	protected:
		BYTE bQueueCount;
		std::vector<DWORD> m_vecMetinstonesQueue;
#endif
	...
	public:
		...
		int GetAffectDuration(DWORD dwType);
#endif
};
