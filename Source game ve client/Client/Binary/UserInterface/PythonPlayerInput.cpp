...
void CPythonPlayer::__SetTargetVID(DWORD dwVID)
{
	...
	g_dwCurrentTargetVID = m_dwTargetVID;
}
...
{
	static DWORD s_dwLastPickPrivateShopID = 0;
...
void CPythonPlayer::SendClickPrivateShopPacket(DWORD dwPrivateShopID)
{
	...
	if (dwCurTime >= s_dwNextTCPTime)
	{
		...
		rkNetStream.SendPrivateShopStartPacket(dwPrivateShopID);
	}
}

bool CPythonPlayer::__GetPickedPrivateShopID(DWORD* pdwPrivateShopID)
{
	CPythonPrivateShop& rkPrivateShopMgr = CPythonPrivateShop::Instance();
	return rkPrivateShopMgr.GetPickedInstanceVID(pdwPrivateShopID);
}
