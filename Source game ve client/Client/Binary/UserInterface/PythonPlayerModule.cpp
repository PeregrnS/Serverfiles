...
PyObject* playerGetSkillRemainBooks(PyObject* poSelf, PyObject* poArgs)
{
	int iSlotIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iSlotIndex))
		return Py_BuildException();

	return Py_BuildValue("i", CPythonPlayer::Instance().GetSkillRemainBook(iSlotIndex));
}
#endif
...
PyObject* playerSendDragonSoulRefine(PyObject* poSelf, PyObject* poArgs)
{
	...
	case DS_SUB_HEADER_DO_UPGRADE_ALL:
	{
		int iKind = 0;
		int iClass = 0;
		if (!PyTuple_GetInteger(poArgs, 1, &iKind))
		{
			return Py_BadArgument();
		}
		if (!PyTuple_GetInteger(poArgs, 2, &iClass))
		{
			return Py_BadArgument();
		}

		RefineItemPoses[0].cell = iKind;
		RefineItemPoses[1].cell = iClass;
	}
	break;
#endif
	}
...
PyObject * playerGetEquippedWeapon(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (NULL != data)
	{
		...
		if (data->transmutation != 0)
		{
			return Py_BuildValue("i", data->transmutation);
		}
#endif
		if (data->vnum != 0)
		{
...
PyObject * playerGetEquippedArmor(PyObject* poSelf, PyObject* poArgs)
{
	...
	const TItemData* data_costume = player->GetItemData(TItemPos(EQUIPMENT, c_Costume_Slot_Start));
...
PyObject* playerIsInBlockArea(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase* pMainInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();
	if (!pMainInstance)
		return Py_BuildValue("i", FALSE);

	return Py_BuildValue("i", pMainInstance->IsInBlock());
}
#endif
...
PyObject* playerCanAct(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase* pInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();
	return Py_BuildValue("b", pInstance->CanAct());
}

PyObject* playerCanAttack(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase* pInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();
	return Py_BuildValue("b", pInstance->CanAttack());
}

PyObject* playerCanMove(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase* pInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();
	return Py_BuildValue("b", pInstance->CanMove());
}

PyObject * playerGetCash(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("i", CPythonPlayer::Instance().GetCash());
}

PyObject * playerGetCoins(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("i", CPythonPlayer::Instance().GetCoins());
}
...
PyObject* playerGetAffectDuration(PyObject* poSelf, PyObject* poArgs)
{
	DWORD dwType;
	if (!PyTuple_GetUnsignedLong(poArgs, 0, &dwType))
	{
...
void initPlayer()
{
	static PyMethodDef s_methods[] =
	{
...
#ifdef ENABLE_SKILL_DESCRIPTION_RENEWAL
		{ "GetRemainingBooks",						playerGetSkillRemainBooks,						METH_VARARGS },
#endif
...
#ifdef ENABLE_SAVE_BLOCK_ATTR
		{ "IsInBlockArea",				playerIsInBlockArea,				METH_VARARGS },
#endif
		...
		{ "CanAct",						playerCanAct,							METH_VARARGS },
		{ "CanAttack",					playerCanAttack,						METH_VARARGS },
		{ "CanMove",					playerCanMove,							METH_VARARGS },

		{ "GetCash",					playerGetCash,								METH_VARARGS },
		{ "GetCoins",					playerGetCoins,								METH_VARARGS },

#ifdef ENABLE_PLAYER_MODULE_CHECK_AFFECT
		{ "CheckAffect",							playerCheckAffect,							METH_VARARGS },
		{ "GetAffectDuration",						playerGetAffectDuration,					METH_VARARGS },
#endif
...
#if defined(ENABLE_RENDER_TARGET_PREVIEW) || defined(ENABLE_INGAME_WIKI)
	PyModule_AddIntConstant(poModule, "MAIN_RACE_WARRIOR_M", CPythonPlayer::MAIN_RACE_WARRIOR_M);
	PyModule_AddIntConstant(poModule, "MAIN_RACE_ASSASSIN_W", CPythonPlayer::MAIN_RACE_ASSASSIN_W);
	PyModule_AddIntConstant(poModule, "MAIN_RACE_SURA_M", CPythonPlayer::MAIN_RACE_SURA_M);
	PyModule_AddIntConstant(poModule, "MAIN_RACE_SHAMAN_W", CPythonPlayer::MAIN_RACE_SHAMAN_W);
	PyModule_AddIntConstant(poModule, "MAIN_RACE_WARRIOR_W", CPythonPlayer::MAIN_RACE_WARRIOR_W);
	PyModule_AddIntConstant(poModule, "MAIN_RACE_ASSASSIN_M", CPythonPlayer::MAIN_RACE_ASSASSIN_M);
	PyModule_AddIntConstant(poModule, "MAIN_RACE_SURA_W", CPythonPlayer::MAIN_RACE_SURA_W);
	PyModule_AddIntConstant(poModule, "MAIN_RACE_SHAMAN_M", CPythonPlayer::MAIN_RACE_SHAMAN_M);
#ifdef ENABLE_WOLFMAN_CHARACTER
	PyModule_AddIntConstant(poModule, "MAIN_RACE_WOLFMAN_M", CPythonPlayer::MAIN_RACE_WOLFMAN_M);
#endif
	PyModule_AddIntConstant(poModule, "MAIN_RACE_MAX_NUM", CPythonPlayer::MAIN_RACE_MAX_NUM);
#else
	...
	PyModule_AddIntConstant(poModule, "CHANGE_EQUIP_SLOT_END", c_change_equip_Slot_End);
	...
	PyModule_AddIntConstant(poModule, "BLOCK_EQUIPMENT_VIEW",		BLOCK_EQUIPMENT_VIEW);
...
#endif
#ifdef ENABLE_ACCE_COSTUME_SYSTEM
	PyModule_AddIntConstant(poModule, "ON_TOP_WND_COMBINE", ON_TOP_WND_COMBINE);
	PyModule_AddIntConstant(poModule, "ON_TOP_WND_ABSORB_PHASE1", ON_TOP_WND_ABSORB_PHASE1);
	PyModule_AddIntConstant(poModule, "ON_TOP_WND_ABSORB_PHASE2", ON_TOP_WND_ABSORB_PHASE2);
#endif
#ifdef ENABLE_ITEM_MANAGER
	PyModule_AddIntConstant(poModule, "ON_TOP_WND_MANAGER", ON_TOP_WND_MANAGER);
#endif
	...
	PyModule_AddIntConstant(poModule, "ON_TOP_WND_CUBE",		ON_TOP_WND_CUBE);
#endif
...
#endif
#endif // WJ_ENABLE_TRADABLE_ICON

#ifdef ENABLE_PREMIUM_PRIVATE_SHOP
	PyModule_AddIntConstant(poModule, "POINT_PRIVATE_SHOP_UNLOCKED_SLOT", POINT_PRIVATE_SHOP_UNLOCKED_SLOT);
	...
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_LEVEL_UP", REWARD_MISSION_LEVEL_UP);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_LEVEL_UP_PET", REWARD_MISSION_LEVEL_UP_PET);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_SKILL_UPGRADE", REWARD_MISSION_SKILL_UPGRADE);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_KILL_MONSTER", REWARD_MISSION_KILL_MONSTER);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_KILL_STONE", REWARD_MISSION_KILL_STONE);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_KILL_BOSS", REWARD_MISSION_KILL_BOSS);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_INVENTORY_SLOT", REWARD_MISSION_INVENTORY_SLOT);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_OFFLINESHOP_SLOT", REWARD_MISSION_OFFLINESHOP_SLOT);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_AVERAGE_BONUS", REWARD_MISSION_AVERAGE_BONUS);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_BATTLEPASS", REWARD_MISSION_BATTLEPASS);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_FIRST_ITEM", REWARD_MISSION_FIRST_ITEM);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_CUSTOM_SASH", REWARD_MISSION_CUSTOM_SASH);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_BIOLOG", REWARD_MISSION_BIOLOG);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_PASSIVE_SKILL_COMPLETE", REWARD_MISSION_PASSIVE_SKILL_COMPLETE);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_DUNGEON", REWARD_MISSION_DUNGEON);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_USE_ITEM", REWARD_MISSION_USE_ITEM);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_SELL_ITEM", REWARD_MISSION_SELL_ITEM);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_BUY_ITEM", REWARD_MISSION_BUY_ITEM);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_PLAYTIME", REWARD_MISSION_PLAYTIME);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION_COMPLETE_SKILL", REWARD_MISSION_COMPLETE_SKILL);
#endif

#ifdef ENABLE_EXTENDED_DS_INVENTORY
	PyModule_AddIntConstant(poModule, "DRAGON_SOUL_INVENTORY_PAGE_COUNT", c_DragonSoul_Inventory_Page_Count);
	PyModule_AddIntConstant(poModule, "DRAGON_SOUL_GRADE_MAX", EDragonSoulGradeTypes::DRAGON_SOUL_GRADE_MAX);
#endif

#ifdef ENABLE_DS_REFINE_ALL
	PyModule_AddIntConstant(poModule, "DS_SUB_HEADER_DO_UPGRADE_ALL", DS_SUB_HEADER_DO_UPGRADE_ALL);
#endif
