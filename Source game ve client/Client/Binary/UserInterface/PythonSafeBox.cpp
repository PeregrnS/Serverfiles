...
void CPythonSafeBox::OpenGuild(int iSize)
{
	...
	{
		TItemData & rInstance = m_GuildItemInstanceVector[i];
		ZeroMemory(&rInstance, sizeof(rInstance));
	}
}

void CPythonSafeBox::SetGuildItemData(DWORD dwSlotIndex, const TItemData & rItemData)
{
...
void CPythonSafeBox::DelGuildItemData(DWORD dwSlotIndex)
{
	...
	TItemData & rInstance = m_GuildItemInstanceVector[dwSlotIndex];
	ZeroMemory(&rInstance, sizeof(rInstance));
}

BOOL CPythonSafeBox::GetGuildItemDataPtr(DWORD dwSlotIndex, TItemData ** ppInstance)
{
	if (dwSlotIndex >= m_GuildItemInstanceVector.size())
	{
...
void CPythonSafeBox::SetGuildMoney(DWORD dwMoney)
{
	m_dwGuildMoney = dwMoney;
}

DWORD CPythonSafeBox::GetGuildMoney()
{
	return m_dwGuildMoney;
}
...
{
	return Py_BuildValue("i", CPythonSafeBox::Instance().GetGuildMoney());
}
#endif
...
//#ifdef ENABLE_GUILD_SAFEBOX
//	PyModule_AddIntConstant(poModule, "GUILD_SAFEBOX_SLOT_X_COUNT", CPythonSafeBox::GUILD_SAFEBOX_SLOT_X_COUNT);
//	PyModule_AddIntConstant(poModule, "GUILD_SAFEBOX_SLOT_Y_COUNT", CPythonSafeBox::GUILD_SAFEBOX_SLOT_Y_COUNT);
//	PyModule_AddIntConstant(poModule, "GUILD_SAFEBOX_PAGE_SIZE", CPythonSafeBox::GUILD_SAFEBOX_PAGE_SIZE);
//#endif
}
