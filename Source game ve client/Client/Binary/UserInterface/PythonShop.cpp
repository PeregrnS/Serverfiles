...
BOOL CPythonShop::GetItemData(BYTE tabIdx, DWORD dwSlotPos, const TShopItemData ** c_ppItemData)
{
	if (tabIdx >= SHOP_TAB_COUNT_MAX || dwSlotPos >= SHOP_HOST_ITEM_MAX_NUM)
	{
		// TODO : check why this is triggered with shopEx
		//TraceError("Out of Index. tabIdx(%d) must be less than %d. dwSlotPos(%d) must be less than %d", tabIdx, SHOP_TAB_COUNT_MAX, dwSlotPos, SHOP_HOST_ITEM_MAX_NUM);
		return FALSE;
	}
...
void CPythonShop::ClearPrivateShopStock()
{
	...
	void CPythonShop::AddPrivateShopItemStock(TItemPos ItemPos, BYTE dwDisplayPos, unsigned long long dwPrice)
#else
...
#ifdef ENABLE_GOLD_MAX_EXTENDED
	unsigned long long CPythonShop::GetPrivateShopItemPrice(TItemPos ItemPos)
#else
...
#ifdef ENABLE_GOLD_MAX_EXTENDED
	unsigned long long iValue = CPythonShop::Instance().GetPrivateShopItemPrice(TItemPos(bItemWindowType, wItemSlotIndex));
...
PyObject* shopGetItemPriceItem(PyObject* poSelf, PyObject* poArgs)
{
	...
	int info;
	if (!PyTuple_GetInteger(poArgs, 1, &info))
	{
		return Py_BuildException();
	}
	...
	if (CPythonShop::Instance().GetItemData(iIndex, &c_pItemData))
	{
		return Py_BuildValue("i", c_pItemData->item_price[info]);
	}

	return Py_BuildValue("i", 0);
}

PyObject* shopGetItemVnumItem(PyObject* poSelf, PyObject* poArgs)
{
	int iIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iIndex))
	{
	...
	if (CPythonShop::Instance().GetItemData(iIndex, &c_pItemData))
	{
		return Py_BuildValue("i", c_pItemData->item_vnum[info]);
	}
...
void initshop()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetItemVnumItem",			shopGetItemVnumItem,			METH_VARARGS },
		{ "GetItemVnumPrice",			shopGetItemPriceItem,			METH_VARARGS },
#endif
		{ NULL,							NULL,							NULL },
	};
...
#if defined(ENABLE_RENEWAL_SHOPEX)
	PyModule_AddIntConstant(poModule, "SHOP_MAX_ITEM_NUM", SHOP_MAX_ITEM_NUM);
	...
	PyModule_AddIntConstant(poModule, "SHOPEX_GEM", SHOPEX_GEM);
#endif
