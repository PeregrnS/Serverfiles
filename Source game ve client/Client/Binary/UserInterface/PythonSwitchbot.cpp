...
void CPythonSwitchbot::Initialize()
{
	...
	m_map_AttributesRareBySet.clear(); // # GRM Update 16112023
}
...
void CPythonSwitchbot::AddAttributeRareToMap(const TSwitchbottAttributeTable& table) // # GRM Update 16112023
{
	const auto& it = m_map_AttributesRareBySet.find(table.attribute_set);
	if (it == m_map_AttributesRareBySet.end())
	{
		std::map<BYTE, long> attribute_map;
		attribute_map.insert(std::make_pair(table.apply_num, table.max_value));

		m_map_AttributesRareBySet.insert(std::make_pair(table.attribute_set, attribute_map));
	}
	else
	{
...
void CPythonSwitchbot::GetAttributesBySet(int iAttributeSet, std::vector<TPlayerItemAttribute>& vec_attributes, BYTE bAttrIndex)
{
	if (iAttributeSet == -1)
	{
		return;
	...
	auto& it = m_map_AttributesBySet.find(iAttributeSet);
	auto& itEnd = m_map_AttributesBySet.end();

	// rare # GRM Update 16112023
	if (bAttrIndex >= 5)
	{
		it = m_map_AttributesRareBySet.find(iAttributeSet);
		itEnd = m_map_AttributesRareBySet.end();
	}
	// END
...
long CPythonSwitchbot::GetAttributeMaxValue(int iAttributeSet, BYTE attrType, BYTE bAttrIndex)
{
	if (iAttributeSet == -1)
	{
		return 0;
...
PyObject* switchbotGetAttributesForSet(PyObject* poSelf, PyObject* poArgs)
{
	...
	BYTE bAttrIndex;
	if (!PyTuple_GetByte(poArgs, 1, &bAttrIndex))
	{
		return Py_BuildException();
	}
	...
	CPythonSwitchbot::Instance().GetAttributesBySet(iAttributeSet, vec_attributes, bAttrIndex);
...
PyObject* switchbotGetAttributeMaxValue(PyObject* poSelf, PyObject* poArgs)
{
	...
	long maxValue = CPythonSwitchbot::Instance().GetAttributeMaxValue(iAttributeSet, bAttrType, bAttrIndex);
