...
#pragma pack()
	...
	void AddAttributeRareToMap(const TSwitchbottAttributeTable& table);
	void AddAttributeToMap(const TSwitchbottAttributeTable& table);
	void GetAttributesBySet(int iAttributeSet, std::vector<TPlayerItemAttribute>& vec_attributes, BYTE bAttrIndex);
	long GetAttributeMaxValue(int iAttributeSet, BYTE attrType, BYTE bAttrIndex);

protected:
	TSwitchbotTable m_SwitchbotTable;
	std::map<BYTE, std::map<BYTE, long>> m_map_AttributesBySet;
	std::map<BYTE, std::map<BYTE, long>> m_map_AttributesRareBySet;
};
#endif