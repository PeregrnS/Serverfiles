...
PyObject* wikiGetMobInfo(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (!mobData->dropList.empty())
	{
		...
		int idx = 0;
		for (const auto& it: mobData->dropList) {
...
PyObject* wikiLoadInfo(PyObject* poSelf, PyObject* poArgs)
{
	...
	unsigned long long retID = static_cast<unsigned long long>(iretID);
...
PyObject* wikiGetRefineInfo(PyObject* poSelf, PyObject* poArgs)
{
	...
	{
		if (!table || (table && !table->GetWikiTable()))
			return Py_BuildNone();

		const size_t refineMaxLevel = table->GetWikiTable()->pRefineData.size();
...
PyObject* wikiSetAuraModel(PyObject* poSelf, PyObject* poArgs)
{
	int moduleID;
	if (!PyTuple_GetInteger(poArgs, 0, &moduleID))
		return Py_BadArgument();
	...
	int auraVnum;
	if (!PyTuple_GetInteger(poArgs, 1, &auraVnum))
		return Py_BadArgument();

	CPythonWikiRenderTarget::instance().SetAuraModel(moduleID, auraVnum);
	return Py_BuildNone();
}
#endif

#ifdef ENABLE_ACCE_COSTUME_SYSTEM
PyObject* wikiSetSashModel(PyObject* poSelf, PyObject* poArgs)
{
	int moduleID;
	if (!PyTuple_GetInteger(poArgs, 0, &moduleID))
		return Py_BadArgument();
	...
	CPythonWikiRenderTarget::instance().SetSashModel(moduleID, sashVnum);
	return Py_BuildNone();
}
#endif

#ifdef ENABLE_SHINING_ITEM_SYSTEM
PyObject* wikiSetShiningModel(PyObject* poSelf, PyObject* poArgs)
{
	int moduleID;
	if (!PyTuple_GetInteger(poArgs, 0, &moduleID))
	{
	...
	int shiningVnum;
	if (!PyTuple_GetInteger(poArgs, 1, &shiningVnum))
	{
		return Py_BadArgument();
	}

	CPythonWikiRenderTarget::instance().SetShiningModel(moduleID, shiningVnum);
	return Py_BuildNone();
}
#endif
...
PyObject* wikiSetUseSkillWithEffect(PyObject* poSelf, PyObject* poArgs)
{
	int moduleID;
	if (!PyTuple_GetInteger(poArgs, 0, &moduleID))
		return Py_BadArgument();
	...
	CPythonWikiRenderTarget::instance().SetUseSkillWithEffect(moduleID, skillID, effectID);
	return Py_BuildNone();
}
...
void initWiki()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "SetUseSkillWithEffect", wikiSetUseSkillWithEffect, METH_VARARGS },

#ifdef ENABLE_COSTUME_AURA_SYSTEM
		{ "SetAuraModel", wikiSetAuraModel, METH_VARARGS },
#endif
#ifdef ENABLE_ACCE_COSTUME_SYSTEM
		{ "SetSashModel", wikiSetSashModel, METH_VARARGS },
#endif
#ifdef ENABLE_SHINING_ITEM_SYSTEM
		{ "SetShiningModel", wikiSetShiningModel, METH_VARARGS },
#endif
