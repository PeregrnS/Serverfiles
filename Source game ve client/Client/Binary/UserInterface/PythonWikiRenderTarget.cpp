...
void CPythonWikiRenderTarget::SetShiningModel(int module_id, int shining_vnum)
{
	auto _Wrt = _GetRenderTargetHandle(module_id);
	if (!_Wrt)
	{
	...
	_Wrt->SetShiningModel(shining_vnum);
}
#endif
...
void CPythonWikiRenderTarget::SetUseSkillWithEffect(int module_id, int skillID, int effectID)
{
	auto _Wrt = _GetRenderTargetHandle(module_id);
	if (!_Wrt)
		return;

	_Wrt->UseSkillWithEffect(skillID, effectID);
}
