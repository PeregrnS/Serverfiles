...
CClientManager::CClientManager() :
	...
	m_iDailyQuestTableSize(0),
	m_pDailyQuestTable(NULL),
	m_iDailyQuestRewardTableSize(0),
	m_pDailyQuestRewardTable(NULL),
#endif
...
void CClientManager::MainLoop()
{
	...
	auto shopCache = m_map_privateShopCache.begin();
	while (shopCache != m_map_privateShopCache.end())
	{
		CPrivateShopCache* c = (shopCache++)->second.get();
		c->Flush();
	}
	m_map_privateShopCache.clear();

	auto shopItemCache = m_map_privateShopItemCache.begin();
	while (shopItemCache != m_map_privateShopItemCache.end())
	{
		CPrivateShopItemCache* c = (shopItemCache++)->second.get();
		c->Flush();
	}
	m_map_privateShopItemCache.clear();

	auto shopSaleCache = m_map_privateShopSaleCache.begin();
	while (shopSaleCache != m_map_privateShopSaleCache.end())
	{
		CPrivateShopSaleCache* c = (shopSaleCache++)->second.get();
		c->Flush();
	}
	m_map_privateShopSaleCache.clear();
#endif
...
void CClientManager::QUERY_BOOT(CPeer* peer, TPacketGDBoot * p)
{
	...
	DWORD adwAdminConfig[GM_DISABLE];
...
#ifdef __DAILY_QUESTS__
		sizeof(uint16_t) + sizeof(uint16_t) + sizeof(TDailyQuestTable) * m_iDailyQuestTableSize +
		sizeof(uint16_t) + sizeof(uint16_t) + sizeof(TDailyQuestRewardTable) * m_iDailyQuestRewardTableSize +
#endif
...
#endif
#ifdef __PREMIUM_PRIVATE_SHOP__
		sizeof(WORD) + sizeof(WORD) + sizeof(TMarketItemPrice) * m_map_marketItemPrice.size() +
#endif
...
#endif
#ifdef __GUILD_SAFEBOX__
		sizeof(WORD) + sizeof(WORD) + sizeof(TGuildSafeboxInitial) * CGuildSafeboxManager::Instance().GetSafeboxCount() +
#endif
		sizeof(time_t) +
...
#ifdef __DAILY_QUESTS__
	peer->EncodeWORD(sizeof(TDailyQuestTable));
	peer->EncodeWORD(m_iDailyQuestTableSize);
	peer->Encode(m_pDailyQuestTable, sizeof(TDailyQuestTable) * m_iDailyQuestTableSize);

	peer->EncodeWORD(sizeof(TDailyQuestRewardTable));
	peer->EncodeWORD(m_iDailyQuestRewardTableSize);
	peer->Encode(m_pDailyQuestRewardTable, sizeof(TDailyQuestRewardTable) * m_iDailyQuestRewardTableSize);
#endif
...
#ifdef __PREMIUM_PRIVATE_SHOP__
	peer->EncodeWORD(sizeof(TMarketItemPrice));
	peer->EncodeWORD(m_map_marketItemPrice.size());
	if (m_map_marketItemPrice.size())
	{
		for (const auto& kv : m_map_marketItemPrice)
		{
			static TMarketItemPrice TItemPrice{};
			TItemPrice.dwVnum = kv.first;
			TItemPrice.TPrice = kv.second;

			peer->Encode(&TItemPrice, sizeof(TMarketItemPrice));
		}
	}
#endif
...
#ifdef __GUILD_SAFEBOX__
	peer->EncodeWORD(sizeof(TGuildSafeboxInitial));
	peer->EncodeWORD(CGuildSafeboxManager::Instance().GetSafeboxCount());
	CGuildSafeboxManager::Instance().InitSafeboxCore(peer);
#endif
...
void CClientManager::QUERY_SETUP(CPeer * peer, DWORD dwHandle, const char * c_pData)
{
	...
	for (DWORD c = 0; c < p->dwLoginCount; ++c, ++pck)
	{
		...
		strlcpy(r.pin, pck->pin, sizeof(r.pin));
#endif
		...
		{
			sys_log(0, "PRIVATE_SHOP: Recreating private shop %u from channel %u port %u", pck->dwPID, p->bChannel, p->wListenPort);
			...
			{
				sys_err("Cannot spawn private shop %u", pck->dwPID);
			}
...
void CClientManager::QUERY_ITEM_SAVE(CPeer * pkPeer, const char * c_pData)
{
	...
	if (p->window == GUILDSTORAGE)
	{
		...
		"REPLACE INTO item%s "
		"(id, owner_id, window, pos, count, vnum, dwVnum, socket0, socket1, socket2, "
	#else
		"REPLACE INTO item%s "
		...
		"attrtype6, attrvalue6) "
	#ifdef __CHANGE_LOOK_SYSTEM__
		"VALUES(%u, %u, 'GUILDSTORAGE', %d, %d, %u, %u, %u, %u, %u, "
	#else
		"VALUES(%u, %u, 'GUILDSTORAGE', %d, %d, %u, %u, %u, %u, "
	#endif
		"%d, %d, "  // attr0
		"%d, %d, "  // attr1
		"%d, %d, "  // attr2
		...
		TPlayerItem* item = new TPlayerItem;
		memcpy(item, p, sizeof(TPlayerItem));
		CDBManager::instance().ReturnQuery(szQuery, QID_ITEM_SAVE, pkPeer == nullptr ? 0 : pkPeer->GetHandle(), item);
		return;
	}
...
void CClientManager::UpdatePlayerCache()
{
	...
	while (it != m_map_playerCache.end())
	{
		...
		if (c->CheckTimeout())
		{
			...
			UpdatePrivateShopSaleCacheSet(c->Get()->id);
#endif
		}
...
#ifdef __PREMIUM_PRIVATE_SHOP__
	FlushPrivateShopCache(dwPID);
	FlushPrivateShopItemCacheSet(dwPID);
	FlushPrivateShopSaleCacheSet(dwPID);
#endif
...
void CClientManager::QUERY_AUTH_LOGIN(CPeer * pkPeer, DWORD dwHandle, TPacketGDAuthLogin * p)
{
	...
	else
	{
		...
		ForwardPacket(HEADER_DG_IP, p->szIP, sizeof(p->szIP));
#endif
...
#endif
#ifdef ENABLE_PIN_LOGIN
		strlcpy(r.pin, p->pin, sizeof(r.pin));
#endif
		r.dwCash = p->dwCash;
		r.dwCoins = p->dwCoins;
...
void CClientManager::ProcessPackets(CPeer * peer)
{
	...
	while (peer->PeekPacket(i, header, dwHandle, dwLength, &data))
	{
		...
		switch (header)
		{
			...
			case HEADER_GD_GUILDSTORAGE:
				RecvGuildStoragePacket(peer, data);
				break;
		...
		case HEADER_GD_ADD_RANKING:
			...
			case HEADER_GD_PROMOTION_REDEEM:
				...
				break;
#endif
			case HEADER_GD_SET_CASH:
				QUERY_ACCOUNT_SET_CASH(peer, dwHandle, (TPacketGDSetCash *)data);
				break;

			case HEADER_GD_SET_COINS:
				QUERY_ACCOUNT_SET_COINS(peer, dwHandle, (TPacketGDSetCoins *)data);
				break;
			...
			case HEADER_GD_GUILD_SAFEBOX_LOAD:
				CGuildSafeboxManager::Instance().ProcessPacket(peer, dwHandle, header, data);
				break;
#endif
...
int CClientManager::AnalyzeQueryResult(SQLMsg * msg)
{
	...
	switch (qi->iType)
	{
		...
		case QID_GUILDSTORAGE_LOAD:
			QueryResultGuildStorage(peer, msg, qi);
			break;

		case QID_GUILDSTORAGE_LOAD_GOLD:
			QueryResultGuildStorageGoldLoad(peer, msg, qi);
			break;
		
		case QID_GUILDSTORAGE_LOAD_LOG:
			QueryResultGuildStorageLogLoad(peer, msg, qi);
			break;
		
		case QID_GUILDSTORAGE_SAVE_LOG:
			break;
		
		case QID_GUILDSTORAGE_GOLDUPDATE:
			QueryResultGuildStorageGoldUpdate(peer, msg, qi);
			break;

		case QID_ITEM_SAVE:
			if (qi->pvData != nullptr)
				QueryResultGuildStorageAddItem(peer, msg, qi);
			break;
		...
		// case QID_ITEM_SAVE: @ commented since is used for guild storage
		case QID_ITEM_DESTROY:
		case QID_QUEST_SAVE:
		case QID_PLAYER_SAVE:
		case QID_ITEM_AWARD_TAKEN:
#ifdef __PREMIUM_PRIVATE_SHOP__
		case QID_PRIVATE_SHOP_SAVE:
		case QID_PRIVATE_SHOP_DELETE:
		case QID_PRIVATE_SHOP_ITEM_SAVE:
		case QID_PRIVATE_SHOP_ITEM_DELETE:
		case QID_PRIVATE_SHOP_SALE_SAVE:
#endif
#ifdef ENABLE_SKILL_COLOR_SYSTEM
...
int CClientManager::Process()
{
	...
	while (pulses--)
	{
		...
		if (!(thecore_heart->pulse % thecore_heart->passes_per_sec))
		{
			...
			UpdatePrivateShopSaleCache();

			if (!(thecore_heart->pulse % (thecore_heart->passes_per_sec * MARKET_ITEM_PRICE_UPDATE_SEC_INTERVAL)))
...
#ifdef __GUILD_SAFEBOX__
			CGuildSafeboxManager::Instance().Update();
#endif
#ifdef ENABLE_EVENT_CALENDAR
			UpdateEventManager();
#endif
		...
		if (!(thecore_heart->pulse % (thecore_heart->passes_per_sec * (5*60))))
		{
			CDBManager::instance().AsyncQuery("DELETE FROM guild_log WHERE DATEDIFF(NOW(), time) > 3;", SQL_PLAYER);
		}
...
bool CClientManager::__GetAdminConfig(DWORD pAdminConfig[GM_DISABLE])
{
	memset(pAdminConfig, 1, sizeof(DWORD) * GM_DISABLE);

	char szQuery[512];
	snprintf(szQuery, sizeof(szQuery), "SELECT authority, general_allow+0 FROM gmconfig");
...
void CClientManager::ReloadAdmin(CPeer*, TPacketReloadAdmin* p)
{
	...
	DWORD dwPacketSize = sizeof(WORD) + sizeof (WORD) + sizeof(tAdminInfo) * vAdmin.size() +
		  sizeof(WORD) + sizeof(WORD) + 16 * vHost.size();
	dwPacketSize += sizeof(DWORD) * GM_DISABLE;
...
void CClientManager::EraseItemCache(DWORD id)
{
	CItemCache* c = GetItemCache(id);
	...
	if (c)
	{
		...
		{
			if (g_test_server)
				sys_log(0, "ITEM_CACHE: erase cache owner %u id %u", c->Get()->owner, id);

			it->second->erase(c);
		}

		m_map_itemCache.erase(id);
...
{
	itertype(m_map_itemTableByVnum) it = m_map_itemTableByVnum.find(dwVnum);
	if (it == m_map_itemTableByVnum.end())
		return NULL;
...
void CClientManager::QUERY_ADD_RANKING(CPeer * peer, TPacketGDAddRanking * p, DWORD dwLen)
{
	...
	for (int i = 0; i < iSize; ++i, ++p)
	{
		...
		snprintf(queryStr, sizeof(queryStr),
				"REPLACE INTO ranking%s (dwPID, rank, lValue) "
				"VALUES(%u, %u, %lld)",
...
bool IsDontHaveEndTimeEvent(uint8_t eventIndex)
{
	switch (eventIndex)
	{
...
{
	const uint8_t subIndex = *(uint8_t*)data;
	data += sizeof(uint8_t);
	...
	{
		const uint16_t index = *(uint16_t*)data;
		...
		if (it->second.size())
		{
			for (uint32_t j = 0; j < it->second.size(); ++j)
			{
				...
				if (sendStatusPacket)
				{
					...
					buf.write(&pData.eventID, sizeof(uint16_t));
...
bool CClientManager::InitializeEventManager(bool updateFromGameMaster)
{
	...
	if (pMsg->Get()->uiNumRows != 0)
	{
		...
		for (int n = 0; (row = mysql_fetch_row(pMsg->Get()->pSQLResult)) != NULL; ++n)
		{
			...
			if (vEventStartKey.tm_mon != vKey.tm_mon && vEventEndKey.tm_mon != vKey.tm_mon)
				continue;
			SetTimeToString(&p);
...
void CClientManager::SendEventData(CPeer* pkPeer, bool updateFromGameMaster)
{
	...
	const uint8_t dayCount = m_EventManager.size();
	TEMP_BUFFER buf;
	buf.write(&subIndex, sizeof(uint8_t));
	...
	for (auto it = m_EventManager.begin(); it != m_EventManager.end(); ++it)
	{
		...
		uint8_t dayEventCount = dayData.size();
