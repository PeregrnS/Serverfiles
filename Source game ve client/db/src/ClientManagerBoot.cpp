...
bool CClientManager::InitializeTables()
{
	...
	if (!InitializeDailyQuestTable())
	{
		sys_err("InitializeDailyQuestTable FAILED");
		return false;
	}

	if (!InitializeDailyQuestRewardTable())
	{
		sys_err("InitializeDailyQuestRewardTable FAILED");
		return false;
	}
#endif
	...
	if (!InitializePrivateShopMarketItemPrice())
	{
		sys_err("InitializePrivateShopMarketItemPrice FAILED");
		return false;
	}
#endif
...
bool CClientManager::InitializeShopEXTable()
{
	...
	char s_szQuery[4096];
	int len = sprintf(s_szQuery, "SELECT shopex.vnum, shopex.name, shopex.npc_vnum, shopex_item.item_vnum, shopex_item.count, shopex_item.price, shopex_item.price_type+0, ");

	for (BYTE i = 0; i < SHOP_MAX_ITEM_NUM; i++)
		len += sprintf(s_szQuery + len, "shopex_item.price_vnum%d, shopex_item.price_count%d, ", i, i);

	for (BYTE i = 0; i < ITEM_SOCKET_MAX_NUM; i++)
		len += sprintf(s_szQuery + len, "shopex_item.socket%d, ", i);

	for (BYTE i = 0; i < ITEM_ATTRIBUTE_MAX_NUM; i++)
	{
		len += sprintf(s_szQuery + len, "shopex_item.attrtype%d, shopex_item.attrvalue%d ", i, i);
...
bool CClientManager::InitializeItemTable()
{
	...
	{
		if (m_vec_itemTable[i].dwVnumRange > 0)
		{
			m_vec_itemVnumRange.push_back(&m_vec_itemTable[i]);
		}
	}
#endif
...
bool CClientManager::InitializeGemShopTable()
{
	...
	static const char *s_szQuery = "SELECT id, vnum, count, price FROM gem_shop ORDER BY id";
...
#ifdef __DAILY_QUESTS__
bool CClientManager::InitializeDailyQuestTable()
{
	char query[2048];

	snprintf(query, sizeof(query),
		"SELECT id, type+0, vnum, value1, amount, min_level, max_level FROM daily_quests%s",
		GetTablePostfix());
	...
	m_iDailyQuestTableSize = pRes->uiNumRows;

	m_pDailyQuestTable = new TDailyQuestTable[m_iDailyQuestTableSize];
	memset(m_pDailyQuestTable, 0, sizeof(TDailyQuestTable) * m_iDailyQuestTableSize);

	TDailyQuestTable* prt = m_pDailyQuestTable;
	MYSQL_ROW data;
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		int32_t col = 0;
		str_to_number(prt->id, data[col++]);
		str_to_number(prt->type, data[col++]);
		str_to_number(prt->vnum, data[col++]);
		str_to_number(prt->value, data[col++]);
		str_to_number(prt->amount, data[col++]);
		str_to_number(prt->min_level, data[col++]);
		str_to_number(prt->max_level, data[col++]);
...
bool CClientManager::InitializeDailyQuestRewardTable()
{
	char query[2048];

	snprintf(query, sizeof(query),
		"SELECT id, grade, vnum0, count0, vnum1, count1, vnum2, count2, vnum3, count3, vnum4, count4, vnum5, count5, min_level, max_level FROM daily_quest_rewards%s",
		GetTablePostfix());
	...
	m_iDailyQuestRewardTableSize = pRes->uiNumRows;

	m_pDailyQuestRewardTable = new TDailyQuestRewardTable[m_iDailyQuestRewardTableSize];
	memset(m_pDailyQuestRewardTable, 0, sizeof(TDailyQuestRewardTable) * m_iDailyQuestRewardTableSize);

	TDailyQuestRewardTable* prt = m_pDailyQuestRewardTable;
	MYSQL_ROW data;
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		int32_t col = 0;
		str_to_number(prt->id, data[col++]);
		str_to_number(prt->grade, data[col++]);

		for (int i = 0; i < 6; ++i)
		{
			str_to_number(prt->rewardVnum[i], data[col++]);
			str_to_number(prt->rewardCount[i], data[col++]);
		}
