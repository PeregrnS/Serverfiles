...
size_t CreatePlayerSaveQuery(char * pszQuery, size_t querySize, TPlayerTable * pkTab)
{
	...
	CDBManager::instance().EscapeString(text, pkTab->bSkillMotion, sizeof(pkTab->bSkillMotion));
	queryLen += snprintf(pszQuery + queryLen, querySize - queryLen, "skill_motion = '%s', ", text);
#endif
...
void CClientManager::QUERY_PLAYER_LOAD(CPeer * peer, DWORD dwHandle, TPlayerLoadPacket * packet)
{
	...
	if ((c = GetPlayerCache(packet->player_id)))
	{
		...
		if (pSet)
		{
			...
			snprintf(szQuery, sizeof(szQuery),
				"SELECT player_id, mission_id, battle_pass_id, extra_info, completed FROM battle_pass%s WHERE player_id = %d",
				GetTablePostfix(), pTab->id);
			CDBManager::instance().ReturnQuery(szQuery, QID_BATTLE_PASS, peer->GetHandle(), new ClientHandleInfo(dwHandle));
		}
		/////////////////////////////////////////////
...
#ifdef __SKILL_COSTUME__
	if (row[col])
		memcpy(pkTab->bSkillMotion, row[col], sizeof(pkTab->bSkillMotion));
	else
		memset(pkTab->bSkillMotion, 0, sizeof(pkTab->bSkillMotion));
...
#ifdef ENABLE_AFFECT_RENEWAL
			 ,
			 p->elem.dwExpireTime, p->elem.dwExpireTime
#endif
			);
...
void CClientManager::QUERY_SAVE_BATTLE_PASS(CPeer * peer, DWORD dwHandle, TPlayerBattlePassMission* battlePass)
{
	...
	char szQuery[QUERY_MAX_LEN];
	snprintf(szQuery, sizeof(szQuery),
		"REPLACE INTO battle_pass%s (player_id, mission_id, battle_pass_id, extra_info, completed) VALUES (%u, %d, %d, %d, %d)",
		GetTablePostfix(),
...
void CClientManager::QUERY_ACCOUNT_SET_CASH(CPeer *peer, DWORD dwHandle, TPacketGDSetCash *data)
{
	CLoginData *pkLD = GetLoginDataByAID(data->dwAccountID);

	if (pkLD)
	{
		pkLD->GetAccountRef().dwCash = data->dwCash;
	}
}

void CClientManager::QUERY_ACCOUNT_SET_COINS(CPeer *peer, DWORD dwHandle, TPacketGDSetCoins *data)
{
	CLoginData *pkLD = GetLoginDataByAID(data->dwAccountID);

	if (pkLD)
	{
		pkLD->GetAccountRef().dwCoins = data->dwCoins;
	}
}
