...
bool CItemIDRangeManager::BuildRange(DWORD dwMin, DWORD dwMax, TItemIDRangeTable& range)
{
	...
	DWORD dwPrivateShopItemMaxID = 0;
	snprintf(szQuery, sizeof(szQuery), "SELECT MAX(id) FROM private_shop_item%s WHERE id >= %u and id <= %u", GetTablePostfix(), dwMin, dwMax);
	...
	if (pMsg != NULL)
	{
		...
		{
			row = mysql_fetch_row(pMsg->Get()->pSQLResult);
			str_to_number(dwPrivateShopItemMaxID, row[0]);
		}
	}

	if (dwPrivateShopItemMaxID > dwItemMaxID)
		dwItemMaxID = dwPrivateShopItemMaxID;
#endif
...
#ifdef __PREMIUM_PRIVATE_SHOP__
		{
			snprintf(szQuery, sizeof(szQuery), "SELECT COUNT(*) FROM private_shop_item%s WHERE id >= %u AND id <= %u",
			...
			if (pMsg != NULL)
			{
				if (pMsg->Get()->uiNumRows > 0)
				{
					...
					if (count > 0)
					{
						sys_err("ItemIDRange: Build: %u ~ %u\thave a private shop item", range.dwUsableItemIDMin, range.dwMax);
						return false;
					}
				}
			}
