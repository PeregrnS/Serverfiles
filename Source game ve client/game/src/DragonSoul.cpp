...
bool DSManager::IsValidCellForThisItem(const LPITEM pItem, const TItemPos& Cell) const
{
	...
	if (Cell.window_type != DRAGON_SOUL_INVENTORY || (Cell.cell < wBaseCell || Cell.cell >= (wBaseCell + (DRAGON_SOUL_BOX_SIZE * DRAGON_SOUL_INVENTORY_PAGE_COUNT))))
#else
...
#ifdef ENABLE_EXTENDED_DS_INVENTORY
	return col_type * DRAGON_SOUL_GRADE_MAX * DRAGON_SOUL_BOX_SIZE * DRAGON_SOUL_INVENTORY_PAGE_COUNT + row_type * DRAGON_SOUL_BOX_SIZE * DRAGON_SOUL_INVENTORY_PAGE_COUNT;
#else
...
bool DSManager::DoRefineGrade(LPCHARACTER ch, TItemPos (&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE]
	...
	, bool returnTrueInCaseOfFailedRefine
#endif
)
{
	if (NULL == ch)
	...
	if (ch->GetGold() < (unsigned long long)fee)
#else
	if (ch->GetGold() < fee)
#endif
	{
...
#ifdef __ENABLE_DS_REFINE_ALL__
		return returnTrueInCaseOfFailedRefine ? true : false;
#else
		return false;
#endif
	}
...
#ifdef __ENABLE_DS_REFINE_ALL__
bool DSManager::DoAllRefineGrade(LPCHARACTER ch, BYTE dsKind, BYTE dsGrade)
{
	if (ch == nullptr)
	{
		return false;
	...
	if (dsKind >= DS_SLOT_MAX)
	{
		return false;
	}

	if (dsGrade >= DRAGON_SOUL_GRADE_MAX - 1)
	{
		return false;
	}
...
#ifdef ENABLE_NEWSTUFF
	if (!PulseManager::Instance().IncreaseClock(ch->GetPlayerID(), ePulse::RequestRefineAllDs, std::chrono::milliseconds(1000)))
	{
...
#ifdef ENABLE_EXTENDED_DS_INVENTORY
	auto startPos = (DRAGON_SOUL_BOX_SIZE * DRAGON_SOUL_INVENTORY_PAGE_COUNT * dsGrade) + (dsKind * DRAGON_SOUL_INVENTORY_PAGE_COUNT * DRAGON_SOUL_BOX_SIZE * DRAGON_SOUL_GRADE_MAX);
	auto endPos = startPos + (DRAGON_SOUL_BOX_SIZE * DRAGON_SOUL_INVENTORY_PAGE_COUNT);
#else
	auto startPos = (DRAGON_SOUL_BOX_SIZE * dsGrade) + (dsKind * DRAGON_SOUL_BOX_SIZE * DRAGON_SOUL_GRADE_MAX);
	auto endPos = startPos + DRAGON_SOUL_BOX_SIZE;
#endif

	int32_t needCount = -1, fee;
	uint8_t dsType, gradeIdx, stepIdx, strengthIdx;
	std::vector<float> probabilities;

	TItemPos poses[DRAGON_SOUL_REFINE_GRID_SIZE];
	size_t posesI = 0;
	// sys_log(0, "startPos %d endPos %d", startPos, endPos);
	for (auto i = startPos; i < endPos; i++)
	{
		auto item = ch->GetItem(TItemPos(DRAGON_SOUL_INVENTORY, i));
		if (!item)
		{
			continue;
		}
		...
		if (needCount == -1)
		{
			GetDragonSoulInfo(item->GetVnum(), dsType, gradeIdx, stepIdx, strengthIdx);

			if (!m_pTable->GetRefineGradeValues(dsType, gradeIdx, needCount, fee, probabilities))
			{
				return false;
			}
		}

		auto& pos = poses[posesI++];
		pos.window_type = item->GetWindow();
		pos.cell = item->GetCell();

		if (posesI >= needCount)
		{
			DoRefineGrade(ch, poses, true);
			posesI = 0;
		}
	}
...
bool DSManager::ChangeAttributeDragonSoul(LPITEM pDS)
{
	if (!pDS->IsDragonSoul())
	{
		sys_err ("This item(ID : %d) is not DragonSoul.", pDS->GetID());
	...
	{
		pDS->SetForceAttribute(DRAGON_SOUL_ADDITIONAL_ATTR_START_IDX + i, APPLY_NONE, 0);
	}
