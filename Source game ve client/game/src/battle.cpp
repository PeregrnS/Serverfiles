...
switch(ch->GetMapIndex())
{
	case 41:
		...
		break;  // Optional, falls du mehr logik hinzufugen willst
	
	case 35:
		{
			// Deine Logik fur channel 35
			if (ch->IsPC() && victim->IsPC())
				return false;
		}
		break;
...
int CalcMagicDamage(LPCHARACTER pkAttacker, LPCHARACTER pkVictim)
{
	...
	if (pkAttacker->GetQuestFlag("duel_type") >= 1 || pkVictim->GetQuestFlag("duel_type") >= 1)
		iDam = 0;
...
int CalcAttBonus(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int iAtk)
{
	...
	if (pkVictim->IsNPC())
	{
		...
		{
			iAtk += (iAtk * pkAttacker->GetPoint(POINT_ATTBONUS_ELEMENTS)) / 200;
		}
		if (pkVictim->IsStone())
		{
...
#ifdef ENABLE_SURA_BONUS_PVM_GIVE
		if (pkAttacker->IsPC() && pkAttacker->GetJob() == JOB_SURA) 
		{
			int iGiveBonusSura = ENABLE_SURA_BONUS_PVM_GIVE_VALUE;
			if (iGiveBonusSura)
				iAtk += (iAtk * iGiveBonusSura) / 100;
		}		
#endif
	...
	if (pkAttacker->IsPC())
	{
		// --- GLOBALER PVP-DAMPFER (BASIS-ABWEHR) ---
		// Wenn das Opfer ein Spieler ist, senken wir den Schaden hier fur alle gleich.
		if (pkVictim->IsPC())
		{
			// 80 bedeutet: Jeder hat 20% interne Abwehr gegen jeden (80% Restschaden).
			iAtk = (iAtk * 80) / 100;
		}
#ifdef ENABLE_EXTENDED_ATTRS
	...
	if (pkAttacker->IsNPC() && pkVictim->IsPC())
	{
		...
		if (pkVictim->IsRaceFlag(RACE_FLAG_ATT_ELEC) ||
				...
				pkVictim->IsRaceFlag(RACE_FLAG_ATT_ICE))
		{
			iAtk -= (iAtk * 30 * pkVictim->GetPoint(POINT_ENCHANT_ELEMENTS)) / 10000;
		}
#endif
	}
...
void Item_GetDamage(LPITEM pkItem, int* pdamMin, int* pdamMax)
{
	...
	*pdamMax = pkItem->GetValue(4);
	...
	if (*pdamMin > *pdamMax)
		sys_log(0, "pdamMin for weapon %d is bigger than pdamMax", pkItem->GetVnum());
}
