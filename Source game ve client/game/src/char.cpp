...
bool CAN_ENTER_ZONE(const LPCHARACTER& ch, int map_index)
{
	if (ch->IsGM()) return true; // GMs durfen immer rein
	if (ch->GetLevel() >= 310) return true; // Spieler ab Level 310 durfen uberall rein

	int iLvl = ch->GetLevel();
	...
	switch (map_index)
	{
		...
		case 424:
			if (iLvl < 50) {
				ch->ChatPacket(CHAT_TYPE_INFO, "Du musst mindestens Level 50 sein, um HLM1 zu betreten.");
				return false;
			}
			if (iLvl >= 130) {
				ch->ChatPacket(CHAT_TYPE_INFO, "Dein Level ist zu hoch fur HLM1 (Max. 129).");
				return false;
			}
			break;
		...
		case 339:
			if (iLvl < 130) {
				ch->ChatPacket(CHAT_TYPE_INFO, "Du musst mindestens Level 130 sein, um HLM2 zu betreten.");
				return false;
			}
			if (iLvl >= 145) {
				ch->ChatPacket(CHAT_TYPE_INFO, "Dein Level ist zu hoch fur HLM2 (Max. 144).");
				return false;
			}
			break;
		...
		case 311:
			if (iLvl < 145) {
				ch->ChatPacket(CHAT_TYPE_INFO, "Du musst mindestens Level 145 sein, um HLM3 zu betreten.");
				return false;
			}
			if (iLvl >= 185) {
				ch->ChatPacket(CHAT_TYPE_INFO, "Dein Level ist zu hoch fur HLM3 (Max. 184).");
				return false;
			}
			break;
		...
		case 116:
			if (iLvl < 185) {
				ch->ChatPacket(CHAT_TYPE_INFO, "Du musst mindestens Level 185 sein, um HLM4 zu betreten.");
				return false;
			}
			if (iLvl >= 295) {
				ch->ChatPacket(CHAT_TYPE_INFO, "Dein Level ist zu hoch fur HLM4 (Max. 294).");
				return false;
			}
			break;
		...
		case 130:
			if (iLvl < 135) {
				ch->ChatPacket(CHAT_TYPE_INFO, "Du musst mindestens Level 135 sein, um Tal der Toten zu betreten.");
				return false;
			}
			break;
		...
		case 133:
			if (iLvl < 295) {
				ch->ChatPacket(CHAT_TYPE_INFO, "Du musst mindestens Level 295 sein, um Grotte 2 zu betreten.");
				return false;
			}
			break;
		...
		case 428:
			if (iLvl < 50) {
				ch->ChatPacket(CHAT_TYPE_INFO, "Du musst mindestens Level 50 sein, um die Bossmap zu betreten.");
				return false;
			}
			break;
		...
		case 423:
			if (iLvl < 50) {
				ch->ChatPacket(CHAT_TYPE_INFO, "Du musst mindestens Level 100 sein, um den Drachenrun zu betreten.");
				return false;
			}
			break;
		...
		case 131:
			if (iLvl < 80) {
				ch->ChatPacket(CHAT_TYPE_INFO, "Du musst mindestens Level 80 sein, um die Verlassene Stadt zu betreten.");
				return false;
			}
			break;
		...
		case 430:
			if (iLvl < 50) {
				ch->ChatPacket(CHAT_TYPE_INFO, "Du musst mindestens Level 50 sein, um die Metin Map 1 zu betreten.");
				return false;
			}
			break;
		...
		case 322:
			if (iLvl < 30) {
				ch->ChatPacket(CHAT_TYPE_INFO, "Du musst mindestens Level 50 sein, um die Metin Map 2 zu betreten.");
				return false;
			}
			break;
		// --- Metin Map 3 ---
		case 431:
			if (iLvl < 295) {
				ch->ChatPacket(CHAT_TYPE_INFO, "Du musst mindestens Level 295 sein, um die Metin Map 3 zu betreten.");
				return false;
			}
			break;
	}
...
void CHARACTER::OpenMyShop(const char * c_pszSign, TShopItemTable * pTable
...
)
{
	...
	if (!check_allow(GetGMLevel(), GM_ALLOW_CREATE_PRIVATE_SHOP))
	{
	...
	if (GetAllowedGold() <= nTotalMoney)
	{
		...
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You cannot trade because you are carrying more than 2kkkk billion Yang."));
		return;
	}
#else
...
void CHARACTER::EncodeInsertPacket(LPENTITY entity)
{
	...
	)
	{
		...
		addPacket.byTitleIndex = GetProtectTime("title_index");
...
void CHARACTER::UpdatePacket()
{
	...
	pack.byTitleIndex = GetProtectTime("title_index");
	...
	if (GetQuestFlag("duel_type") == 1) //DUEL ROMANIAN
	{
		PointChange(POINT_MAX_HP, 45000-GetMaxHP());
	}
	if (GetQuestFlag("duel_type") == 2) //DUEL GERMAN
	{
		if (GetPoint(POINT_CRITICAL_PCT) < 500)
			PointChange(POINT_CRITICAL_PCT, 500);
		if (GetJob() == JOB_WARRIOR)
			PointChange(POINT_MAX_HP, 170000-GetMaxHP());
		else
			PointChange(POINT_MAX_HP, 150000-GetMaxHP());
	}
...
#ifdef __SKILL_COSTUME__
	memcpy(tab.bSkillMotion, m_bSkillMotion, sizeof(tab.bSkillMotion));
#endif
...
void CHARACTER::Disconnect(const char * c_pszReason)
{
...
#ifdef ENABLE_GM_LOGIN_NOTICE
	if (GetGMLevel() > GM_PLAYER && !IsSetWarp())
	{
		char notice[256];
		const char* color = "|cffFFFF00"; // Standard Gelb

		if (GetGMLevel() == GM_IMPLEMENTOR) // SA
			color = "|cffFF0000"; // Knallrot
		else if (GetGMLevel() == GM_GOD) // GA
			color = "|cff00FF00"; // Grun
		else if (GetGMLevel() == GM_HIGH_WIZARD) // SGM
			color = "|cff00FFFF"; // Cyan/Hellblau

		snprintf(notice, sizeof(notice), "%s[Team-Info] %s ist nun offline!|r", color, GetName());
		BroadcastNotice(notice);

		// DAS HIER EINFUGEN:
		// Setzt das Flag zuruck, damit beim nachsten Login die Nachricht wieder kommt
		SetQuestFlag("gm_notice.sent", 0); 
	}
#endif
	...
	if (GetProtectTime("spintboss_itemidx") && GetProtectTime("spintboss_itemcount"))
	{
		AutoGiveItem(GetProtectTime("spintboss_itemidx"), GetProtectTime("spintboss_itemcount"));
		SetProtectTime("spintboss_itemidx", 0);
		SetProtectTime("spintboss_itemcount", 0);
	}
#endif
...
// #endif
	...
	if ((d = GetDesc()))
	{
		CAntiMultipleFarm::instance().Logout(d->GetLoginMacAdress(), GetPlayerID(), is_warping);
	}
#endif
	...
	{
		GuildStorageManager::Instance().CloseGuildStorage(this);
	}
	...
	CPVPManager::instance().EraseDuel(this);
...
void CHARACTER::PointsPacket()
{
...
#ifndef ENABLE_GOLD_MAX_EXTENDED
	...
	GetDesc()->Packet(packGold);
#endif

	TPacketGCDragonPoints packDragonPoints;
	packDragonPoints.byHeader = HEADER_GC_CHARACTER_DRAGON_POINTS;
	packDragonPoints.dwCash = GetCash();
	packDragonPoints.dwCoins = GetCoins();
	GetDesc()->Packet(&packDragonPoints, sizeof(TPacketGCDragonPoints));
}
...
bool CHARACTER::ChangeSex()
{
...
#ifdef ENABLE_GENDER_TRANSFORMATION_WITHOUT_RELOG
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        if (currentSex == SEX_MALE && IS_SET(costume->GetAntiFlag(), ITEM_ANTIFLAG_MALE) || currentSex == SEX_FEMALE && IS_SET(costume->GetAntiFlag(), ITEM_ANTIFLAG_FEMALE))
        {
            UnequipItem(costume);
        }
	}
...
void CHARACTER::ComputePoints()
{
	...
	if (IsPC())
	{
		...
		switch (GetRaceNum())
		{
			...
			case MAIN_RACE_SHAMAN_W:
			{
				SetPoint(POINT_ATTBONUS_MONSTER, 20);
				SetPoint(POINT_ATTBONUS_STONE, 10);
				SetPoint(POINT_ATTBONUS_BOSS, 10);
			}
		}
		
		if (CPVPManager::instance().IsPlayerInPVP(this))
		{
			// PointChange(POINT_MOV_SPEED, 200-m_RealMoveSpeed);
			// PointChange(POINT_ATT_SPEED, 170-m_RealAttackSpeed);

			if (GetQuestFlag("duel_type") == 1) //DUEL ROMANIAN
			{
				PointChange(POINT_MAX_HP, 45000-GetMaxHP());
				PointChange(POINT_CASTING_SPEED, 170-m_RealCastingSpeed);
			}
			if (GetQuestFlag("duel_type") == 2) //DUEL GERMAN
			{
				PointChange(POINT_CRITICAL_PCT, 500);
				if (GetJob() == JOB_WARRIOR)
				{
					if (GetSkillGroup() == 2)
					{
						PointChange(POINT_MAX_HP, 180000-GetMaxHP());
						PointChange(POINT_CASTING_SPEED, 155-m_RealCastingSpeed);
					}
					else
					{
						PointChange(POINT_MAX_HP, 170000-GetMaxHP());
						PointChange(POINT_CASTING_SPEED, 120-m_RealCastingSpeed);
					}
				}
				...
				{
					PointChange(POINT_MAX_HP, 150400-GetMaxHP());
					PointChange(POINT_CASTING_SPEED, 110-m_RealCastingSpeed);
				}
				else
				{
					PointChange(POINT_MAX_HP, 150000-GetMaxHP());
					PointChange(POINT_CASTING_SPEED, 150-m_RealCastingSpeed);
				}
			}
		}
	...
	if (IsPC())
	{
		...
		for (int i = 0; i < INVENTORY_MAX_NUM; ++i)
		{
			...
			if (item && item->GetSocket(1) != 0 && item->GetType() == ITEM_USE && item->GetSubType() == USE_TITLE)
			{
	...
	if (GetHP() > GetMaxHP() && GetQuestFlag("duel_type") == 0)
		PointChange(POINT_HP, GetMaxHP() - GetHP());
...
EVENTFUNC(recovery_event)
{
	...
	else
	{
		...
		// @ grm is in duel tactic
		if (ch->GetQuestFlag("duel_type") == 2)
		{
			return 3;
		}
...
bool CHARACTER::Sync(long x, long y)
{
	...
	const long currentX = GetX(), currentY = GetY();
	if(currentX != 0 && currentY != 0 && (currentX != x || currentY != y))
	{
		LPSECTREE_MAP pkSectreeMap = SECTREE_MANAGER::instance().GetMap(GetMapIndex());
		if(pkSectreeMap)
		{
			LPSECTREE tree = pkSectreeMap->Find(currentX, currentY);
			if (tree)
			{
				const DWORD dwAttr = tree->GetAttribute(currentX, currentY);
				if (!IS_SET(dwAttr, ATTR_BLOCK | ATTR_OBJECT | ATTR_WATER))
					SetXYZLast(currentX, currentY, GetZ());
			}
		}
	}
#endif
	...
	if (m_bAntiTPInit && m_lLastIdx == GetMapIndex())
	{
		if (AntiTeleportCheck(x, y))
		{
			m_dwLastSyncTime = now;
			return true;
		}
	}
#endif
...
#ifdef BLOCK_TELEPORT_HACK
	m_lLastX = x; m_lLastY = y; m_lLastIdx = GetMapIndex();
	m_dwLastSyncTime = now;
	m_bAntiTPInit = true;
#endif
...
void CHARACTER::SetPoint(BYTE type, int val)
{
	...
	if (type == POINT_MOV_SPEED && !IsRiding())
		val = 200;
...
#ifdef ENABLE_RANK_PLAYER
	RankPlayer::instance().SendInfoPlayer(this, RANK_BY_YANG, GetGold(), false);
#endif
...
void CHARACTER::CheckMapLevelLimit()
{
	if (IsGM()) return;
	if (GetLevel() >= 310) return; // Wer Level 310 oder hoher ist, wird ignoriert

	int iMapIdx = GetMapIndex();
	int iLvl = GetLevel();
	bool bKick = false;
	...
	switch (iMapIdx)
	{
		case 424: if (iLvl >= 130) { bKick = true; szMapName = "HLM1"; } break;
		case 339: if (iLvl >= 145) { bKick = true; szMapName = "HLM2"; } break;
		case 311: if (iLvl >= 185) { bKick = true; szMapName = "HLM3"; } break;
		case 116: if (iLvl >= 295) { bKick = true; szMapName = "HLM4"; } break;
	}

	if (bKick)
	{
		ChatPacket(CHAT_TYPE_INFO, "Dein Level ist zu hoch fur %s! Du wirst teleportiert.", szMapName);
		WarpSet(469300, 964200); // Deine Ziel-Koordinaten
	}
}
...
#ifndef DISABLE_AUTO_GIVE_POTION_ON_LEVELUP
				if (GetLevel() <= 10)
...
DWORD CHARACTER::GetPart(BYTE bPartPos) const // @fixme502
{
	...
	else if (bPartPos == PART_MAIN && GetWear(WEAR_COSTUME_BODY) && GetQuestFlag("hide.costume_body") == 1)
	{
		if (const LPITEM pArmor = GetWear(WEAR_BODY))
		{
			...
			{
				return pArmor->GetTransmutation();
			}
			else
			{
	...
	else if (bPartPos == PART_WEAPON && GetWear(WEAR_COSTUME_WEAPON) && GetQuestFlag("hide.costume_weapon") == 1)
	{
		if (const LPITEM pWeapon = GetWear(WEAR_WEAPON))
		{
			...
			{
				return pWeapon->GetTransmutation();
			}
			else
			{
...
#endif
#ifdef ENABLE_ACCE_COSTUME_SYSTEM
	else if (bPartPos == PART_ACCE && GetWear(WEAR_COSTUME_ACCE) && GetQuestFlag("hide.costume_acce") == 1)
	{
		return 0;
	}
#endif
#ifdef ENABLE_AURA_COSTUME_SYSTEM
	else if (bPartPos == PART_AURA && GetWear(WEAR_COSTUME_AURA) && GetQuestFlag("hide.costume_aura") == 1)
	{
		return 0;
	}
#endif
...
DWORD CHARACTER::GetOriginalPart(BYTE bPartPos) const // @fixme502
{
	switch (bPartPos)
	{
		...
		case PART_ACCE:
#ifdef ENABLE_HIDE_COSTUME_SYSTEM
			if (GetWear(WEAR_COSTUME_ACCE) && GetQuestFlag("hide.costume_acce") == 1)
			{
				return 0;
			}
#endif
...
void CHARACTER::OnClick(LPCHARACTER pkChrCauser)
{
...
#ifdef ENABLE_ACCE_COSTUME_SYSTEM
						|| pkChrCauser->IsAcceOpened()
#endif
#if defined(__BL_67_ATTR__)
						|| pkChrCauser->Is67AttrOpen()
#endif	
						|| pkChrCauser->IsOpenGuildStorage()
					)
					{
						pkChrCauser->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("다른 거래중(창고,교환,상점)에는 개인상점을 사용할 수 없습니다."));
						return;
	...
	if (CEventManager::Instance().GetEventState(EVENT_TYPE_HIDE_AND_SEEK))
	{
		...
		if (iTargetVID == vid)
		{
			...
			if (LPITEM item = pkChrCauser->AutoGiveItem(HIDE_AND_SEEK_REWARD_VNUM, HIDE_AND_SEEK_REWARD_COUNT))
			{
...
void CHARACTER::SetTarget(LPCHARACTER pkChrTarget)
{
	...
	if (m_pkChrTarget)
	{
		...
		{
			p.bCostumeHide[ECostumeHideParts::HIDE_COSTUME_BODY] = m_pkChrTarget->GetQuestFlag("hide.costume_body");
			p.bCostumeHide[ECostumeHideParts::HIDE_COSTUME_HAIR] = m_pkChrTarget->GetQuestFlag("hide.costume_hair");
#ifdef ENABLE_WEAPON_COSTUME_SYSTEM
			p.bCostumeHide[ECostumeHideParts::HIDE_COSTUME_WEAPON] = m_pkChrTarget->GetQuestFlag("hide.costume_weapon");
#endif
#ifdef ENABLE_ACCE_COSTUME_SYSTEM
			p.bCostumeHide[ECostumeHideParts::HIDE_COSTUME_ACCE] = m_pkChrTarget->GetQuestFlag("hide.costume_acce");
#endif
#ifdef ENABLE_AURA_COSTUME_SYSTEM
			p.bCostumeHide[ECostumeHideParts::HIDE_COSTUME_AURA] = m_pkChrTarget->GetQuestFlag("hide.costume_aura");
#endif
		}
#endif
...
void CHARACTER::BroadcastTargetPacket()
{
	...
	else
	{
		...
		if (IsPC())
		{
			p.bCostumeHide[ECostumeHideParts::HIDE_COSTUME_BODY] = GetQuestFlag("hide.costume_body");
			p.bCostumeHide[ECostumeHideParts::HIDE_COSTUME_HAIR] = GetQuestFlag("hide.costume_hair");
#ifdef ENABLE_WEAPON_COSTUME_SYSTEM
			p.bCostumeHide[ECostumeHideParts::HIDE_COSTUME_WEAPON] = GetQuestFlag("hide.costume_weapon");
#endif
#ifdef ENABLE_ACCE_COSTUME_SYSTEM
			p.bCostumeHide[ECostumeHideParts::HIDE_COSTUME_ACCE] = GetQuestFlag("hide.costume_acce");
#endif
#ifdef ENABLE_AURA_COSTUME_SYSTEM
			p.bCostumeHide[ECostumeHideParts::HIDE_COSTUME_AURA] = GetQuestFlag("hide.costume_aura");
#endif
		}
#endif
	}
...
bool CHARACTER::CheckWarpEquip() const
{
	if (!IsPC())
		return true;
	...
	int iNeedNormalSlots = 0;
	int iNeedDragonSoulSlots = 0;
	...
	for (int i = 0; i < WEAR_MAX_NUM; ++i)
	{
		...
		bool bInvalid = false;
		const TItemTable* pTable = pItem->GetProto();
...
#ifdef ENABLE_WOLFMAN_CHARACTER
			case JOB_WOLFMAN:
				if (pItem->GetAntiFlag() & ITEM_ANTIFLAG_WOLFMAN)
					bInvalid = true;
				break;
#endif
		}
		...
		if (!bInvalid && pTable)
		{
			...
			{
				const long limit = pTable->aLimits[j].lValue;

				switch (pTable->aLimits[j].bType)
				{
					case LIMIT_LEVEL:
						if (GetLevel() < limit)
							bInvalid = true;
	...
	int iFreeDragonSoulSlots = 0;
	for (int i = 0; i < DRAGON_SOUL_INVENTORY_MAX_NUM; ++i)
	{
		if (!GetItem(TItemPos(DRAGON_SOUL_INVENTORY, i)))
			++iFreeDragonSoulSlots;
	}

	if (iNeedNormalSlots > iFreeNormalSlots)
		return false;

	if (iNeedDragonSoulSlots > iFreeDragonSoulSlots)
		return false;
...
bool CHARACTER::WarpSet(long x, long y, long lPrivateMapIndex, long lCustomAddr, WORD wCustomPort)
{
...
#ifdef ENABLE_FRATELLO_FIX
	if (GetGMLevel() > GM_PLAYER && !CheckWarpEquip())
	{
		ChatPacket(CHAT_TYPE_INFO, "Your inventory is full. Clear some space before teleporting.");
		return false;
	}
#endif
	...
	if (!CAN_ENTER_ZONE(this, lMapIndex))
	{
		// Die Fehlermeldung kommt bereits aus der CAN_ENTER_ZONE Funktion
		return false;
	}
	...
	if (p.wPort != mother_port)
	{
		...
		sys_log(0, "WarpSet::P2PSendSwitchbot");
	}
#endif
...
void CHARACTER::ReqSafeboxLoad(const char* pszPassword)
{
	...
	if (!check_allow(GetGMLevel(), GM_ALLOW_USE_SAFEBOX))
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You cannot do this with this gamemaster rank."));
		return;
	}
...
void CHARACTER::SetBlockMode(BYTE bFlag)
{
	...
	SetQuestFlag("game_option.block_equipment_view", bFlag & BLOCK_EQUIPMENT_VIEW ? 1 : 0);
}
...
void CHARACTER::LoadBlockMode()
{
	BYTE bFlag = 0;
	if (GetQuestFlag("game_option.block_exchange"))
	{
		bFlag += 1;
	}
	if (GetQuestFlag("game_option.block_party_invite"))
	{
		bFlag += 2;
	}
	if (GetQuestFlag("game_option.block_guild_invite"))
	{
		bFlag += 4;
	}
	if (GetQuestFlag("game_option.block_whisper"))
	{
		bFlag += 8;
	}
	if (GetQuestFlag("game_option.block_messenger_invite"))
	{
		bFlag += 16;
	}
	if (GetQuestFlag("game_option.block_party_request"))
	{
		bFlag += 32;
	}
	if (GetQuestFlag("game_option.block_equipment_view"))
	{
		bFlag += 64;
	}
	m_pointsInstant.bBlockMode = bFlag;
	ChatPacket(CHAT_TYPE_COMMAND, "setblockmode %d", bFlag);
}
...
{
	/*
	GET PC FORCE instead of GetPC
	- to not reset current character & selected dungeon 
	*/
	...
	quest::PC* pPC = q.GetPCForce(GetPlayerID());
	if (!pPC)
	{
		return 0;
	}	
...
void CHARACTER::ResetChatCounter()
{
	...
	/* //AFK_BONUS_DEAKTIVIERT
	int iLastAfkTimeProcess = GetProtectTime("afk.wait_time");
	if (iLastAfkTimeProcess == 0 || iLastAfkTimeProcess < get_global_time())
	{
		// ChatPacket(CHAT_TYPE_INFO, "debug - msg 1 afktime");
		
		const DWORD dwMinAFKSecondsToStackBonus = 120; // and here the time when to start after x seconds u are afk time..u can edit now is 2m time start
		
		DWORD dwCurTime = get_dword_time();
		if (m_dwLastMoveTime && dwCurTime - m_dwLastMoveTime >= dwMinAFKSecondsToStackBonus * 1000)
		{
			// ChatPacket(CHAT_TYPE_INFO, "debug - msg 2 afktime u are not moving...is ok");
			
			auto pkAffect = FindAffect(AFFECT_AFK_TIME);
			if (pkAffect)
			{
				if (pkAffect->lDuration < 28800) // 8 hours
					pkAffect->lDuration = pkAffect->lDuration + 60;
					
				// ChatPacket(CHAT_TYPE_INFO, "debug - msg 3 afktime trying to inc time");
			}
			else
			{
				AddAffect(AFFECT_AFK_TIME, POINT_NONE, 0, 0, 60, 0, false);
				// ChatPacket(CHAT_TYPE_INFO, "debug - msg 4 afktime trying to add time");
			}
			
			SetProtectTime("afk.wait_time", get_global_time() + 120);
		}
	}
*/
}
...
void CHARACTER::AddAcceMaterial(TItemPos tPos, BYTE bPos)
{
	...
	else if ((bPos == 1) && (m_bAcceAbsorption))
	{
		...
		else if ((pkItem->GetType() == ITEM_ARMOR))
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You can absorb just the bonuses from weapons."));
			return;
		}
...
#ifdef ENABLE_GOLD_MAX_EXTENDED
		if (GetGold() < static_cast<unsigned long long>(dwPrice))
#else
		if (GetGold() < dwPrice)
#endif
		{
...
#ifdef ENABLE_GLOBAL_REWARD
			if (dwAbs == 25)
				CHARACTER_MANAGER::Instance().DoReward(this, REWARD_MISSION_CUSTOM_SASH, 25, 1);
#endif
...
void CHARACTER::GemShopBuy(BYTE bPos)
{
	...
	{
		sys_log(0, "[Gaya] Slot overflow.");
		return;
	}

	DWORD dwVnum = CShopManager::instance().GemShopGetVnumById(bPos);
	BYTE bCount = CShopManager::instance().GemShopGetCountById(bPos);
	DWORD dwPrice = CShopManager::instance().GemShopGetPriceById(bPos);

	if (!dwVnum || !bCount || !dwPrice)
	{
		return;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You don't have enough Gaya."));
		return;
	}
	...
	if (item)
	{
		...
		if (iEmptyPos < 0)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Gaya] Your inventory is full."));
			return;
		}
...
void CHARACTER::OpenGemShop()
{
	...
	for (int i = 0; i < GEM_SLOTS_MAX_NUM; ++i)
	{
		...
		pack.shopItems[i].dwVnum = CShopManager::instance().GemShopGetVnumById(i);
		pack.shopItems[i].bCount = CShopManager::instance().GemShopGetCountById(i);
		pack.shopItems[i].dwPrice = CShopManager::instance().GemShopGetPriceById(i);
	}
...
bool CHARACTER::CreateGaya(int glimmerstone_count, LPITEM metinstone_item, int cost, int pct)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Gaya] You don't have enough Glimmerstones."));
		return false;
	}
...
#endif
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Gaya] You don't have enough Yang to cut Glimmerstones."));
		return false;
	}

	if ((GetGem() + 3) >= (GEM_MAX - 1))
	{
		return false;
	}
...
#ifdef ENABLE_GOLD_MAX_EXTENDED
	if ((llCurrentGold + GetPrivateShopTable()->llGold) >= GOLD_MAX_MAX)
#else
...
#ifdef ENABLE_GOLD_MAX_EXTENDED
	ChangeGold(GetPrivateShopTable()->llGold);
#else
...
void CHARACTER::MasterAllSkills(LPCHARACTER ch)
{
	ch->SetHorseLevel(21);
	for (int i = 0; i < SKILL_MAX_NUM; i++)
	{
		if (true == ch->CanUseSkill(i))
		{
			...
			// @fixme154 END
			default:
				ch->SetSkillLevel(i, 30/*SKILL_MAX_LEVEL*/);
				break;
			}
		}
		else
	...
	ch->ComputePoints();
	ch->SkillLevelPacket();
	sys_log(0, "MasterAllSkills Reached");
}
#endif
...
bool CHARACTER::SkillToBook(BYTE skillIdx, DWORD& itemIdx, DWORD& socket0, DWORD& exorcismIdx, DWORD& concentratedIdx)
{
	...
	sys_log(0, "skillGrade : %d", skillGrade);
	...
	sys_log(0, "ItemIdx: %d", itemIdx);
	return false;
}

bool CHARACTER::ReadSkill(BYTE skillIdx)
{
	if (!GetSkillGroup() || IsPolymorphed() || IsDead())
		return false;
...
void CHARACTER::GetBuffs(long lDuration)
{
	if (!IsPC())
	{
		return;
	...
	// Dragon skills
	if (!FindAffect(SKILL_CRITICAL_BUFF) && !IsAffectFlag(AFF_GICHEON))
	{
		AddAffect(SKILL_CRITICAL_BUFF, POINT_CRITICAL_PCT, 50, AFF_CRITICAL_BUFF, lDuration, 0, true, true);
	}

	if (!FindAffect(SKILL_REFLECT_BUFF) && !IsAffectFlag(AFF_BOHO))
	{
		AddAffect(SKILL_REFLECT_BUFF, POINT_REFLECT_MELEE, 20, AFF_REFLECT_BUFF, lDuration, 0, true, true);
	}

	if (!FindAffect(SKILL_RESIST_BUFF) && !IsAffectFlag(AFF_HOSIN))
	{
		AddAffect(SKILL_RESIST_BUFF, POINT_RESIST_NORMAL_DAMAGE, 20, AFF_RESIST_BUFF, lDuration, 0, true, true);
	}

	// Heal skills
	if (!FindAffect(SKILL_MOV_SPEED_BUFF) && !IsAffectFlag(AFF_KWAESOK))
	{
		AddAffect(SKILL_MOV_SPEED_BUFF, POINT_MOV_SPEED, 48, AFF_MOV_SPEED_BUFF, lDuration, 0, true, true);
		AddAffect(SKILL_MOV_SPEED_BUFF, POINT_CASTING_SPEED, 44, AFF_MOV_SPEED_BUFF, lDuration, 0, true, true);
	}

	if (!FindAffect(SKILL_ATT_GRADE_BUFF) && !IsAffectFlag(AFF_JEUNGRYEOK))
	{
		AddAffect(SKILL_ATT_GRADE_BUFF, POINT_ATT_GRADE_BONUS, 50, AFF_ATT_GRADE_BUFF, lDuration, 0, true, true);
	}
}
#endif
...
{
	char fileName[254];
	snprintf(fileName, sizeof(fileName), "%s/reward_solo/%u", LocaleService_GetBasePath().c_str(), GetPlayerID());
	FILE* fp;
	// if (!fp)
	// 	return;
...
void CHARACTER::UpdateCostumeVisibilityStatus()
{
	...
	if (d)
	{
		...
		pack.type[ECostumeHideParts::HIDE_COSTUME_BODY] = GetQuestFlag("hide.costume_body");
		pack.type[ECostumeHideParts::HIDE_COSTUME_HAIR] = GetQuestFlag("hide.costume_hair");
#ifdef ENABLE_WEAPON_COSTUME_SYSTEM
		pack.type[ECostumeHideParts::HIDE_COSTUME_WEAPON] = GetQuestFlag("hide.costume_weapon");
#endif
#ifdef ENABLE_ACCE_COSTUME_SYSTEM
		pack.type[ECostumeHideParts::HIDE_COSTUME_ACCE] = GetQuestFlag("hide.costume_acce");
#endif
#ifdef ENABLE_AURA_COSTUME_SYSTEM
		pack.type[ECostumeHideParts::HIDE_COSTUME_AURA] = GetQuestFlag("hide.costume_aura");
#endif
...
#ifdef ENABLE_D_DECREASE_HP_BY_PCT
void CHARACTER::DecreaseHPByPercentage(int iPercent)
{
	const int amount = (m_pointsInstant.iMaxHP * iPercent) / 100;
	const bool bIsOverDamage = amount > GetHP();

	PointChange(POINT_HP, -(bIsOverDamage ? GetHP() : amount));
	if (GetHP() <= 0)
		Dead();
}
#endif
...
#ifdef ENABLE_ANTI_MULTIPLE_FARM
bool CHARACTER::HasBlockedDrops()
{
	LPDESC d = nullptr;
	if (!(d = GetDesc()))
		return false;
...
void CHARACTER::SaveRankingInfo()
{
	...
	RankPlayer::instance().SendInfoPlayer(this, RANK_BY_LEVEL, GetLevel());
}
#endif
...
void CHARACTER::UpdateMissionProgress(DWORD dwMissionID, DWORD dwBattlePassID, DWORD dwUpdateValue, DWORD dwTotalValue, bool isOverride, bool isSkip)
{
	...
	while (it != m_listBattlePass.end())
	{
		...
		if (pkMission->dwMissionId == dwMissionID && pkMission->dwBattlePassId == dwBattlePassID)
		{
			...
			if (pkMission->dwExtraInfo >= dwTotalValue)
			{
				...
				ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("You have completed a battle pass mission."));
				// ChatPacket(CHAT_TYPE_BIG_NOTICE, LC_TEXT("You succesfully finished %s mission from Battle Pass %s."), LC_TEXT(stMissionName.c_str()), LC_TEXT(stBattlePassName.c_str()));
...
void CHARACTER::SkipBattlePassMission(BYTE missionID)
{
	BYTE bBattlePassID = GetBattlePassId();
	if (!bBattlePassID)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You need to buy Battle Pass from ItemShop to get reward for missions."));
		return;
	}
	
	// sys_log(0, "Request to skip mission with id %d", missionID);

	DWORD dwCount, dwNotUsed;
	if (CBattlePass::instance().BattlePassMissionGetInfo(bBattlePassID, missionID, &dwNotUsed, &dwCount))
	{
		// sys_log(0, "bBattlePassID %d, missionID %d, dwNotUsed %d, dwCount %d", bBattlePassID, missionID, dwNotUsed, dwCount);

		if (GetMissionProgress(missionID, bBattlePassID) >= dwCount)
		{
			// sys_log(0, "Returned for bBattlePassID %d, missionID %d dwCount %d", bBattlePassID, missionID, dwCount);
			return;
		}

		int iSkipCashPrice = 2500;
		if (GetCoins() < iSkipCashPrice)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You need %d cash to skip the battle pass mission."), iSkipCashPrice);
			return;
		}
		
		SetCoins(GetCoins() - iSkipCashPrice);
		UpdateMissionProgress(missionID, bBattlePassID, dwCount, dwCount, true, true);
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You skipped the mission for %d cash."), iSkipCashPrice);
	}
	
	//Automatically give reward if everything is finished
	CBattlePass::instance().BattlePassRequestReward(this);
}
#endif
...
void CHARACTER::SendPromotionRewardPacket(BYTE byAnswer, std::vector<TPromotionItemTable> items)
{
	...
	itemshop_info.size = sizeof(TPacketGCItemshopInfo) + sizeof(BYTE) + sizeof(size_t) + items.size() * sizeof(TPromotionItemTable);
...
void CHARACTER::SendItemshopSingleItemRefreshPacket(TItemshopItemTable* item)
{
	...
	TPacketGCItemshopInfo itemshop_info;
	itemshop_info.header = HEADER_GC_ITEMSHOP;
	itemshop_info.size = sizeof(TPacketGCItemshopInfo) + sizeof(TItemshopItemTable);
...
void CHARACTER::SendItemshopSingleItemAddPacket(TItemshopItemTable* item, TItemshopCategoryInfo category)
{
	...
	TPacketGCItemshopInfo itemshop_info;
	itemshop_info.header = HEADER_GC_ITEMSHOP;
	itemshop_info.size = sizeof(TPacketGCItemshopInfo) + sizeof(TItemshopCategoryInfo) + sizeof(TItemshopItemTable);
...
void CHARACTER::SendItemshopItemsPacket(const std::unordered_map <BYTE, std::vector<TItemshopItemTable>> items, const std::unordered_map <BYTE, TItemshopCategoryInfo> categories)
{
	...
	TPacketGCItemshopInfo itemshop_info;
	itemshop_info.header = HEADER_GC_ITEMSHOP;
	itemshop_info.size = sizeof(TPacketGCItemshopInfo) + sizeof(TPacketGCitemshopCategorySize) + categories.size() * (sizeof(BYTE) + sizeof(TItemshopCategoryInfo)) + sizeof(TPacketGCItemshopItemSize) + items.size() * (sizeof(BYTE) + sizeof(size_t));
	itemshop_info.subheader = SUBHEADER_ITEMSHOP_REFRESH_ITEMS;
	
	for (const auto& [key, vec] : items)
	{
		itemshop_info.size += vec.size() * sizeof(TItemshopItemTable);
	}
	...
	for (const auto& [key, vec] : items) {
		...
		for (const auto& val : vec) {
			buf.write(&val, sizeof(TItemshopItemTable));
		}
	}
	GetDesc()->Packet(buf.read_peek(), buf.size());
}
void CHARACTER::SendItemshopCoinPacket()
{
	...
	d->GetAccountTable().dwCoins = coin;

	TPacketGDSetCoins packet;
	packet.dwAccountID = d->GetAccountTable().id;
	packet.dwCoins = coin;

	db_clientdesc->DBPacket(HEADER_GD_SET_COINS, d->GetHandle(), &packet, sizeof(TPacketGDSetCoins));
	DBManager::instance().Query("UPDATE account.account SET coins = %u WHERE id = '%d'", coin, d->GetAccountTable().id);

	TPacketGCDragonPointsChange p;
	p.byHeader = HEADER_GC_CHARACTER_DRAGON_POINTS_CHANGE;
	p.dwVID = m_vid;
	p.dwCash = MAX(0, GetCash());
	p.dwCoins = MAX(0, coin);
	d->Packet(&p, sizeof(TPacketGCDragonPointsChange));
}
#endif
...
DWORD CHARACTER::GetCash()
{
	if (GetDesc())
	{
		return GetDesc()->GetAccountTable().dwCash;
	}
...
void CHARACTER::SetCash(DWORD dwCash)
{
	...
	d->GetAccountTable().dwCash = dwCash;

	TPacketGDSetCash packet;
	packet.dwAccountID = d->GetAccountTable().id;
	packet.dwCash = dwCash;

	db_clientdesc->DBPacket(HEADER_GD_SET_CASH, d->GetHandle(), &packet, sizeof(TPacketGDSetCash));
	DBManager::instance().Query("UPDATE account.account SET cash = %u WHERE id = '%d'", dwCash, d->GetAccountTable().id);

	TPacketGCDragonPointsChange p;
	p.byHeader = HEADER_GC_CHARACTER_DRAGON_POINTS_CHANGE;
	p.dwVID = m_vid;
	p.dwCash = MAX(0, dwCash);
	p.dwCoins = MAX(0, GetCoins());
	d->Packet(&p, sizeof(TPacketGCDragonPointsChange));

	LogManager::instance().CashLog(GetPlayerID(), GetName(), dwCash);
}
...
DWORD CHARACTER::GetCoins()
{
	if (GetDesc())
	{
		return GetDesc()->GetAccountTable().dwCoins;
	}

	return 0;
}

void CHARACTER::SetCoins(DWORD dwCoins)
{
	LPDESC d = GetDesc();
	...
	d->GetAccountTable().dwCoins = dwCoins;

	TPacketGDSetCoins packet;
	packet.dwAccountID = d->GetAccountTable().id;
	packet.dwCoins = dwCoins;

	db_clientdesc->DBPacket(HEADER_GD_SET_COINS, d->GetHandle(), &packet, sizeof(TPacketGDSetCoins));
	DBManager::instance().Query("UPDATE account.account SET coins = %u WHERE id = '%d'", dwCoins, d->GetAccountTable().id);

	TPacketGCDragonPointsChange p;
	p.byHeader = HEADER_GC_CHARACTER_DRAGON_POINTS_CHANGE;
	p.dwVID = m_vid;
	p.dwCash = MAX(0, GetCash());
	p.dwCoins = MAX(0, dwCoins);
	d->Packet(&p, sizeof(TPacketGCDragonPointsChange));
}
...
#ifdef __GUILD_SAFEBOX__
bool CHARACTER::IsOpenGuildSafebox()
{
	return GetGuild() && GetGuild()->GetSafeBox().HasSafebox() && GetGuild()->GetSafeBox().IsViewer(this);
}
#endif

#ifdef ENABLE_TELEPORT_GUILD
bool CHARACTER::WarpSetNew(long x, long y, long lMapIndex, WORD wPort)
{
	if (!IsPC())
		return false;
	...
	if (!CMapLocation::instance().Get(GetX(), GetY(), lMapIndexempty, lAddr, wPortEmty))
	{
		sys_err("cannot find map location index ...");
		return false;
	}
...
bool CHARACTER::IsCanRespondRequestWarp(std::string stName)
{
	if (stName.empty())
		return false;
	
	auto iter = m_map_warp_request.find(stName);
	if (iter != m_map_warp_request.end() && iter->second >= get_global_time())
		return true;

	return false;
}

void CHARACTER::SetCanRespondRequestWarp(std::string stName)
{
	// Maximum 5 requests in last 10 seconds..
	if (m_map_warp_request.size() >= 5)
		return;
	
	if (!stName.empty())
		m_map_warp_request[stName] = get_global_time() + 10;
}

void CHARACTER::RemoveCanRespondRequestWarp(std::string stName)
{
	if (stName.empty())
		return;
	
	auto iter = m_map_warp_request.find(stName);
	if (iter != m_map_warp_request.end())
		m_map_warp_request.erase(iter);
}
...
#ifdef BLOCK_TELEPORT_HACK
static const long   kHardTpDistance = 3000;
static const DWORD  kIgnoreDtThreshold = 3000;
static const DWORD  kMinDt = 30;
static const DWORD  kMaxSampleDt = 1000;
static const float  kBaseUnitsPerSec = 350.0f;
static const float  kUnitsPerMoveSpd = 5.0f;
static const float  kFudge = 300.0f;
static const int    kStrikeLimit = 3;

void CHARACTER::StopAntiTPInit()
{
	m_bAntiTPInit = false;
}

bool CHARACTER::AntiTeleportCheck(long x, long y)
{
	const DWORD now = get_dword_time();
	const DWORD dt = (m_dwLastSyncTime ? now - m_dwLastSyncTime : 0);

	if (!m_bAntiTPInit) return false;
	if (m_lLastIdx != GetMapIndex()) return false;
	if (IsDead() || IsStun()) return false;

	const long dx = x - m_lLastX;
	const long dy = y - m_lLastY;
	const long dist = DISTANCE_APPROX(dx, dy);

	// -------- HARD TELEPORT --------
	if (dist >= kHardTpDistance)
	{
		sys_log(0, "ANTI_TP(HARD) PID:%u Name:%s Map:%ld Dist:%ld dt:%u",
			GetPlayerID(), GetName(), GetMapIndex(), dist, dt);

		if (!IsGM()) // Ausnahme fur GMs
		{
			LPDESC d = GetDesc();
			if (d)
			{
				ChatPacket(CHAT_TYPE_INFO, "Teleport Hack detected (Hard).");
	...
	if (dt >= kIgnoreDtThreshold) return false;
	if (dt < kMinDt) return false;

	const int moveSpd = GetLimitPoint(POINT_MOV_SPEED);
	const float unitsPerSec = kBaseUnitsPerSec + kUnitsPerMoveSpd * moveSpd;

	const DWORD dtClamped = dt ? std::min<DWORD>(dt, kMaxSampleDt) : 1;
	const float allowed = unitsPerSec * (dtClamped / 1000.0f) + kFudge;

	// -------- SOFT TELEPORT (Strike System) --------
	if (dist > allowed)
	{
		m_iTpStrike++;

		sys_log(0, "ANTI_TP PID:%u Name:%s Map:%ld Dist:%ld Allowed:%.0f dt:%u (spd:%d) strikes:%d",
			GetPlayerID(), GetName(), GetMapIndex(), dist, allowed, dt, moveSpd, m_iTpStrike);

		if (m_iTpStrike >= kStrikeLimit)
		{
			if (!IsGM()) // Ausnahme fur GMs
			{
				LPDESC d = GetDesc();
				if (d)
				{
					ChatPacket(CHAT_TYPE_INFO, "Teleport Hack detected.");
					d->DelayedDisconnect(3);
				}
			}
	...
	{
		if (m_iTpStrike > 0)
			m_iTpStrike = std::max(0, m_iTpStrike - 1);
	}
