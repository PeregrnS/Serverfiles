...
#if defined(__ENABLE_BIOLOG_SYSTEM__) || defined(RENEWAL_PICKUP_AFFECT) || defined(ENABLE_VOTE_4_BUFF)
			switch (pkAff->dwType)
			{
#if defined(__ENABLE_BIOLOG_SYSTEM__)
				case AFFECT_BIOLOG:
#endif
#if defined(RENEWAL_PICKUP_AFFECT)
				case AFFECT_PICKUP_ENABLE:
				case AFFECT_PICKUP_DEACTIVE:
#endif
#if defined(ENABLE_VOTE_4_BUFF)
				case AFFECT_VOTE_4_BUFF:
#endif
...
//AFK BONUS
			if (pkAff->dwType == AFFECT_AFK_TIME)
			{
				++it;
				continue;
			}
			...
			if (bSkipAffect)
			{
				...
				case (SKILL_MOV_SPEED_BUFF):
				case (SKILL_ATT_GRADE_BUFF):
#endif
				{
					++it;
					continue;
...
int CHARACTER::ProcessAffect()
{
	...
	while (it != m_list_pkAffect.end())
	{
		pkAff = *it;
		//AFK BONUS
		if (pkAff->dwType == AFFECT_AFK_TIME) // if is afk timeaffect and is not moved in last 10seconds then not process time
		{
			bool bMoving = (get_dword_time() - GetLastMoveTime()) < 3000;
			if (!bMoving)
			{
				// ChatPacket(CHAT_TYPE_INFO, "debug - msg afktime skipping the process time of this");
		...
		if (bEnd)
		{
			...
			// <<< NEU: Punkte sofort aktualisieren
			ComputePoints();
			UpdatePacket();
...
void CHARACTER::LoadAffect(DWORD dwCount, TPacketAffectElement * pElements)
{
	...
	// The player isn't in the dungeon anymore. Remove unwanted affects
	if (!GetDungeon())
		RemoveAffect(AFFECT_UNIQUE_POINT_DUNGEON);
#endif
	...
	RemoveAffect(AFFECT_VOTE_4_BUFF);

	DWORD dwPlayerId = GetDesc()->GetAccountTable().id;
	...
	int iDurVote4Buff = quest::CQuestManager::instance().GetEventFlag(voteGameFlag);
	int iApplyOnVote4Buff = quest::CQuestManager::instance().GetEventFlag(voteGameFlagApply);
	int iApplyValVote4Buff = quest::CQuestManager::instance().GetEventFlag(voteGameFlagValue);
	
	if (iDurVote4Buff >= get_global_time())
		AddAffect(AFFECT_VOTE_4_BUFF, iApplyOnVote4Buff, iApplyValVote4Buff, 0, (iDurVote4Buff - get_global_time()), 0, false, false, iDurVote4Buff);
#endif
}
...
void CHARACTER::ComputeAffect(CAffect * pkAff, bool bAdd)
{
	...
	if (!bAdd) // Affect wird entfernt → nur Points abziehen, wenn Buff existiert!
	{
		auto it = m_map_buff_on_attrs.find(pkAff->bApplyOn);
	
		// Wenn kein Buff existiert → NICHT PointChange ausfuhren → verhindert Crash!
		if (it == m_map_buff_on_attrs.end() || it->second == NULL)
		{
			// Buff existiert nicht → keine Anderung notig
		}
		else
		{
			// Buff existiert → normaler Affect-Remove
			PointChange(pkAff->bApplyOn, -pkAff->lApplyValue);
		}
	}
	else
	{
		// Affect wird hinzugefugt → normaler Ablauf
		PointChange(pkAff->bApplyOn, pkAff->lApplyValue);
	}
//AFK BONUS
	if (pkAff->dwType == AFFECT_AFK_TIME)
	{
		// ChatPacket(CHAT_TYPE_INFO, "badd %d afk_time ", bAdd);
		
		PointChange(POINT_ATTBONUS_MONSTER, bAdd ? 10 : -10);
		PointChange(POINT_ATTBONUS_BOSS, bAdd ? 10 : -10);
		PointChange(POINT_ATTBONUS_STONE, bAdd ? 10 : -10);
	}
...
void CHARACTER::RemoveGoodAffect()
{
	...
	RemoveAffect(SKILL_MOV_SPEED_BUFF);
	RemoveAffect(SKILL_ATT_GRADE_BUFF);
#endif
}
