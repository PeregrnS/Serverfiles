...
bool CHARACTER::Attack(LPCHARACTER pkVictim, BYTE bType)
{
	...
	if (g_bChannel == 3)  // <-- HIER CHANNEL ANDERN!
	{
		if (IsPC() && pkVictim && pkVictim->IsPC())
		{
			// Optional: Infos fur Spieler
			ChatPacket(CHAT_TYPE_INFO, "PvP ist auf diesem Channel deaktiviert.");
	...
	if (IsPC())
	{
		...
		else
		{
			...
			if (pkVictim->IsMonster() && pkVictim->GetMobRank() >= MOB_RANK_BOSS)
				result = GetCShield()->CheckAttackspeedHack(IsRiding(), isDagger || isFan || isClaw, isTwoHanded, ani_attack_speed(this), GetLimitPoint(POINT_ATT_SPEED), pkVictim->GetVID(), get_dword_time(), 1, extendedCheck);
			else if (pkVictim->IsMonster())
				result = GetCShield()->CheckAttackspeedHack(IsRiding(), isDagger || isFan || isClaw, isTwoHanded, ani_attack_speed(this), GetLimitPoint(POINT_ATT_SPEED), pkVictim->GetVID(), get_dword_time(), 2, extendedCheck);
			else if (pkVictim->IsStone())
				result = GetCShield()->CheckAttackspeedHack(IsRiding(), isDagger || isFan || isClaw, isTwoHanded, ani_attack_speed(this), GetLimitPoint(POINT_ATT_SPEED), pkVictim->GetVID(), get_dword_time(), 3, extendedCheck);
			else if (pkVictim->IsPC())
				result = GetCShield()->CheckAttackspeedHack(IsRiding(), isDagger || isFan || isClaw, isTwoHanded, ani_attack_speed(this), GetLimitPoint(POINT_ATT_SPEED), pkVictim->GetVID(), get_dword_time(), 4, extendedCheck);
		}
#else
		if (IsPolymorphed())
		{
...
#ifdef ENABLE_DAILY_BOSS
	if (CDailyBoss::Instance().IsBossMap(GetMapIndex()))
	{
		return;
	}
#endif
...
{
	// START_BLOCK_PLAYER
	if (pkAttacker && pkAttacker->IsPC() && g_isBlockedPlayer(pkAttacker->GetName()))
		return;
	// END_OF_BLOCK_PLAYER
...
/* 	if (GetMobRank() >= MOB_RANK_BOSS)
	...
	else
	{
...
#ifdef ENABLE_MAP_LEVEL_YANG_LIMIT
		// --- Start Map-Level-Yang-Check ---
		int iMapIdx = pkAttacker->GetMapIndex();
		int iLvl = pkAttacker->GetLevel();
		bool bBlockYang = false;

		switch (iMapIdx)
		{
			case 1: case 21: case 41: // Map 1 Startgebiete
				if (iLvl >= 20) bBlockYang = true; break;
			case 3: case 23: case 43: // Map 2 Gebiete
				if (iLvl >= 30) bBlockYang = true; break;
			case 424: if (iLvl >= 125) bBlockYang = true; break;
			case 339: if (iLvl >= 185) bBlockYang = true; break;
			case 311: if (iLvl >= 200) bBlockYang = true; break;
			case 116: if (iLvl >= 290) bBlockYang = true; break;
			case 430: case 322: case 431: // Metin Maps
				if (iLvl >= 100) bBlockYang = true; break;
			case 104: case 71: case 337: case 423: case 426: case 130: case 320: // Dungeons & Runs
				if (iLvl >= 100) bBlockYang = true; break;
		}
		...
		// WICHTIG: Nur den Gold-Teil uberspringen, nicht die ganze Funktion!
		if (iGold > 0) 
		{
			int iSplitCount;
			if (iGold >= 3)
...
void CHARACTER::Reward(bool bItemDrop)
{
	...
	{
		// START_BLOCK_PLAYER
		if (g_isBlockedPlayer(pkAttacker->GetName()))
			return;
		// END_OF_BLOCK_PLAYER
		...
		if (bBattlePassId)
		{
			...
			if (CBattlePass::instance().BattlePassMissionGetInfo(bBattlePassId, MONSTER_KILL_ALL, &dwMonsterVnum, &dwToKillCount))
			{
				// sys_log(0, "dwToKillCount for MONSTER_KILL_ALL is %d", dwToKillCount);

				if (IsMonster() && pkAttacker->GetMissionProgress(MONSTER_KILL_ALL, bBattlePassId) < dwToKillCount)
				{
					pkAttacker->UpdateMissionProgress(MONSTER_KILL_ALL, bBattlePassId, 1, dwToKillCount);
				}
			}
		}
#endif
		...
		if (!number(0, 9))
		{
			if (pkAttacker->GetPoint(POINT_KILL_HP_RECOVERY))
			{
				...
				CreateFly(FLY_HP_MEDIUM, pkAttacker);
			}
			...
			if (pkAttacker->GetPoint(POINT_KILL_SP_RECOVER))
			{
				...
				CreateFly(FLY_SP_MEDIUM, pkAttacker);
			}
		}
	}
	// pu1.Pop();
	...
	if (ITEM_MANAGER::instance().CreateDropItem(this, pkAttacker, s_vec_item))
	{
		...
		else if (CHARACTER_MANAGER::instance().IsWorldBoss(GetRaceNum()))
		{
            // Everyone shares ownership
            int iItemIdx = s_vec_item.size() - 1;

            std::priority_queue<std::pair<long long int, LPCHARACTER>> pq;

            long long total_dam = 0;

            for (auto &it : m_map_kDamage)
            {
                auto iDamage = it.second.iTotalDamage;
			...
			const int iMaxDropsCharactersByDamageHighest = 5; // edit value by how many chars can get items from it...(sorted by highest dmg)
			...
			while (!pq.empty())
			{
				...
				if (iCharsDmg > iMaxDropsCharactersByDamageHighest)
					break;
			}
...

			{
				if (!v.empty() && v.size() > 1)
				{
					const auto randomIt = number(0, v.size() - 1);
					ch = v[randomIt];
				}
			}
		...
		else if (s_vec_item.size() == 1)
		{
			...
			else
			{
				...
				if (pkAttacker->CanPickupDirectly())
				{
					...
					// sys_log(0, "OnReward CanPickUpDirectley 001!");
				}
	#endif
			}
#else
			...
			{
				pkAttacker->PickupItem(item->GetVID());
				// sys_log(0, "OnReward CanPickUpDirectley 002!");
			}
#else
			item->StartDestroyEvent();
#endif
		...
		else
		{
			...
			int maxOwners = 20; 
			while (!pq.empty() && v.size() < maxOwners)
			{
				v.push_back(pq.top().second);
				pq.pop();
			}
			...
			if (v.empty())
			{
				LPCHARACTER fallbackOwner = pkAttacker;
			
				// Falls pkAttacker NULL ist, nimm MostAttacked
				if (!fallbackOwner)
					fallbackOwner = GetMostAttacked();
				...
				while (iItemIdx >= 0)
				{
					...
					if (fallbackOwner)
						item->SetOwnership(fallbackOwner);  // <<< WICHTIG
				...
				while (iItemIdx >= 0)
				{
					...
					else
					{
						...
						if (ch->CanPickupDirectly())
						{
							...
							// sys_log(0, "OnReward CanPickUpDirectley 003!");
						}
	#endif
					}
#else
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	if (pkKiller && pkKiller->IsPC())
	{
		...
		if (IsPC())
		{
			RankPlayer::instance().SendInfoPlayer(this, RANK_BY_DEATH, RankPlayer::instance().GetProgressByPID(GetPlayerID(), RANK_BY_DEATH) + 1);
		}
#endif
...
#ifdef __DAILY_QUESTS__
		CDailyQuestManager::instance().OnKill(pkKiller, this);
#endif

#ifdef __SPIN_WHEEL__
		if (quest::CQuestManager::instance().GetEventFlag("spin_wheel") == 1 && IsStone())
		{
		...
		if (quest::CQuestManager::instance().GetEventFlag("spin_wheel_boss") == 1 && IsBoss())
		{
			const int newBossCount = pkKiller->GetQuestFlag("spin_wheel_boss.count") + 1;
			if (newBossCount <= SPIN_WHEEL_BOSS_COUNT)
			{
				pkKiller->SetQuestFlag("spin_wheel_boss.count", newBossCount);
				pkKiller->ChatPacket(CHAT_TYPE_COMMAND, "SetSpinWheelBoss %d 1", newBossCount);
			}
		}
#endif
		...
		{
			if (IsStone())
				CHARACTER_MANAGER::Instance().DoReward(pkKiller, REWARD_MISSION_KILL_STONE, GetRaceNum(), 1, GetLevel());
			else if (GetMobRank() >= MOB_RANK_BOSS)
				CHARACTER_MANAGER::Instance().DoReward(pkKiller, REWARD_MISSION_KILL_BOSS, GetRaceNum(), 1, GetLevel());
			else
				CHARACTER_MANAGER::Instance().DoReward(pkKiller, REWARD_MISSION_KILL_MONSTER, GetRaceNum(), 1, GetLevel());
		}
#endif
		...
		{
			if (CDailyBoss::instance().IsDailyBoss(GetRaceNum()))
			{
				CDailyBoss::instance().OnKill(pkKiller, (DWORD)GetVID(), GetRaceNum());
			}
		}
#endif
...
#ifdef ENABLE_EVENT_MANAGER
	if (GetMapIndex() == SIEGE_MAP_INDEX)
	{
		...
		else
		{
			...
			if (bBattlePassId)
			{
				...
				if (CBattlePass::instance().BattlePassMissionGetInfo(bBattlePassId, PLAYER_KILL, &dwMinLevel, &dwToKillCount))
				{
					...
					if ((GetDesc()->GetHostName() != pkKiller->GetDesc()->GetHostName()) 
						&& GetLevel() >= dwMinLevel 
						&& CBattlePass::instance().IsEligibleForPlayerKill(pkKiller->GetPlayerID(), GetPlayerID()))
					{
						if (pkKiller->GetMissionProgress(PLAYER_KILL, bBattlePassId) < dwToKillCount)
						{
	...
	// BOSS_KILL_LOG
#ifdef ENABLE_RANK_PLAYER
	if (pkKiller && pkKiller->IsPC() && GetRaceNum() > 100)
	{
		if (IsMonster() == true && GetMobRank() < MOB_RANK_BOSS)
		{
			RankPlayer::instance().SendInfoPlayer(pkKiller, RANK_BY_MONSTER, RankPlayer::instance().GetProgressByPID(pkKiller->GetPlayerID(), RANK_BY_MONSTER) + 1);
		}
		else if (IsStone())
		{
		...
		else if (GetMobRank() >= MOB_RANK_BOSS && !IsStone())
		{
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
	...
	if (pAttacker)
	{
		if (pAttacker->IsPC() && CDailyBoss::Instance().IsBossMap(pAttacker->GetMapIndex())) 
		{
			// if (CDailyBoss::Instance().IsDailyBoss(GetRaceNum()) || GetRaceNum() == CDailyBoss::Instance().SubBossVnum())
			if (CDailyBoss::Instance().IsDailyBoss(GetRaceNum()))
			{
				int emblemProb = number(1, 100);
				if (emblemProb <= 10)
				{
					pAttacker->AutoGiveItem(CDailyBoss::Instance().GetEmblemVnum(), 1);
				}
			}
		}
	}
...
//		if (pPC->IsRunning())
//		{
//			// AUTO-FIX: Hangende Quest beenden
//			auto& mgr = quest::CQuestManager::instance();

//			sys_err("AUTO-FIX: Player %s had suspended quest state (%s). Forcing quest end.",
//				pAttacker->GetName(),
//				pPC->GetCurrentQuestName().c_str());

//			pPC->EndRunning(); // <- beendet die Quest sauber
//			// Alternative falls deine Version keine EndRunning() hat:
//			// pPC->SetRunningState(false);

//			// Danach Schaden normal zulassen:
//			// KEIN return false !
//		}
//	}
//#endif
	...
	if (type == DAMAGE_TYPE_MELEE || type == DAMAGE_TYPE_RANGE || type == DAMAGE_TYPE_MAGIC)
	{
		if (pAttacker)
		{
			...
			if (iCriticalPct)
			{
				...
				if (GetQuestFlag("duel_type") == 2 && CPVPManager::instance().IsSpecialDuel(pAttacker, this)) // german
				{
	...
	switch (type)
	{
		...
		case DAMAGE_TYPE_NORMAL_RANGE:
			...
			if (CPVPManager::instance().IsSpecialDuel(pAttacker, this))
			{
				if (GetQuestFlag("duel_type") == 1) // DUEL TIP TACTIC
				{
					switch (GetJob())
					{
						case JOB_ASSASSIN:
				...
				if (GetQuestFlag("duel_type") == 2) // DUEL TIP GERMAN
				{
					switch (GetJob())
					{
						case JOB_ASSASSIN:
		...
		case DAMAGE_TYPE_MAGIC:
			if (pAttacker && (IsMonster() || IsStone()))
			{
				...
				// sys_log(0, "pAttacker applied skill damage bonus against monster");
			}
			...
			if (!CPVPManager::instance().IsSpecialDuel(pAttacker, this))
				...
				dam = dam * (100 - GetPoint(POINT_SKILL_DEFEND_BONUS)) / 100;
#else
			dam = dam * (100 - MIN(99, GetPoint(POINT_SKILL_DEFEND_BONUS))) / 100;
#endif
			break;
	...
	// @cap damage stone @grimm
	if (dam > 0 && pAttacker && pAttacker->IsPC() && IsStone())
	{
		if (dam > GetMaxHP())
			dam = GetMaxHP() / 2;
	}
	...
	if (pAttacker)
	{
		...
		if (pAttacker)
		{
			...
			if (IsPC() && !pAttacker->IsPC())
				CDailyQuestManager::instance().Progress(this, 0, DAILY_TYPE_RECEIVE_DAMAGE, false, dam);
			if (!IsPC() && pAttacker->IsPC())
				CDailyQuestManager::instance().Progress(pAttacker, 0, DAILY_TYPE_SEND_DAMAGE, false, dam);
#endif
			
			SendDamagePacket(pAttacker, dam, damageFlag);
#ifdef ENABLE_DAILY_BOSS
			if (CDailyBoss::Instance().IsDailyBoss(GetRaceNum()) &&
				CDailyBoss::Instance().IsBossMap(pAttacker->GetMapIndex()) && pAttacker->IsPC())
				pAttacker->RegisterDamage((DWORD)GetVID(), dam);
#endif
		}
			...
			if (bBattlePassId)
			{
				if (type != EDamageType::DAMAGE_TYPE_POISON)
				{
					...
					else
					{
						...
						if (CBattlePass::instance().BattlePassMissionGetInfo(bBattlePassId, MONSTER_DAMAGE_ALL, &dwMonsterVnum, &dwDamage))
						{
							DWORD dwCurrentDamage = pAttacker->GetMissionProgress(MONSTER_DAMAGE_ALL, bBattlePassId);
							if (IsMonster() && GetLevel() >= 30  && dwCurrentDamage < dwDamage/* && dwCurrentDamage < dam*/)
							{
								pAttacker->UpdateMissionProgress(MONSTER_DAMAGE_ALL, bBattlePassId, dam, dwDamage);
							}
						}
					}
				}
	...
	if (GetHP() <= 0)
	{
		if (IsPC() && pAttacker->IsNPC())
		{
			SetQuestFlag("player.slain_by_monster", 1);
		}
		else
			SetQuestFlag("player.slain_by_monster", 0);
	...
	public:
		...
		void operator () (DWORD dwTargetVID)
		{
			...
			switch (m_bType)
			{
				...
				case SKILL_BYEURAK: // fix skill vnum 107
					{
						m_me->OnMove(true);
						pkVictim->OnMove();
						...
						sys_log(0, "%s - Skill %d -> %s", m_me->GetName(), m_bType, pkVictim->GetName());
						//m_me->ComputeSkill(m_bType, pkVictim); // fix double dn
					}
					break;
...
//#ifdef ENABLE_HIGHER_BRAVERY_CAPE_CHANCE
//			if (number(1, 100) <= 100) // 100% Chance
//#else
//			if (number(1, 100) <= 50) // 50% Chance
//#endif

#ifdef ENABLE_BRAVERY_CAPE_RANGE_EXTENDED
				if (DISTANCE_APPROX(ch->GetX() - m_ch->GetX(), ch->GetY() - m_ch->GetY()) < 5000) //change bravy cape distance
#else
...
#endif
						//ch->PointChange(POINT_MOV_SPEED, 150); //change to your move speed
						//ch->UpdatePacket();
					}
				}
		}
...
#ifdef ENABLE_DAILY_BOSS
int CHARACTER::GetDamageByVid(DWORD dwTargetVID)
{
	auto it = m_CharacterDamageInfo.find(dwTargetVID);
	if (it == m_CharacterDamageInfo.end())
	{
		return 0;
	}

	int iTotalDamage = it->second;
	return iTotalDamage;
}

void CHARACTER::RegisterDamage(DWORD dwTargetVID, int iDamage)
{
	DWORD iTotalDamage = 0;
	if (GetDamageByVid(dwTargetVID) == 0)
	{
		m_CharacterDamageInfo.insert(std::make_pair(dwTargetVID, iDamage));
	}

	iTotalDamage = GetDamageByVid(dwTargetVID);
	iTotalDamage += iDamage;
	m_CharacterDamageInfo[dwTargetVID] = iTotalDamage;
}
#endif
