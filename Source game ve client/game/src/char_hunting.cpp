...
void CHARACTER::CheckHunting(bool isLevelUp)
{
	...
	if (GetQuestFlag("hunting_system.is_active") == -1 && GetLevel() >= GetQuestFlag("hunting_system.level"))
		ChatPacket(CHAT_TYPE_COMMAND, "HuntingButtonFlash");
	
	if (GetQuestFlag("hunting_system.is_active") == 1 && GetQuestFlag("hunting_system.count") == THuntingMissions[GetQuestFlag("hunting_system.level")][GetQuestFlag("hunting_system.type")][1])
		ChatPacket(CHAT_TYPE_COMMAND, "HuntingButtonFlash");
}
...
void CHARACTER::UpdateHuntingMission(DWORD dwMonsterVnum)
{
	...
	if (GetLevel() >= GetQuestFlag("hunting_system.level") && THuntingMissions[iLevel][iType][0] == dwMonsterVnum && THuntingMissions[iLevel][iType][1] > GetQuestFlag("hunting_system.count"))
	{
	...
	if (THuntingMissions[iLevel][iType][1] <= GetQuestFlag("hunting_system.count") && GetQuestFlag("hunting_system.reward_cached") == 0)
	{
		SetCachedRewards();
		ChatPacket(CHAT_TYPE_COMMAND, "HuntingButtonFlash");
	}
...
void CHARACTER::ReciveHuntingRewards()
{
	if (GetQuestFlag("hunting_system.reward_race") != 0) { 
		...
		LPITEM RaceItem = ITEM_MANAGER::instance().CreateItem(item_vnum, item_count, 0, false);
		
#ifdef ENABLE_SPECIAL_INVENTORY
		int cell = GetEmptyInventory(RaceItem);
#else
		int cell = GetEmptyInventory(RaceItem->GetSize());
#endif
		if (cell != -1) {
			if (RaceItem->IsDragonSoul())
				RaceItem->AddToCharacter(this, TItemPos(DRAGON_SOUL_INVENTORY, cell));
			else
				RaceItem->AddToCharacter(this, TItemPos(INVENTORY, cell));

			if (RaceItem->GetType() == ITEM_USE && RaceItem->GetSubType() == USE_POTION) {
				TQuickslot * pSlot;
			...
			RaceItem->AddToGround(GetMapIndex(), GetXYZ());
			RaceItem->StartDestroyEvent();
			RaceItem->SetOwnership(this, 60);
		}
	...
	if (GetQuestFlag("hunting_system.reward_money") != 0) { 
		...
		ChangeGold(GetQuestFlag("hunting_system.reward_money"));
#else
...
void CHARACTER::SetRewardRandomItemFromTable()
{
	...
	if (MissionLevel >= 1 && MissionLevel <= 5) {
		int tableSize = (sizeof THuntingRandomItem_01_5/sizeof THuntingRandomItem_01_5[0]);
		int randItem = number(0, tableSize-1);
		itemVnum = THuntingRandomItem_01_5[randItem][0];
		itemCount = THuntingRandomItem_01_5[randItem][1];
	}
	if (MissionLevel >= 6 && MissionLevel <= 10){
		int tableSize = (sizeof THuntingRandomItem_6_10/sizeof THuntingRandomItem_6_10[0]);
		int randItem = number(0, tableSize-1);
		itemVnum = THuntingRandomItem_6_10[randItem][0];
		itemCount = THuntingRandomItem_6_10[randItem][1];
	}
	if (MissionLevel >= 11 && MissionLevel <= 15){
		int tableSize = (sizeof THuntingRandomItem_11_15/sizeof THuntingRandomItem_11_15[0]);
		int randItem = number(0, tableSize-1);
		itemVnum = THuntingRandomItem_11_15[randItem][0];
		itemCount = THuntingRandomItem_11_15[randItem][1];
	}
	if (MissionLevel >= 16 && MissionLevel <= 22){
		int tableSize = (sizeof THuntingRandomItem_16_22/sizeof THuntingRandomItem_16_22[0]);
		int randItem = number(0, tableSize-1);
		itemVnum = THuntingRandomItem_16_22[randItem][0];
		itemCount = THuntingRandomItem_16_22[randItem][1];
	}
	if (MissionLevel >= 23 && MissionLevel <= 28){
		int tableSize = (sizeof THuntingRandomItem_23_28/sizeof THuntingRandomItem_23_28[0]);
		int randItem = number(0, tableSize-1);
		itemVnum = THuntingRandomItem_23_28[randItem][0];
		itemCount = THuntingRandomItem_23_28[randItem][1];
	}
...
void CHARACTER::SendRandomItemPacket(bool IsSelectWindow)
{
	...
	if (MissionLevel >= 1 && MissionLevel <= 5) {
		DWORD TableSize = (sizeof THuntingRandomItem_01_5/sizeof THuntingRandomItem_01_5[0]);
		for (int i = 0; i < TableSize; ++i) {
			TPacketGCReciveRandomItems pack;
			pack.bHeader = HEADER_GC_HUNTING_RECIVE_RAND_ITEMS;
			pack.bWindow = IsSelectWindow;
			pack.dItemVnum = THuntingRandomItem_01_5[i][0];
			pack.dItemCount = THuntingRandomItem_01_5[i][1];
			GetDesc()->Packet(&pack, sizeof(pack));
		}
	}
	if (MissionLevel >= 6 && MissionLevel <= 10) {
		DWORD TableSize = (sizeof THuntingRandomItem_6_10/sizeof THuntingRandomItem_6_10[0]);
		for (int i = 0; i < TableSize; ++i) {
			TPacketGCReciveRandomItems pack;
			pack.bHeader = HEADER_GC_HUNTING_RECIVE_RAND_ITEMS;
			pack.bWindow = IsSelectWindow;
			pack.dItemVnum = THuntingRandomItem_6_10[i][0];
			pack.dItemCount = THuntingRandomItem_6_10[i][1];
			GetDesc()->Packet(&pack, sizeof(pack));
		}
	}
	if (MissionLevel >= 11 && MissionLevel <= 15) {
		DWORD TableSize = (sizeof THuntingRandomItem_11_15/sizeof THuntingRandomItem_11_15[0]);
		for (int i = 0; i < TableSize; ++i) {
			TPacketGCReciveRandomItems pack;
			pack.bHeader = HEADER_GC_HUNTING_RECIVE_RAND_ITEMS;
			pack.bWindow = IsSelectWindow;
			pack.dItemVnum = THuntingRandomItem_11_15[i][0];
			pack.dItemCount = THuntingRandomItem_11_15[i][1];
			GetDesc()->Packet(&pack, sizeof(pack));
		}
	}
	if (MissionLevel >= 16 && MissionLevel <= 22) {
		DWORD TableSize = (sizeof THuntingRandomItem_16_22/sizeof THuntingRandomItem_16_22[0]);
		for (int i = 0; i < TableSize; ++i) {
			TPacketGCReciveRandomItems pack;
			pack.bHeader = HEADER_GC_HUNTING_RECIVE_RAND_ITEMS;
			pack.bWindow = IsSelectWindow;
			pack.dItemVnum = THuntingRandomItem_16_22[i][0];
			pack.dItemCount = THuntingRandomItem_16_22[i][1];
			GetDesc()->Packet(&pack, sizeof(pack));
		}
	}
	if (MissionLevel >= 23 && MissionLevel <= 28) {
		DWORD TableSize = (sizeof THuntingRandomItem_23_28/sizeof THuntingRandomItem_23_28[0]);
		for (int i = 0; i < TableSize; ++i) {
			TPacketGCReciveRandomItems pack;
			pack.bHeader = HEADER_GC_HUNTING_RECIVE_RAND_ITEMS;
			pack.bWindow = IsSelectWindow;
			pack.dItemVnum = THuntingRandomItem_23_28[i][0];
			pack.dItemCount = THuntingRandomItem_23_28[i][1];
			GetDesc()->Packet(&pack, sizeof(pack));
		}
	}
}
...
int CHARACTER::GetMinMaxMoney(DWORD missionLevel, bool AskMax)
{
	for(int i = 0; i < HUNTING_MONEY_TABLE_SIZE; ++i) {
		...
		if (missionLevel >= info.dLevelFrom && missionLevel <= info.dLevelTo)
			if (!AskMax)
