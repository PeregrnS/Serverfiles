...
public:
	void DoReward(LPCHARACTER ch, const BYTE bType, const DWORD dwSubValue, DWORD value, const BYTE levelDifference = 0);
	void SetGlobalReward(const char* szPlayerName, const char* szFilename, ...);
	bool GetGlobalReward(std::string& szPlayerName, const char* szFilename, ...);
	void SendRewardInfo(const BYTE bType, const bool isGlobal, bool isP2P = false, LPCHARACTER ch = NULL);
#endif
...
public:	
	void	LoadConfigWorldBoss();
	bool	CheckWorldBossSpawn();

	std::vector<SConfigWorldBoss> m_vec_worldBoss;
	std::map<DWORD, bool> m_map_worldBoss;

	bool	IsWorldBoss(DWORD dwRaceNum)
	{
		return (m_map_worldBoss.find(dwRaceNum) != m_map_worldBoss.end());
	}
#endif
