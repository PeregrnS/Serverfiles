...
void CHARACTER::SetSkillLevel(DWORD dwVnum, BYTE bLev)
{
	...
	if (GetSkillGroup())
	{
		if (dwVnum <= 111)
		{
			CHARACTER_MANAGER::Instance().DoReward(this, REWARD_MISSION_SKILL_UPGRADE, m_pSkillLevels[dwVnum].bLevel, 1);

			const std::vector<std::vector<std::vector<BYTE>>> m_vecSkills = {
				 {
					  { 1, 2, 3, 4, 5},
					  { 16, 17, 18, 19, 20},
				 },
			...
			const std::vector<BYTE>& skilllist = m_vecSkills[GetJob()][GetSkillGroup() - 1];
			for (BYTE i = 0; i < skilllist.size(); ++i)
			{
				if (GetSkillLevel(skilllist[i]) < 50)
					return;
			}

			CHARACTER_MANAGER::Instance().DoReward(this, REWARD_MISSION_COMPLETE_SKILL, 0, 1);
		}
		else
		{
			if (bLev == 40)//last-level-passive
				CHARACTER_MANAGER::Instance().DoReward(this, REWARD_MISSION_PASSIVE_SKILL_COMPLETE, dwVnum, 1);
		}
	}
#endif
}
	...
	else
	{
...

		{
			...
			if (pPC)
			{
				...
				int read_count = m_pSkillLevels[dwSkillVnum].bBooks;
#else
				char flag[128+1];
				...
				if (number(1, 100) > percent)
				{
					...
					{
#ifdef ENABLE_SKILL_DESCRIPTION_RENEWAL
						m_pSkillLevels[dwSkillVnum].bBooks += 1;
#else
	...
	void operator () (LPENTITY ent)
	{
		...
		switch (m_pkSk->bSkillAttrType)
		{
			case SKILL_ATTR_TYPE_NORMAL:
					if (CPVPManager::instance().IsSpecialDuel(m_pkChr, pkChrVictim))
					{

						if (m_pkChr->GetQuestFlag("duel_type") == 1) //DUEL TACTIC
						{
							if (m_pkChr->GetJob() == JOB_ASSASSIN)
							{
								if (m_pkChr->GetSkillGroup() == 1) //LAME
								{
									switch (m_pkSk->dwVnum)
									{
										...
										case 32: //fulger
											iDam = 3100;
											break;
										case 35: //nor otravitor
											iDam = 2000;
											break;
										case 33: //nor otravitor
											iDam = 1700;
											break;
									}
								}
							}
							if (m_pkChr->GetJob() == JOB_SURA)
							{
								if (m_pkChr->GetSkillGroup() == 1) //ARME MAGICE
								{
									switch (m_pkSk->dwVnum)
									{
										case 62: //vartej
								...
								if (m_pkChr->GetSkillGroup() == 2) //BLACK MAGIC (BM)
								{
									switch (m_pkSk->dwVnum)
									{
										case 76: //Lovitura Intunecata
											iDam = 1400;
											break;
										case 77: //Lovitura Focului
											iDam = 1500;
											break;
										case 78: //Spiritul Flacarii
											iDam = 800;
											break;
										case 80: //Lovitura Spiritului
											iDam = 1400;
											break;
										case 81: //Sfera Intunecata
											iDam = 2200;
											break;
									}
								}
							}
							if (m_pkChr->GetJob() == JOB_WARRIOR)
							{
								if (m_pkChr->GetSkillGroup() == 1) //CORPORAL
								{
									switch (m_pkSk->dwVnum)
									{
										case 2: //varteju sabiei
											iDam = 2200;
											break;
										case 1: //incizia
											iDam = 800;
								...
								else if (m_pkChr->GetSkillGroup() == 2) //MENTAL
								{
									switch (m_pkSk->dwVnum)
									{
										case 16: //atacul spiritului
											iDam = 3800;
											break;
										case 20: //atacul sabiei
											iDam = 1800;
											break;
										case 18: //zdruncinare
											iDam = 2600;
							...
							if (m_pkChr->GetJob() == JOB_SHAMAN)
							{
								if (m_pkChr->GetSkillGroup() == 1) //Romanian Dragon
								{
									switch (m_pkSk->dwVnum)
									{
										case 91: //vartej
						...
						if (m_pkChr->GetQuestFlag("duel_type") == 2) //DUEL GERMAN
						{
							if (m_pkChr->GetJob() == JOB_ASSASSIN)
							{
								if (m_pkChr->GetSkillGroup() == 1) //LAME
							...
							if (m_pkChr->GetJob() == JOB_SHAMAN)
							{
								if (m_pkChr->GetSkillGroup() == 1) //Zmeu
								{
									switch (m_pkSk->dwVnum)
									{
										case 91:
								...
								// else if (m_pkChr->GetSkillGroup() == 2) //Vindecare
								// {
									// switch (m_pkSk->dwVnum)
									// {
...
struct FComputeSkillParty
{
	...
	{
#ifdef ENABLE_SKILL_COLOR_SYSTEM
		if (m_dwVnum == SKILL_HOSIN || m_dwVnum == SKILL_REFLECT || m_dwVnum == SKILL_GICHEON || m_dwVnum == SKILL_KWAESOK || m_dwVnum == SKILL_JEUNGRYEOK
#ifdef ENABLE_WOLFMAN_CHARACTER
			 || m_dwVnum == SKILL_CHEONGRANG 
#endif
			)
		{
			BYTE skill = 0;
...
#if defined(ENABLE_BUFF_ITEMS_SYSTEM) && defined(ENABLE_SKILL_FLAG_PARTY)
	if (pkVictim)
	{
		// Dragon Skills
		...
		// Heal Skills
		if (pkVictim != this && pkVictim->IsAffectFlag(AFF_MOV_SPEED_BUFF) && (dwVnum == SKILL_KWAESOK))
		{
			return false;
		}
		else if (this && IsAffectFlag(AFF_MOV_SPEED_BUFF) && (dwVnum == SKILL_KWAESOK) && (pkVictim == this))
		{
			return false;
		}

		if (pkVictim != this && pkVictim->IsAffectFlag(AFF_ATT_GRADE_BUFF) && (dwVnum == SKILL_JEUNGRYEOK))
		{
			return false;
		}
		else if (this && IsAffectFlag(AFF_ATT_GRADE_BUFF) && (dwVnum == SKILL_JEUNGRYEOK) && (pkVictim == this))
		{
			return false;
		}
	}
...
bool CHARACTER::UseSkill(DWORD dwVnum, LPCHARACTER pkVictim, bool bUseGrandMaster)
{
	...
	if (dwVnum == SKILL_HOSIN || dwVnum == SKILL_REFLECT || dwVnum == SKILL_GICHEON || dwVnum == SKILL_KWAESOK || dwVnum == SKILL_JEUNGRYEOK)
	{
		if (IsGM() == true && pkVictim->IsPC() == true)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("GM_BLOCK_BUFF_TEAM"));
			return false;
		}
		
		if (IsPC() == true && pkVictim->IsGM() == true)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("GM_BLOCK_BUFF"));
			return false;
		}
	}
#endif
	...
	SetProtectTime("emotion.restrict_time", get_global_time() + 10);
...
// #endif
	...
	if (pkVictim != NULL && (dwVnum == 94 || dwVnum == 95 || dwVnum == 96 || dwVnum == 110 || dwVnum == 111))
	{
		...
		DWORD data[ESkillColorLength::MAX_SKILL_COUNT + ESkillColorLength::MAX_BUFF_COUNT][ESkillColorLength::MAX_EFFECT_COUNT];
		thecore_memcpy(data, pkVictim->GetSkillColor(), sizeof(data));

		DWORD dataAttacker[ESkillColorLength::MAX_SKILL_COUNT + ESkillColorLength::MAX_BUFF_COUNT][ESkillColorLength::MAX_EFFECT_COUNT];
		thecore_memcpy(dataAttacker, this->GetSkillColor(), sizeof(dataAttacker));
...
BYTE CHARACTER::GetSkillGradeByLevel(BYTE bSkillLevel) const
{
	// sys_log(0, "bSkillLevel: %d", bSkillLevel);
#ifdef ENABLE_FORCE2MASTERSKILL
	if (bSkillLevel >= 17 && bSkillLevel < 30)
#else
	if (bSkillLevel >= 20 && bSkillLevel < 30)
#endif
	{
		return SKILL_MASTER;
	}
	else if (bSkillLevel >= 30 && bSkillLevel < 40)
	{
		return SKILL_GRAND_MASTER;
	}
	else if (bSkillLevel >= 40)
	{
...
#ifdef ENABLE_SKILL_DESCRIPTION_RENEWAL
	m_pSkillLevels[dwVnum].bBooks = 0;
#endif
...
#ifdef ENABLE_SKILL_FLAG_PARTY
bool CHARACTER::IsBuffSkill(DWORD dwSkillVnum) const
{
	switch (dwSkillVnum)
	{
	case SKILL_HOSIN:
