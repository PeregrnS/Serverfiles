...
ACMD(do_emotion)
{
	...
	if (ch->GetQuestFlag("duel_restricted") == 1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "You can't use emotions while in a duel.");
		return;
	}
	
	if (ch->GetProtectTime("emotion.restrict_time") >= get_global_time())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "You can't use emotions 10s after you used an skill!");
		return;
	}
