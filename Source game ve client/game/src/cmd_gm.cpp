...
ACMD(do_item_purge)
{
	...
	if (!strArg.compare(0, 3, "all"))
	{
		for (i = 0; i < INVENTORY_AND_EQUIP_SLOT_MAX; ++i)
		{
			if ((item = ch->GetInventoryItem(i)))
			{
				...
				{
					int affect_type = ch->GetAffectType(item);
	...
	for (i = 0; i < INVENTORY_AND_EQUIP_SLOT_MAX; ++i)
	{
		if ((item = ch->GetInventoryItem(i)))
		{
			...
			if (item->GetType() == ITEM_BLEND)
			{
				int affect_type = (item->GetValue(2) == 1) ? AFFECT_BLEND_EX : AFFECT_BLEND;
				int apply_type = aApplyInfo[item->GetValue(0)].bPointType;
				CAffect *pAffect = ch->FindAffect(affect_type, apply_type);
				if (item->GetSocket(1) == 1)
				{
...
ACMD(do_notice)
{
    ...
    // Erstelle einen Puffer fur die finale Nachricht
    char final_message[512];

    // Fuge den Spielernamen und die Nachricht zusammen
    snprintf(final_message, sizeof(final_message), "[%s]: %s", ch->GetName(), argument);

    // Sende die Nachricht als globale Notice
...
#ifdef ENABLE_GEM_SYSTEM
		case DoSetTypes::GEM: // gem
		{
			int gem = 0;
			str_to_number(gem, arg3);
			// LogManager::instance().MoneyLog(MONEY_LOG_MISC, 3, gem);
			tch->PointChange(POINT_GEM, gem, true);
		}
		break;
#endif

		case DoSetTypes::CASH: // cash
		{
			if (ch->GetGMLevel() < GM_IMPLEMENTOR)
			{
				DWORD cash = 0;
				str_to_number(cash, arg3);
				tch->SetCash(tch->GetCash() + cash);
			}
			else
				ch->ChatPacket(CHAT_TYPE_INFO, "You can set cash at a test server only.");
		}
		break;

		case DoSetTypes::COINS: // coins
		{
			if (ch->GetGMLevel() < GM_IMPLEMENTOR)
			{
				DWORD coins = 0;
				str_to_number(coins, arg3);
				tch->SetCoins(tch->GetCoins() + coins);
			}
			else
				ch->ChatPacket(CHAT_TYPE_INFO, "You can set coins at a test server only.");
		}
		break;
	}
...
#ifdef ENABLE_GOLD_MAX_EXTENDED
					if (ch->GetGold() < (unsigned long long)t->dwPrice)
#else
...
ACMD (do_item_full_set)
{
	...
	case JOB_SURA:
		{
			item = ITEM_MANAGER::instance().CreateItem(172879);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(172719);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(79099 );
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(79189 );
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(172749 );
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(172769 );
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(172809 );
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(172979 );
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
		}
		break;
	case JOB_WARRIOR:
		{
			item = ITEM_MANAGER::instance().CreateItem(172889);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(172759);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(79189 );
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(172969 );
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
		}
		break;
	case JOB_SHAMAN:
		{
			item = ITEM_MANAGER::instance().CreateItem(172909);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(172729);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(79189 );
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(172989 );
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
		}
		break;
	case JOB_ASSASSIN:
		{
			item = ITEM_MANAGER::instance().CreateItem(172899);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(172739);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(79189 );
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(172999 );
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
		}
		break;
	...
	case JOB_WOLFMAN:
		{
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(92419);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(92449);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(92459);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(92439);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(92469);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
		}
		break;
...
ACMD (do_attr_full_set)
{
	...
	case JOB_WOLFMAN:
#endif
		{
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute( 0, APPLY_HP_REGEN, 20);
				item->SetForceAttribute( 1, APPLY_POISON_PCT, 10);
				item->SetForceAttribute( 2, APPLY_POISON_PCT, 10);
				item->SetForceAttribute( 3, APPLY_DODGE, 20);
				item->SetForceAttribute( 4, APPLY_MAX_HP, 1500);
				item->SetForceAttribute( 5, APPLY_ATTBONUS_MONSTER, 20);
				item->SetForceAttribute( 6, APPLY_ATT_GRADE_BONUS, 100);
			}
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute( 0, APPLY_NORMAL_HIT_DAMAGE_BONUS, 50);
				item->SetForceAttribute( 1, APPLY_STR, 10);
				item->SetForceAttribute( 2, APPLY_INT, 10);
				item->SetForceAttribute( 3, APPLY_CRITICAL_PCT, 10);
				item->SetForceAttribute( 4, APPLY_STUN_PCT, 10);
				item->SetForceAttribute( 5, APPLY_ATTBONUS_MONSTER, 20);
				item->SetForceAttribute( 6, APPLY_ATT_GRADE_BONUS, 100);
			}
			...
			if (item != NULL)
			{
				...
				item->SetForceAttribute( 2, APPLY_INT, 10);
				item->SetForceAttribute( 3, APPLY_CON, 10);
				item->SetForceAttribute( 4, APPLY_REFLECT_MELEE, 15);
				item->SetForceAttribute( 5, APPLY_ATTBONUS_MONSTER, 20);
				item->SetForceAttribute( 6, APPLY_ATT_GRADE_BONUS, 100);
			}
			...
			if (item != NULL)
			{
				...
				item->SetForceAttribute( 3, APPLY_MAX_HP, 3500);
				item->SetForceAttribute( 4, APPLY_DEF_GRADE_BONUS, 100);
				item->SetForceAttribute( 5, APPLY_ATTBONUS_MONSTER, 20);
				item->SetForceAttribute( 6, APPLY_ATT_GRADE_BONUS, 100);
			}
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute( 0, APPLY_MAX_HP, 3500);
				item->SetForceAttribute( 1, APPLY_CRITICAL_PCT, 10);
				item->SetForceAttribute( 2, APPLY_STEAL_HP, 15);
				item->SetForceAttribute( 3, APPLY_MOV_SPEED, 15);
				item->SetForceAttribute( 4, APPLY_ATT_SPEED, 8);
				item->SetForceAttribute( 5, APPLY_ATTBONUS_MONSTER, 20);
				item->SetForceAttribute( 6, APPLY_ATT_GRADE_BONUS, 100);
			}
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute( 0, APPLY_STEAL_HP, 15);
				item->SetForceAttribute( 1, APPLY_PENETRATE_PCT, 10);
				item->SetForceAttribute( 2, APPLY_MAX_HP, 2000);
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute( 0, APPLY_CRITICAL_PCT, 10);
				item->SetForceAttribute( 1, APPLY_PENETRATE_PCT, 10);
				item->SetForceAttribute( 2, APPLY_HP_REGEN, 20);
				item->SetForceAttribute( 3, APPLY_MAX_HP, 2000);
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute( 0, APPLY_POISON_REDUCE, 30);
				item->SetForceAttribute( 1, APPLY_MOV_SPEED, 15);
				item->SetForceAttribute( 2, APPLY_MAX_HP, 1500);
				item->SetForceAttribute( 3, APPLY_CRITICAL_PCT, 10);
				item->SetForceAttribute( 4, APPLY_ATTBONUS_DEVIL, 10);
				item->SetForceAttribute( 5, APPLY_ATTBONUS_MONSTER, 20);
				item->SetForceAttribute( 6, APPLY_ATT_GRADE_BONUS, 100);
...
ACMD(do_maintenance)
{
	...
	{
		info.shutdown_time += get_global_time();
