...
bool g_isBlockedPlayer(std::string stName)
{
	return (quest::CQuestManager::instance().GetEventFlag(("block_" + stName).c_str()) > get_global_time());
}
...
#ifdef __HIT_LIMITER_ENABLE__
		TOKEN("HIT_LIMITER_ENABLE")
		{
			str_to_number(CHitLimiter::g_limiter_enable, value_string);
			continue;
		}

		TOKEN("HIT_LIMITER_BUFFER_VALUE")
		{
			str_to_number(CHitLimiter::HIT_LIMIT_BUFF, value_string);
			continue;
		}
#endif
	}
