...
bool Cube_make (LPCHARACTER ch)
{
	...
	if (gold < 0 || gold < static_cast<unsigned long long>(cube_proto->gold))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Not enough Yang or not enough space in the inventory."));
		return false;
	}
#else
	...
	{
#ifdef ENABLE_GOLD_MAX_EXTENDED
		ch->ChangeGold(-(unsigned long long)cube_proto->gold);
#else
