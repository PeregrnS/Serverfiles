...
void DBManager::SendAuthLogin(LPDESC d)
{
	...
	strlcpy(ptod.pin, r.pin, sizeof(ptod.pin));
#endif
	...
	ptod.dwCash = r.dwCash;
	ptod.dwCoins = r.dwCoins;
...
void DBManager::AnalyzeReturnQuery(SQLMsg * pMsg)
{
...
#ifdef ENABLE_PIN_LOGIN
					strlcpy(szPin, row[col++], sizeof(szPin));
					if (!row[col]) 
					{
						sys_err("error column %d", col); 
						M2_DELETE(pinfo);
...
#ifdef ENABLE_PIN_LOGIN
					else if (strcmp(szPin, pinfo->pin))
					{
...
#ifdef ENABLE_MAINTENANCE_STATUS
						bool bServerStatus = false;

						BYTE status_server = 0;
						std::unique_ptr<SQLMsg> _serverstatusnmsg(DBManager::instance().DirectQuery("SELECT status FROM account.server_status"));
						if (!_serverstatusnmsg->uiSQLErrno)
						{
							if (_serverstatusnmsg->Get()->uiNumRows)
							{
								while (MYSQL_ROW row_serverstatus = mysql_fetch_row(_serverstatusnmsg->Get()->pSQLResult))
								{
									str_to_number(status_server, row_serverstatus[0]);
									sys_log(0, "status_server %d", status_server);
									if (status_server == 0)
										bServerStatus = true;
								}
							}
...
#ifdef ENABLE_PIN_LOGIN
						strlcpy(r.pin, szPin, sizeof(r.pin));
#endif
						r.dwCash = dwCash;
						r.dwCoins = dwCoins;
	...
	void DBManager::SendMoneyLog(BYTE type, DWORD vnum, unsigned long long gold)
#else
