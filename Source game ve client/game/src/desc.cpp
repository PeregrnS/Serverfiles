...
int DESC::ProcessInput()
{
	...
	else if (!m_bEncrypted)
	{
		...
		while (!m_pInputProcessor->Process(this, buffer_read_peek(m_lpInputBuffer), buffer_size(m_lpInputBuffer), iBytesProceed))
		{
			int buf_size = buffer_size(m_lpInputBuffer);

			if (iBytesProceed <= 0 || iBytesProceed > buf_size)
			{
				BYTE header = 0;
				if (buf_size > 0)
					header = *(BYTE*)buffer_read_peek(m_lpInputBuffer);

				sys_err("INPUT-ERR(plain): desc %p phase %d ip %s header %u iBytesProceed %d buf_size %d",
					this, m_iPhase, GetHostName(), header, iBytesProceed, buf_size);
				break;
			}
		...
		if (iBytesProceed > 0 && iBytesProceed <= buffer_size(m_lpInputBuffer))
			buffer_read_proceed(m_lpInputBuffer, iBytesProceed);
	}
	else
	{
