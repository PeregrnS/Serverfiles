...
class DESC_MANAGER : public singleton<DESC_MANAGER>
{
	...
	public:
		void AddIPToWhitelist(const char* ip) { ipWhitelist.insert(ip); };
		bool IsIPWhitelisted(const char* ip) { return (ipWhitelist.find(ip) != ipWhitelist.end()); };
		bool HasMultipleHandshakes(const char* ip);
#endif
};
