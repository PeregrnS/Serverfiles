...
void CDungeon::DecMember(LPCHARACTER ch)
{
	...
	if (m_set_pkCharacter.empty())
	{
		...
		event_cancel(&deadEvent);
#ifdef ENABLE_DUNGEON_RE_ENETER
		deadEvent = event_create(dungeon_dead_event, info, PASSES_PER_SEC(360));
#else
...
LPCHARACTER CDungeon::FindAnyMember()
{
	if (m_set_pkCharacter.empty())
	{
		return NULL;
	...
	// Access the first (arbitrary) element using an iterator
	CHARACTER_SET::iterator it = m_set_pkCharacter.begin();
	LPCHARACTER first_element = *it;
...
void CDungeon::UniqueSetPoint(const std::string& key, uint16_t point, int32_t val)
{
	TUniqueMobMap::iterator it = m_map_UniqueMob.find(key);
	if (it == m_map_UniqueMob.end())
	{
	...
	if (point >= POINT_MAX_NUM)
	{
		sys_err("<dungeon> Point type overflow: %u", point);
		return;
	}

	it->second->PointChange(point, val - it->second->GetPoint(point));
}

int32_t CDungeon::UniqueGetPoint(const std::string& key, uint16_t point)
{
	TUniqueMobMap::iterator it = m_map_UniqueMob.find(key);
	if (it == m_map_UniqueMob.end())
	{
	...
	return it->second->GetPoint(point);
}
...
namespace
{
	struct FChangeBonus
	{
		FChangeBonus(uint16_t applyType, int32_t val) : m_applyType(applyType), m_val(val)
		{}
		...
		void operator() (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				{
					int delta = m_val;
					CAffect* pAffect = ch->FindAffect(AFFECT_UNIQUE_POINT_DUNGEON, m_applyType);

					if (pAffect)
						delta += pAffect->lApplyValue;

					ch->AddAffect(AFFECT_UNIQUE_POINT_DUNGEON, m_applyType, delta, AFF_NONE, 60 * 60 * 24, 0, true, true);
					ch->ComputePoints();
				}
			}
		}

		uint16_t m_applyType;
		int32_t m_val;
	};

	struct FRemoveBonus
	{
		FRemoveBonus(uint16_t applyType) : m_applyType(applyType)
		{}
		...
		void operator() (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (ch->IsPC())
				{
					...
					{
						ch->RemoveAffect(AFFECT_UNIQUE_POINT_DUNGEON);
					}
				}
			}
		}
...
// Used to modify the bonuses(APPLY_TYPE) of all the players in the dungeon.
void CDungeon::ChangeBonus(uint16_t applyType, int32_t val)
{
	sys_log(0, "<DUNGEON> add bonus ptr %p apply %u, val %d", this, applyType, val);
	LPSECTREE_MAP pMap = SECTREE_MANAGER::instance().GetMap(m_lMapIndex);

	if (!pMap)
	{
		sys_err("<DUNGEON> cannot find map by index %d", m_lMapIndex);
		return;
	}

	if (applyType >= MAX_APPLY_NUM || applyType < 1)
	{
		sys_err("apply is out of range : %d", applyType);
		return;
	}

	FChangeBonus f(aApplyInfo[applyType].bPointType, val);
	pMap->for_each(f);
}

void CDungeon::RemoveBonus(uint16_t applyType)
{
	sys_log(0, "<DUNGEON> remove bonus ptr %p apply %u", this, applyType);
	LPSECTREE_MAP pMap = SECTREE_MANAGER::instance().GetMap(m_lMapIndex);
	...
	if (applyType >= MAX_APPLY_NUM || applyType < 0)
	{
		sys_err("apply is out of range : %d", applyType);
		return;
	}

	// if applyType = 0 it will remove the entire affect
	FRemoveBonus f(aApplyInfo[applyType].bPointType);
	pMap->for_each(f);
}
#endif
...
namespace
{
	...
	struct FDecreaseHPByPercentage
	{
		FDecreaseHPByPercentage(const int pct) : iPct(pct) {}
		...
		void operator() (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (ch->IsPC() && !ch->IsDead())
					ch->DecreaseHPByPercentage(iPct);
			}
		}
...
#ifdef ENABLE_D_DECREASE_HP_BY_PCT
void CDungeon::DecreaseHPToAll(int iPercent)
{
	const auto pkMap = SECTREE_MANAGER::instance().GetMap(m_lMapIndex);
	if (!pkMap) {
		sys_err("CDungeon: SECTREE_MAP not found for #%ld", m_lMapIndex);
		return;
	}

	FDecreaseHPByPercentage f(iPercent);
	pkMap->for_each(f);
}
#endif
