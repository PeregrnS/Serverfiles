...
class CDungeon
{
	...
	void	DecreaseHPToAll(int iPercent);
#endif
	...
	void	UniqueSetPoint(const std::string& key, uint16_t point, int32_t val);
	int32_t	UniqueGetPoint(const std::string& key, uint16_t point);
	void	ChangeBonus(uint16_t applyType, int32_t val);
	void	RemoveBonus(uint16_t applyType);
#endif
