...
void exchange_packet(LPCHARACTER ch, BYTE sub_header, bool is_me
#ifdef ENABLE_GOLD_MAX_EXTENDED
	, unsigned long long arg1
#else
	, DWORD arg1
#endif
	, TItemPos arg2, DWORD arg3, void * pvData = NULL);
...
void exchange_packet(LPCHARACTER ch, BYTE sub_header, bool is_me
	...
	, DWORD arg1
#endif
	, TItemPos arg2, DWORD arg3, void* pvData)
{
	if (!ch->GetDesc())
		return;
...
bool CHARACTER::ExchangeStart(LPCHARACTER victim)
{
	...
	if ((!check_allow(GetGMLevel(), GM_ALLOW_EXCHANGE_TO_GM) && victim->IsGM()) ||
		(!check_allow(GetGMLevel(), GM_ALLOW_EXCHANGE_TO_PLAYER) && !victim->IsGM()))
	{
		if (!check_allow(GetGMLevel(), GM_ALLOW_EXCHANGE_TO_GM) && victim->IsGM())
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You are not allowed to trade to gamemasters."));
		}
		else if (!check_allow(GetGMLevel(), GM_ALLOW_EXCHANGE_TO_PLAYER) && !victim->IsGM())
		{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("GM_BLOCK_EXCHANGE"));
		return false;
	}
	
	if (IsGM() == true && GetGMLevel() < GM_IMPLEMENTOR && victim->IsGM() == false)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("GM_BLOCK_EXCHANGE_TEAM"));
		return false;
	}
#endif
	...
	if (g_isBlockedPlayer(victim->GetName()))
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("VICTIM_HAS_BLOCK_STATUS"));
		return false;
	}
	// END_OF_BLOCK_PLAYER
...
)
{
	...
	if (pkVictim->IsGM() && !check_allow(m_pOwner->GetGMLevel(), GM_ALLOW_EXCHANGE_PLAYER_ITEM_TO_GM))
	{
		m_pOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You may not exchange a player owned item to a gm."));
		return false;
	}
	else if (!pkVictim->IsGM() && !check_allow(m_pOwner->GetGMLevel(), GM_ALLOW_EXCHANGE_PLAYER_ITEM_TO_PLAYER))
	{
...
bool CExchange::Done(DWORD tradeID)
#else
bool CExchange::Done()
#endif
{
	...
	for (i = 0; i < EXCHANGE_ITEM_MAX_NUM; ++i)
	{
		...
		{
#ifdef ENABLE_TRADE_LOG
			LogManager::instance().ExchangeItemLog(tradeID, item, m_pOwner->GetName(), victim->GetName());
#else
...
bool CExchange::Accept(bool bAccept)
{
	...
	if (m_bAccept && GetCompany()->m_bAccept)
	{
		...
		{
			tradeID = LogManager::instance().ExchangeLog(m_pOwner->GetName(), victim->GetName(), GetOwner()->GetX(), GetOwner()->GetY(), m_lGold, GetCompany()->m_lGold);
		}
#endif
