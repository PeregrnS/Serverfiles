		...
		switch (ret)
		{
			...
			case 0:
				if (item_vnum)
				{
					...
					{
#ifdef __DAILY_QUESTS__
						CDailyQuestManager::instance().Progress(ch, item_vnum, DAILY_TYPE_FISH, true);
#endif
						
#ifdef ENABLE_RANK_PLAYER
						RankPlayer::instance().SendInfoPlayer(ch, RANK_BY_FISH_CAUGHT, RankPlayer::instance().GetProgressByPID(ch->GetPlayerID(), RANK_BY_FISH_CAUGHT) + 1);
#endif
#ifdef __RANKING_SYSTEM__
						RankPlayer::instance().SendInfoPlayer(ch, RANK_BY_FISH_CAUGHT, RankPlayer::instance().GetProgressByPID(ch->GetPlayerID(), RANK_BY_FISH_CAUGHT) + 1);
#endif
