...
#endif
{
	...
	m_data.grade_array[0].auth_flag = GUILD_AUTH_ADD_MEMBER | GUILD_AUTH_REMOVE_MEMBER | GUILD_AUTH_NOTICE | GUILD_AUTH_USE_SKILL
	 ...
	 | GUILD_AUTH_GS_CHECKIN | GUILD_AUTH_GS_CHECKOUT | GUILD_AUTH_GS_GOLDIN | GUILD_AUTH_GS_GOLDOUT
...
bool CGuild::OfferExp(LPCHARACTER ch, int amount)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±?μ?> A|°?C?°i?U C?´A °?C?Aˇ°ˇ ł˛?? °?C?Aˇ?¸´? ¸?˝?´?´?."));
		return false;
	}
...
bool CGuild::ChargeSP(LPCHARACTER ch, int iSP)
{
	...
	unsigned long long gold = iSP * 100;
#else
	int gold = iSP * 100;
#endif
...
int CGuild::GetMaxMemberCount()
{
	...
	return 462 + 2 * (m_data.level-1) + m_iMemberCountBonus;
}
...
void CGuild::RequestGuildMeeting(LPCHARACTER ch)
{
	LPDESC d;
	if (!(d=ch->GetDesc()))
		return;
	...
	for (auto it = m_memberOnline.begin(); it != m_memberOnline.end(); it++)
	{
		...
		if (tch != ch)
		{
			tch->ChatPacket(CHAT_TYPE_COMMAND, "guild_meeting");
		}
	}
}
#endif
...
void CGuild::SendTeleportGuild(DWORD dwLeaderPID, long lX, long lY, long lMapIndex, long wPort)
{
	TGuildMemberOnlineContainer::iterator it;
	...
	{
		LPCHARACTER ch = *it;
		if (!ch || ch->GetPlayerID() == dwLeaderPID)
			continue;
		
		ch->SetQuestFlag("guild_warp_x", lX);
		ch->SetQuestFlag("guild_warp_y", lY);
		ch->SetQuestFlag("guild_warp_map", lMapIndex);
		ch->SetQuestFlag("guild_warp_port", wPort);

		ch->ChatPacket(CHAT_TYPE_COMMAND, "TeleportGuild");
	}
}
#endif
