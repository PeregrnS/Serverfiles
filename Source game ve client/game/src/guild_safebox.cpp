...
void CGuildSafeBox::CheckInItem(LPCHARACTER ch, LPITEM pkItem, int iDestCell)
{
	...
	if (quest::PC* pPC = quest::CQuestManager::instance().GetPC(ch->GetPlayerID()))
	{
		if (pPC->IsRunning() && ch->GetQuestItemPtr() == pkItem)
		{
			sys_err("!! cannot add item to guild safebox %u %s while quest running with this item player %u %s", pkItem->GetID(), pkItem->GetName(), ch->GetPlayerID(), ch->GetName());
			return;
		}
	}
...
void CGuildSafeBox::GiveGold(LPCHARACTER ch, DWORD dwGold)
{
	...
	if (m_dwGold + dwGold >= GOLD_MAX)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You cannot safe any more gold."));
			return;
		}
...
void CGuildSafeBox::TakeGold(LPCHARACTER ch, DWORD dwGold)
{
	...
	if (dwGold && ch->GetGold() + dwGold >= GOLD_MAX)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You cannot take any more gold."));
			return;
		}
