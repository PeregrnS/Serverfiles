...
void CInputLogin::CharacterCreate(LPDESC d, const char * data)
{
	...
	if (!check_account_allow(d->GetAccountTable().login, GM_ALLOW_CREATE_PLAYER))
	{
		sys_err("GM Character creation disabled. account_id: %d, login: %s", d->GetAccountTable().id, pinfo->name);
		/*d->Packet(&packFailure, sizeof(packFailure));
		return;*/
	}
...
void CInputLogin::CharacterDelete(LPDESC d, const char * data)
{
	...
	if (!check_allow(gm_get_level(c_rAccountTable.players[pinfo->index].szName, NULL, c_rAccountTable.login), GM_ALLOW_DELETE_PLAYER))
	{
		sys_err("PlayerDelete: cannot delete gm character [%s]", c_rAccountTable.players[pinfo->index].szName);
		// strlcpy(failurePacket.szStatus, "GM", sizeof(failurePacket.szStatus));
...
void CInputLogin::Entergame(LPDESC d, const char * data)
{
	...
	{
		// Wir prufen ein "Memory-Flag". Dieses existiert nur, solange der Char eingeloggt ist.
		// Beim Porten bleibt dieser Wert erhalten, beim Logout wird er geloscht.
		if (ch->GetQuestFlag("gm_notice.sent") == 0)
		{
			char notice[256];
			const char* color = "|cffFFFF00"; // Gelb

			if (ch->GetGMLevel() == GM_IMPLEMENTOR) color = "|cffFF0000"; // SA Rot
			else if (ch->GetGMLevel() == GM_GOD) color = "|cff00FF00"; // GA Grun
			else if (ch->GetGMLevel() == GM_HIGH_WIZARD) color = "|cff00FFFF"; // SGM Blau

			snprintf(notice, sizeof(notice), "%s[Team-Info] %s ist nun online!|r", color, ch->GetName());
			BroadcastNotice(notice);

			// Wir setzen das Flag auf 1. Es bleibt beim Porten auf 1.
			ch->SetQuestFlag("gm_notice.sent", 1);
		}
	}
#endif
	...
	ch->UpdateCostumeVisibilityStatus();
#endif
	ch->LoadBlockMode();
...
#ifdef ENABLE_EVENT_MANAGER
	if (ch->GetMapIndex() == KINGDOM_WAR_MAP_INDEX && CEventManager::Instance().GetEventState(EVENT_TYPE_KINGDOM_WAR))
	{
	...
	if (ch->IsPrivateShopOwner())
	{
		if (ch->GetPrivateShopTable()->llGold > 0 || ch->GetPrivateShopTable()->dwCheque > 0)
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You have %llu Yangs and %u Wons waiting in your personal shop stash."),
				ch->GetPrivateShopTable()->llGold, ch->GetPrivateShopTable()->dwCheque);
#else
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You have %llu Yangs waiting in your personal shop stash."),
				ch->GetPrivateShopTable()->llGold);
#endif
		}
	}
#endif
...
#ifdef BLOCK_PVP_ON_MAP_INDEX
	if (ch->GetMapIndex() == 65 || ch->GetMapIndex() == 35)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("No damage can be generated through PvP on this map."));
#endif

#ifdef BLOCK_ITEMS_ON_MAP
	int map_index = ch->GetMapIndex();

	// Blocke Items auf Map 110 und 111 sowie alle Instanzen (1100000+, 1110000+)
	if (map_index == 110 || map_index == 111 || map_index / 10000 == 110 || map_index / 10000 == 111)
	{
		// Ausziehen verbotener Slots
		auto* cTheRing1 = ch->GetWear(WEAR_UNIQUE1);
		if (cTheRing1 && cTheRing1->IsEquipped())
			ch->UnequipItem(cTheRing1);

		auto* cTheRing2 = ch->GetWear(WEAR_UNIQUE2);
		if (cTheRing2 && cTheRing2->IsEquipped())
			ch->UnequipItem(cTheRing2);

		auto* cThePet = ch->GetWear(WEAR_PET);
		if (cThePet && cThePet->IsEquipped())
			ch->UnequipItem(cThePet);

		auto* cTheTali = ch->GetWear(WEAR_PENDANT);
		if (cTheTali && cTheTali->IsEquipped())
			ch->UnequipItem(cTheTali);

		auto* cTheBelt = ch->GetWear(WEAR_BELT);
		if (cTheBelt && cTheBelt->IsEquipped())
			ch->UnequipItem(cTheBelt);

		// Mount entfernen
		ch->ClearMountItem();
		ch->HorseSummon(false);

		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("The use of the following items is blocked on this map: Rings, Mounts, Pets, Belts, Talismans, Shashes, Use/Blend items."));
	}
#endif
	...
	CPVPManager::instance().ResetRealBonuses(ch);

#ifdef __DAILY_QUESTS__
	CDailyQuestManager::instance().EnterGame(ch);
#endif
}
