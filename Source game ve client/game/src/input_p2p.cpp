...
#if defined(__BL_MULTI_LANGUAGE_PREMIUM__) || defined(__PREMIUM_PRIVATE_SHOP__)
...
void CInputP2P::Switchbot(LPDESC d, const char *c_pData)
{
	...
	{
		sys_log(0, "CInputP2P::Switchbot:: wPort NULL");
		return;
	}
...
void CInputP2P::PrivateShopItemSearch(const char* c_pData)
{
	...
	TEMP_BUFFER buf;
	CPrivateShopManager::Instance().SearchItem(nullptr, p->Filter, p->bUseFilter, &buf);
...
#ifdef ENABLE_RANK_PLAYER
void CInputP2P::PlayerRank(LPDESC d, const char* c_pData)
{
	if (!d) return;
	
	const TPacketGGPlayerRank* p = reinterpret_cast<const TPacketGGPlayerRank*>(c_pData);
	
	RankPlayer::instance().ReceiveP2P(p->iMode, p->dwPid, p->szName, p->bEmpire, p->lValue);
}
#endif

#ifdef __RANKING_SYSTEM__
int CInputP2P::PlayerRank(LPDESC d, const char* c_pData, size_t uiBytes)
{
	TPacketGGPlayerRank* p = (TPacketGGPlayerRank*)c_pData;
	
	if (uiBytes < sizeof(TPacketGGPlayerRank) + p->lSize)
		return -1;
	...
	{
		const TPacketGGPlayerRank* p = reinterpret_cast<const TPacketGGPlayerRank*>(c_pData);
		RankPlayer::instance().ReceiveP2P(p->iMode, p->dwPid, p->szAccount, p->szName, p->bEmpire, p->lValue);
		return (p->lSize);
	}
	
	if (0 != (p->lSize % sizeof(TPacketGGUpdateRanking)))
	{
		sys_err("invalid packet size PlayerRanking %d", p->lSize);
		return -1;
	}
	
	int cur=0;
	int iCount = p->lSize / sizeof(TPacketGGUpdateRanking);
	
	while (cur < iCount)
	{
		TPacketGGUpdateRanking* pRank = (TPacketGGUpdateRanking*) (c_pData + sizeof(TPacketGGPlayerRank) + (sizeof(TPacketGGUpdateRanking)*cur));
		
		RankPlayer::instance().ReceiveP2P(pRank->iMode, pRank->dwPid, pRank->szAccount, pRank->szName, pRank->bEmpire, pRank->lValue);
		cur++;
	}
...
int CInputP2P::Analyze(LPDESC d, BYTE bHeader, const char * c_pData)
{
...
#ifdef ENABLE_RANK_PLAYER
		case HEADER_GG_PLAYER_RANK:
			PlayerRank(d, c_pData);
			break;	
#endif

#ifdef __RANKING_SYSTEM__
	case HEADER_GG_PLAYER_RANK:
		if ((iExtraLen = PlayerRank(d, c_pData, m_iBufferLeft)) < 0)
			return -1;
		break;
#endif
	...
	case HEADER_GG_REWARD_INFO:
		...
		case HEADER_GG_GUILD_TELEPORT:
			GuildTeleport(d, c_pData);
			break;
#endif
...
#ifdef ENABLE_TELEPORT_GUILD
void CInputP2P::GuildTeleport(LPDESC d, const char * c_pData)
{
	TPacketGGGuildTeleport * p = (TPacketGGGuildTeleport *) c_pData;

	CGuild * g = CGuildManager::instance().FindGuild(p->dwGuild);
	if (!g)
		return;

	g->SendTeleportGuild(p->dwLeaderPID, p->lX, p->lY, p->lMapIndex, p->wPort);
}
#endif

#ifdef ENABLE_TELEPORT_PM
void CInputP2P::RequestWarp(const char* c_pData)
{
	TPacketGGRequestWarp* p = (TPacketGGRequestWarp*)c_pData;
	
	LPCHARACTER tch = CHARACTER_MANAGER::Instance().FindPC(p->szName);
	if (!tch)
		return;
	...
	tch->SetCanRespondRequestWarp(p->szNameRequest); // Set Waiting respond from transfer-char
	tch->ChatPacket(CHAT_TYPE_COMMAND, "requestWarpCharacter %s", p->szNameRequest);
}
#endif
