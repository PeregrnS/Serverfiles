...
static const int ITEM_TIME_EXTENDER_VNUMS[] = {173010, 173011, 173012, 173013, 173014};
#endif
...
#endif
#ifdef ENABLE_ATTR_CHANGE_IMPROVE
	,
...
LPITEM CItem::RemoveFromCharacter()
{
	if (!m_pOwner)
	{
		sys_err("Item::RemoveFromCharacter owner null item vnum: %u ", this->GetVnum());
		return (this);
	}
	...
	if (m_pOwner && GetSocket(1) != 0 && GetType() == ITEM_USE && GetSubType() == USE_TITLE)
	{
		m_pOwner->SetProtectTime("title_index", 0);
	}

#ifdef __SKILL_COSTUME__
	if (m_pOwner && GetSocket(1) != 0 && GetType() == ITEM_SKILL_COSTUME)
	{
		m_pOwner->UseSkillCostumeItem(this, false);
	}
#endif
...
int CItem::FindEquipCell(LPCHARACTER ch, int iCandidateCell)
{
	if ((0 == GetWearFlag() || ITEM_TOTEM == GetType()) && ITEM_COSTUME != GetType() && ITEM_DS != GetType() && ITEM_SPECIAL_DS != GetType() && ITEM_RING != GetType() && ITEM_BELT != GetType()
		...
		&& !(GetType() == ITEM_ARMOR && GetSubType() == ARMOR_PENDANT)
#endif
#ifdef ENABLE_GLOVE_SYSTEM
		&& !(GetType() == ITEM_ARMOR && GetSubType() == ARMOR_GLOVE)
#endif
#ifdef ENABLE_COSTUME_PET
		...
		&& !(GetType() == ITEM_SHINING)
#endif
#ifdef ENABLE_BOOSTER_ITEMS
		&& !(GetType() == ITEM_BOOSTER)
#endif
	)
		return -1;
	...
	{
		if (GetSubType() == BOOSTER_WEAPON)
		{
			return WEAR_BOOSTER_WEAPON;
		}
		else if (GetSubType() == BOOSTER_ARMOR)
		{
			return WEAR_BOOSTER_ARMOR;
		}
		else if (GetSubType() == BOOSTER_HAIR)
		{
			return WEAR_BOOSTER_HAIR;
		}
	}
#endif
...
void CItem::ModifyPoints(bool bAdd)
{
	...
	if (IsItemNewStoneEquipment())
	{
		DWORD dwStone = GetSocket(0);
		if (dwStone)
		{
			TItemTable * p = ITEM_MANAGER::instance().GetTable(dwStone);
			if (p)
			{
	...
	else if (!IsAccessoryForSocket())
	{
#ifdef ENABLE_QUIVER_SYSTEM
		if ((m_pProto->bType == ITEM_WEAPON || m_pProto->bType == ITEM_ARMOR)
		 && m_pProto->bSubType != WEAPON_QUIVER && !IsRealTimeItem())
#else
		if (m_pProto->bType == ITEM_WEAPON || m_pProto->bType == ITEM_ARMOR)
#endif
		{
	...
	else
	{
		...
		{
#ifdef ENABLE_MOUNT_SYSTEM
			// Mount items has handled in different way
			if (GetType() == ITEM_MOUNT)
			{
				continue;
			}
...
bool CItem::IsItemNewStoneEquipment()
{
	if (GetWearFlag() & WEARABLE_SHIELD)
		return true;

	if (GetWearFlag() & WEARABLE_EAR)
		return true;
	
	if (GetWearFlag() & WEARABLE_WRIST)
		return true;
	
	if (GetWearFlag() & WEARABLE_FOOTS)
		return true;
	
	if (GetWearFlag() & WEARABLE_HEAD)
		return true;
	
	if (GetWearFlag() & WEARABLE_NECK)
		return true;
...
#if defined(ENABLE_MOUNT_SYSTEM)
bool CItem::IsMountItem()
{
	if (GetType() == ITEM_MOUNT && GetSubType() == MOUNT_SUBTYPE_SEAL)
	{
		return true;
	}
...
void CItem::PredestructionEvent()
{
	if (m_pOwner && (m_pOwner->GetMountVnum() && !m_pOwner->IsNormalHorse(m_pOwner->GetMountVnum())) && m_pOwner->GetMountItem() == this)
	{
		m_pOwner->ClearMountItem();
		m_pOwner->RefreshMount();
	}
}
#endif
...
int CItem::GetSpecialInventoryType()
{
	...
	switch (GetType())
	{
		case ITEM_COSTUME:
			{
				if (GetSubType() == COSTUME_BODY || GetSubType() == COSTUME_HAIR/* || GetSubType() == COSTUME_MOUNT */|| GetSubType() == COSTUME_WEAPON)
		...
		case ITEM_SHINING:
			{
				if (GetSubType() == SHINING_WEAPON || GetSubType() == SHINING_ARMOR || GetSubType() == SHINING_SPECIAL)
					return SPECIAL_INVENTORY_TYPE_SKILLBOOK;
				else
					return -1;
			}
...
#ifdef ENABLE_SPECIAL_INVENTORY
int CItem::GetSpecialInventoryTypeS(DWORD dwVnum, int iType, int subtype)
{
	switch (dwVnum)
	{
		case 27987:
	...
	switch (iType)
	{
		case ITEM_COSTUME:
		{
			if (subtype == COSTUME_BODY || subtype == COSTUME_HAIR /* || subtype == COSTUME_MOUNT */ || subtype == COSTUME_WEAPON)
				return SPECIAL_INVENTORY_TYPE_SKILLBOOK;
			else
				return -1;
		}
		...
		case ITEM_SHINING:
		{
			if (subtype == SHINING_WEAPON || subtype == SHINING_ARMOR || subtype == SHINING_SPECIAL)
				return SPECIAL_INVENTORY_TYPE_SKILLBOOK;
			else
				return -1;
		}
...
bool CItem::IsFishPotion()
{
	switch (GetVnum())
	{
	// default vnums
	case 27866: case 27868: case 27870:
	case 27873: case 27879: case 27880:
	case 27881: case 27883: case 27884:
	case 27885: case 27886: case 27887:
	// new vnums
	case 19071: case 19072: case 19073:
	case 19074: case 19075: case 19076:
	case 19077: case 19078: case 19079:
	case 19080:
		return true;
	}
...
#ifdef ENABLE_ITEM_TIME_EXTENDER
bool CItem::IsItemTimeExtender()
{
	for (unsigned int i = 0; i < sizeof(ITEM_TIME_EXTENDER_VNUMS) / sizeof(*ITEM_TIME_EXTENDER_VNUMS); i++)
		if ((unsigned int)ITEM_TIME_EXTENDER_VNUMS[i] == this->GetVnum())
			return true;
	return false;
}
#endif
