...
bool ITEM_MANAGER::GetDropPct(LPCHARACTER pkChr, LPCHARACTER pkKiller, OUT int& iDeltaPercent, OUT int& iRandRange)
{
	...
	// *** Levelunterschied KOMPLETT ignorieren ***
	// vorher: PERCENT_LVDELTA(...) / PERCENT_LVDELTA_BOSS(...)
	iDeltaPercent = 100;

	// kleine Random-Boni wie gehabt
	if (1 == number(1, 50000))
		iDeltaPercent += 1000;
	...
	int extraDrop = CPrivManager::instance().GetPriv(pkKiller, PRIV_ITEM_DROP)
...
{
	// 1. Sicherheits-Check: Existieren beide Pointer?
	if (!pkChr || !pkKiller)
		return false;

	// 2. Deine neue Map- + Level-Sperre
	// Ersetze MAP_INDEX durch deine ID (z.B. 1 für Map1)
	if (pkKiller->GetMapIndex() == 117 && pkKiller->GetLevel() >= 51)
	{
		return false; // Beendet die Funktion sofort -> kein Drop
	}
...
#ifdef __ENABLE_BIOLOG_SYSTEM__
	{
		...
		if (pkChr->IsMonster() && pkKiller)
		{
			...
			if (pkBiologManager && pkBiologManager->GetBiologItemByMobVnum(pkKiller, pkChr->GetRaceNum(), itemVnum, chance))
			{
				if (random <= chance)
				{
...
bool ITEM_MANAGER::CreateDropItemInfo(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<TDropInfoItem> &vec_item)
{
	...
	std::transform(map_item.begin(), map_item.end(), std::back_inserter(vec_item), [&](auto & kv)
	{ return kv.second; });
...
CommonWikiData::TWikiInfoTable *ITEM_MANAGER::GetItemWikiInfo(DWORD vnum)
{
	...
	if ((tbl->bType == ITEM_WEAPON || tbl->bType == ITEM_ARMOR || tbl->bType == ITEM_BELT || tbl->bType == ITEM_UNIQUE || tbl->bType == ITEM_METIN) && vnum % 10 == 0 && tbl->dwRefinedVnum)
	{
		newTable->refine_infos_count = GetWikiMaxRefineLevel(vnum);
	}
	else if (tbl->bType == ITEM_GIFTBOX/*  || tbl->bType == ITEM_GACHA */|| (tbl->bType == ITEM_USE && tbl->bSubType == USE_SPECIAL))
	{
...
std::vector<CSpecialItemGroup::CSpecialItemInfo> ITEM_MANAGER::GetWikiChestInfo(DWORD vnum)
{
	...
	if (tbl->bType == ITEM_GIFTBOX/* || tbl->bType == ITEM_GACHA */|| (tbl->bType == ITEM_USE && tbl->bSubType == USE_SPECIAL))
	{
		CSpecialItemGroup *ptr = nullptr;
