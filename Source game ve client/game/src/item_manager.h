...
class ITEM_MANAGER : public singleton<ITEM_MANAGER>
{
	public:
		...
		bool			ReadDropItemGroup(const char * c_pszFileName); // Selbe Zeile wie unten beim #else
		...
		void SaveItemStone(DWORD dwID, DWORD dwStone);
		bool GetItemStone(DWORD dwID, DWORD &dwStone);
		void RemoveItemStoneCache(DWORD dwID, bool bRemoveFile = false);

		std::map<DWORD, DWORD> m_mapCacheStoneInfo;
