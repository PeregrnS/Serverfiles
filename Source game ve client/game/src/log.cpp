...
void LogManager::ItemLog(LPCHARACTER ch, int itemID, int itemVnum, const char * c_pszText, const char * c_pszHint)
{
	...
	void LogManager::CharLog(DWORD dwPID, DWORD x, DWORD y, unsigned long long dwValue, const char *c_pszText, const char *c_pszHint, const char *c_pszIP)
#else
...
#ifdef ENABLE_GOLD_MAX_EXTENDED
	Query("INSERT INTO log%s (type, time, who, x, y, what, how, hint, ip) VALUES('CHARACTER', NOW(), %u, %u, %u, %llu, '%s', '%s', '%s')",
#else
...
#ifdef ENABLE_GOLD_MAX_EXTENDED
	void LogManager::CharLog(LPCHARACTER ch, unsigned long long dw, const char *c_pszText, const char *c_pszHint)
#else
...
#ifdef ENABLE_GOLD_MAX_EXTENDED
	void LogManager::MoneyLog(BYTE type, DWORD vnum, unsigned long long gold)
#else
...
#ifdef ENABLE_GOLD_MAX_EXTENDED
	Query("INSERT INTO money_log%s VALUES (NOW(), %d, %d, %llu)", get_table_postfix(), type, vnum, gold);
#else
...
DWORD LogManager::ExchangeLog(const char* dwPID1, const char* dwPID2, long x, long y, long long goldA, long long goldB)
{
	char szQuery[512];
	snprintf(szQuery, sizeof(szQuery), "INSERT INTO log.exchanges (playerA, playerB, goldA, goldB, x, y, date) VALUES ('%s','%s', %lld, %lld, %ld, %ld, NOW())", dwPID1, dwPID2, goldA, goldB, x, y);
...
void LogManager::ExchangeItemLog(DWORD tradeID, LPITEM item, const char * fromPlayer, const char * toPlayer)
{
	if (!tradeID)
	{
	...
	Query("INSERT INTO exchange_items"
		"(trade_id, `fromPlayer`, `toPlayer`, `item_id`, `vnum`, count,"
		...
		"%d,%d,%d,%d,"
		"NOW())"
		, tradeID, fromPlayer, toPlayer, item->GetID(), item->GetVnum(), item->GetCount()
		, item->GetSocket(0), item->GetSocket(1), item->GetSocket(2)
		, item->GetAttributeType(0), item->GetAttributeValue(0), item->GetAttributeType(1), item->GetAttributeValue(1), item->GetAttributeType(2), item->GetAttributeValue(2)
		, item->GetAttributeType(3), item->GetAttributeValue(3), item->GetAttributeType(4), item->GetAttributeValue(4)
		, item->GetAttributeType(5), item->GetAttributeValue(5), item->GetAttributeType(6), item->GetAttributeValue(6)
	);
}
#endif

void LogManager::CashLog(DWORD pid, const char *player_name, DWORD cash)
{
	LOG_LEVEL_CHECK_N_RET(LOG_LEVEL_MIN);
	Query("INSERT INTO cash_log%s (pid, player_name, cash) "
		  "VALUES(%u, '%s', %u)",
		  get_table_postfix(),
		  pid, player_name, cash);
}
