...
class LogManager : public singleton<LogManager>
{
	public:
		...
		void		CharLog(DWORD dwPID, DWORD x, DWORD y, unsigned long long dw, const char *c_pszText, const char *c_pszHint, const char *c_pszIP);
		void		CharLog(LPCHARACTER ch, unsigned long long dw, const char *c_pszText, const char *c_pszHint);
#else
...
#ifdef ENABLE_GOLD_MAX_EXTENDED
		void		MoneyLog(BYTE type, DWORD vnum, unsigned long long gold);
#else
		...
		void		CShieldLog(LPCHARACTER ch, const DWORD errorcode, const bool kickPlayer = true);
#endif
		...
		DWORD		ExchangeLog(const char * dwPID1, const char * dwPID2, long x, long y, long long goldA = 0, long long goldB = 0);
		void		ExchangeItemLog(DWORD tradeID, LPITEM item, const char * fromPlayer, const char * toPlayer);
#endif
...
#endif
		void		CashLog(DWORD pid, const char *player_name, DWORD cash);
