...
namespace mining
{
...
#ifdef __DAILY_QUESTS__
		CDailyQuestManager::instance().Progress(ch, item->GetVnum(), DAILY_TYPE_MINE, true);
#endif
	}
...
#ifdef ENABLE_RANK_PLAYER
				RankPlayer::instance().SendInfoPlayer(ch, RANK_BY_EXTRACTION, RankPlayer::instance().GetProgressByPID(ch->GetPlayerID(), RANK_BY_EXTRACTION) + 1);
#endif
#ifdef __RANKING_SYSTEM__
				RankPlayer::instance().SendInfoPlayer(ch, RANK_BY_EXTRACTION, RankPlayer::instance().GetProgressByPID(ch->GetPlayerID(), RANK_BY_EXTRACTION) + 1);
#endif
	...
	bool OreRefine(LPCHARACTER ch, LPCHARACTER npc, LPITEM item, int cost, int pct, LPITEM metinstone_item)
	{
		...
		if (ch->GetGold() < (unsigned long long)iCost)
#else
		if (ch->GetGold() < iCost)
#endif
			return false;
