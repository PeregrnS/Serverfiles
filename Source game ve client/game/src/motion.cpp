...
#include "CsvReader.h"
typedef std::map <uint32_t, std::string> TMapNpcList;
TMapNpcList	NpcList;

const char* locale_npc_list_find(uint32_t vnum)
{
	TMapNpcList::const_iterator iter = NpcList.find(vnum);
...
bool CMotionManager::ReadNpcList(const char* c_pszFileName)
{
	cCsvTable nameData;

	if (!nameData.Load(c_pszFileName, '\t'))
		return false;
	...
	while (nameData.Next())
	{
		...
		NpcList.insert(std::make_pair(atoi(nameData.AsStringByIndex(0)), nameData.AsStringByIndex(1)));
...
static float MOB_GetNormalAttackDuration(TMobTable* mobTable)
{
	...
	if ('\0' == folder[0] && (mobTable->bType == CHAR_TYPE_HORSE || mobTable->bType == CHAR_TYPE_MOUNT))
		folder = locale_npc_list_find(mobTable->dwVnum);
#else
	const char * folder = mobTable->szFolder;
#endif
...
static const char* GetMotionFileName(TMobTable* mobTable, EPublicMotion motion)
{
	...
	{
		sys_err("Motion: %s have not motlist.txt vnum(%d) folder(%s)", folder, mobTable->dwVnum,
#ifdef ENABLE_MOUNT_MOTION_FOLDER_RENEWAL
		 folder
#else
		 mobTable->szFolder
...
#ifdef ENABLE_MOUNT_MOTION_FOLDER_RENEWAL
		const char* folder = t->szFolder;

		if('\0' == folder[0] && (t->bType == CHAR_TYPE_HORSE || t->bType == CHAR_TYPE_MOUNT))
			folder = locale_npc_list_find(t->dwVnum);
