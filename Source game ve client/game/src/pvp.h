...
class CPVPManager : public singleton<CPVPManager>
{
	...
	void			EraseDuel(LPCHARACTER pkChr);
	bool			IsPlayerInPVP(LPCHARACTER ch);

#ifdef ENABLE_DUEL_OPTIONS
	void			Insert(LPCHARACTER pkChr, LPCHARACTER pkVictim, bool bSpecialDuelStatus, int DuelType);
	...
	void			GetRealValues(LPCHARACTER ch);
	void			SetCharacterBonuses(LPCHARACTER ch);
	void			ResetRealBonuses(LPCHARACTER ch);
