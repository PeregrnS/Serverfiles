...
namespace quest
{
	...
	ALUA(affect_add_realtime_affect)
	{
		CQuestManager& q = CQuestManager::instance();
		LPCHARACTER ch = q.GetCurrentCharacterPtr();
		...
		ch->AddAffect(affectType, pointType, pointValue, AFF_NONE, duration, 0, false, false, (get_global_time() + duration));
		return 0;
	}
#endif
...
#ifdef ENABLE_VOTE_4_BUFF
			{ "add_realtime_affect",	affect_add_realtime_affect },
#endif
