...
namespace quest
{
	...
	ALUA(dungeon_new_jump)
	{
		...
		pDungeon->SetFlag("posX", (int)lua_tonumber(L, 2) / 100);
		pDungeon->SetFlag("posY", (int)lua_tonumber(L, 3) / 100);
#endif
		return 0;
	}
...
#ifdef ENABLE_DUNGEON_RE_ENETER
		pDungeon->SetFlag("posX", (int)lua_tonumber(L, 2));
		pDungeon->SetFlag("posY", (int)lua_tonumber(L, 3));
#endif
...
#ifdef ENABLE_DUNGEON_RE_ENETER
		pDungeon->SetFlag("posX", (int)lua_tonumber(L, 1));
		pDungeon->SetFlag("posY", (int)lua_tonumber(L, 2));
#endif
		return 0;
	}
	...
	ALUA(dungeon_unique_set_point)
	{
		if (lua_gettop(L) < 3 || !lua_isstring(L, 1) || !lua_isnumber(L, 2) || !lua_isnumber(L, 3))
			return 0;

		LPDUNGEON pDungeon = CQuestManager::instance().GetCurrentDungeon();

		if (pDungeon)
			pDungeon->UniqueSetPoint(lua_tostring(L, 1), (uint16_t)lua_tonumber(L, 2), (int32_t)lua_tonumber(L, 3));

		return 0;
	}

	ALUA(dungeon_unique_get_point)
	{
		LPDUNGEON pDungeon = CQuestManager::instance().GetCurrentDungeon();

		if (!lua_isstring(L, 1) || !lua_isnumber(L, 2) || !pDungeon)
			lua_pushnumber(L, 0);
		else
			lua_pushnumber(L, pDungeon->UniqueGetPoint(lua_tostring(L, 1), (uint16_t)lua_tonumber(L, 2)));

		return 1;
	}

	ALUA(dungeon_change_bonus)
	{
		if (!lua_isnumber(L, 1) || !lua_isnumber(L, 2))
			return 0;
		...
		if (pDungeon)
			pDungeon->ChangeBonus((uint16_t)lua_tonumber(L, 1), (int32_t)lua_tonumber(L, 2));

		return 0;
	}

	ALUA(dungeon_remove_bonus)
	{
		if (!lua_isnumber(L, 1))
			return 0;
		...
		if (pDungeon)
			pDungeon->RemoveBonus((uint16_t)lua_tonumber(L, 1));
	...
	ALUA(dungeon_decrease_hp_by_pct)
	{
		if (!lua_isnumber(L, 1))
			return 0;

		const int pct = (int)lua_tonumber(L, 1);

		CQuestManager& q = CQuestManager::instance();
		const LPDUNGEON pDungeon = q.GetCurrentDungeon();
		if (pDungeon)
			pDungeon->DecreaseHPToAll(pct);

		return 0;
	}
#endif

	ALUA(dungeon_give_item_to_member)
	{
		...
		if (pDungeon)
		{
			LPCHARACTER ch = pDungeon->FindAnyMember();
			if (ch)
			{
...
#ifdef ENABLE_UNIQUE_POINT
			{ "unique_set_point",		dungeon_unique_set_point	},
			{ "unique_get_point",		dungeon_unique_get_point	},
			{ "change_bonus",			dungeon_change_bonus	},
			{ "remove_bonus",			dungeon_remove_bonus	},
#endif

#ifdef ENABLE_D_DECREASE_HP_BY_PCT
			{ "decrease_hp_by_pct",		dungeon_decrease_hp_by_pct	},
#endif

			{ "give_item_to_member",	dungeon_give_item_to_member		},
