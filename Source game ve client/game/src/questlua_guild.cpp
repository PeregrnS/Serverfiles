...
namespace quest
{
	...
	ALUA(guild_give_safebox)
	{
		...
		if (ch)
		{
			if (ch->GetGuild())
			{
				...
				{
					if (lua_isnumber(L, 1))
						rkSafeBox.GiveSafebox(lua_tonumber(L, 1));
					else
						rkSafeBox.GiveSafebox();
					lua_pushboolean(L, 1);
					return 1;
				}
			}
	...
	ALUA(guild_change_safebox_size)
	{
		if (!lua_isnumber(L, 1))
		{
			lua_pushboolean(L, 0);
		...
		if (ch)
		{
			if (ch->GetGuild())
			{
				...
				if (rkSafeBox.HasSafebox())
				{
					rkSafeBox.ChangeSafeboxSize(lua_tonumber(L, 1));
					lua_pushboolean(L, 1);
					return 1;
				}
			}
