...
namespace quest
{
	...
	ALUA(pc_enough_inventory)
	{
		...
		bool bEnoughInventoryForItem = ch->GetEmptyInventory(item) != -1;
#else
	...
	{
		unsigned long long gold = (unsigned long long)lua_tonumber(L, -1);

		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();

		if (static_cast<unsigned long long>(gold) + ch->GetGold() < 0)
		{
			sys_err("QUEST wrong ChangeGold %llu (now %llu)", gold, ch->GetGold());
		}
		else
		{
	...
	ALUA(pc_polymorph)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "You cannot polymorph in this map.");
			return 0;
		}
...
teleport_area:
	...
	ALUA(pc_change_name)
	{
		...
		// TODO
		// if (ch->GetGuild())
		// 	ch->GetGuild()->ChangeMemberName(pid, szName);
...
#ifdef ENABLE_SPECIAL_INVENTORY
		int iEmptyCell = ch->GetEmptyInventory(item);
#else
	...
	ALUA(pc_send_whisper)
	{
		...
		SendWhisper(ch, lua_tostring(L, 1), ((lua_isstring(L, 2)) ? lua_tostring(L, 2) : "[System]"), ((lua_isboolean(L, 3)) ? lua_toboolean(L, 3) : true));
		return 0;
	}
	
	ALUA(pc_send_whisper_to)
	{
		...
		SendWhisper(ch, lua_tostring(L, 2), ((lua_isstring(L, 3)) ? lua_tostring(L, 3) : "[System]"), ((lua_isboolean(L, 4)) ? lua_toboolean(L, 4) : true));
		return 0;
	}
#endif
	...
	ALUA(pc_open_skillgroup)
	{
		...
		packet.bHeader = HEADER_GC_SKILL_GROUP_SELECT;
		packet.bJob = ch->GetJob();

		ch->GetDesc()->Packet(&packet, sizeof(TPacketGCSkillGroup));
		lua_pushnumber(L, 1);
		return 1;
	}
#endif

	ALUA(pc_item_direct_equip)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!ch || !ch->GetDesc())
		{
		...
		if (lua_isnumber(L, 1))
		{
			iVnum = (DWORD) lua_tonumber(L, 1);
		}

		LPITEM item = ITEM_MANAGER::instance().CreateItem(iVnum);
		if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
		{
			sys_err("QUEST Make item call error : wrong item vnum : %d", iVnum);
			return 0;
		}
		return 0;
	}
	...
	ALUA(pc_summon_mount)
	{
		...
		DWORD dwMountVnum = lua_tonumber(L, 1);
		bool bFromFar = lua_isboolean(L, 2) ? lua_toboolean(L, 2) : true;

		if (pChar->GetMountVnum() || pChar->GetHorse())
		{
			if (pChar->GetMountItem() == item)
			{
				pChar->StopRiding();
				pChar->HorseSummon(false);
				pChar->ClearMountItem();
			}
		}
		else
		{
			pChar->HorseSummon(true, bFromFar, dwMountVnum, pChar->GenerateMountName(dwMountVnum), item);
		}
	...
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (ch && lua_isnumber(L, 1) && lua_isnumber(L, 2) && lua_isnumber(L, 3))
			CHARACTER_MANAGER::Instance().DoReward(ch, (BYTE)lua_tonumber(L, 1), (DWORD)lua_tonumber(L, 2), (DWORD)lua_tonumber(L, 3), lua_isnumber(L, 4) ? (BYTE)lua_tonumber(L, 4) : 0);
		return 0;
	}
#endif

#ifdef ENABLE_RANK_PLAYER
	ALUA(pc_add_progress_in_rank)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		...
		if (!lua_isnumber(L, 1))
		{
			sys_err("pc_add_progress_in_rank :: WRONG ARGUMENTS");
			return 0;
		}
		
		BYTE bTypeRank = (BYTE) rint(lua_tonumber(L, 1));
		
		RankPlayer::instance().SendInfoPlayer(ch, bTypeRank, RankPlayer::instance().GetProgressByPID(ch->GetPlayerID(), bTypeRank) + 1);
		return 0;
	}
#endif
...
#ifdef ENABLE_BATTLE_PASS
	ALUA(pc_update_dungeon_progress)
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("arg1 must be number");
	...
	ALUA(pc_open_battle_pass_ranking)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (ch)
		{
	...
	ALUA(pc_give_cash)
	{
		...
		DWORD dwAmount = (int)lua_tonumber(L, 1);

		if (dwAmount <= 0)
		{
			sys_err("QUEST : Cash amount less then zero");
			return 0;
		}

		ch->SetCash(ch->GetCash() + dwAmount);
		return 0;
	}
	...
	ALUA(pc_give_coins)
	{
		...
		if (dwAmount <= 0)
		{
			sys_err("QUEST : Coins amount less then zero");
			return 0;
		}

		ch->SetCoins(ch->GetCoins() + dwAmount);
		return 0;
	}
	...
	void RegisterPCFunctionTable()
	{
		luaL_reg pc_functions[] =
		{
			...
			{ "send_whisper_to",	pc_send_whisper_to	},	// [return nothing]
#endif
#ifdef ENABLE_PC_OPENSHOP
...
#endif
#ifdef ENABLE_SKILL_GROUP_GUI
			{"open_skillgroup", pc_open_skillgroup},
#endif
			{ "item_direct_equip", pc_item_direct_equip	},
#ifdef ENABLE_HWID_BAN
			{ "get_hwid", pc_get_hwid },
#endif
#ifdef ENABLE_MOUNT_SYSTEM
			{ "summon_mount", pc_summon_mount },
#endif
#ifdef ENABLE_DUNGEON_INFO_SYSTEM
			{ "update_dungeon_rank",		pc_update_dungeon_rank },
#endif
#ifdef ENABLE_GLOBAL_REWARD
			{ "do_reward", pc_do_reward },
#endif
#ifdef ENABLE_RANK_PLAYER
			{ "add_progress_in_rank",	pc_add_progress_in_rank		},
#endif
#ifdef __RANKING_SYSTEM__
			{ "add_progress_in_rank", pc_add_progress_in_rank },
#endif
...
#endif
			{"give_cash", pc_give_cash},
			{"give_coins", pc_give_coins},
#ifdef ENABLE_VOTE_4_BUFF
