...
namespace quest
{
	...
	bool CQuestManager::Initialize()
	{
		...
		m_mapEventName.emplace("event_begin", QUEST_GAME_EVENT_BEGIN);
		m_mapEventName.emplace("event_end", QUEST_GAME_EVENT_END);
#endif
#ifdef ENABLE_QUEST_RECEIVE_ITEM
		m_mapEventName.emplace("receive", QUEST_ITEM_RECEIVE_EVENT);
#endif
		m_bNoSend = false;
	...
	void CQuestManager::SelectItem(unsigned int pc, unsigned int selection)
	{
		...
		if (pPC && pPC->IsRunning() && (pPC->GetRunningQuestState()->suspend_state == SUSPEND_STATE_SELECT_ITEM || pPC->GetRunningQuestState()->suspend_state == SUSPEND_STATE_SELECT_ITEM_EX))
#else
	...
	void CQuestManager::Select(unsigned int pc, unsigned int selection)
	{
		...
		{
			if (pPC)
				sys_err("wrong QUEST_SELECT request! : %d (quest : %s)",pc, pPC->GetCurrentQuestName().c_str());
			else
...
#define ENABLE_PARTYKILL
	...
	void CQuestManager::SetEventFlag(const string& name, int value)
	{
		...
		else if (name == "spin_wheel")
		{
			if (!value)
				SendNotice("Spin Wheel closed.");
			else
				SendNotice("Spin Wheel opened.");
			...
			for (auto it = c_ref_set.begin(); it != c_ref_set.end(); ++it)
			{
				...
				ch->ChatPacket(CHAT_TYPE_COMMAND, "SetSpinWheel %d %d", ch->GetQuestFlag("spin_wheel.count"), value);
			}
		}
#endif
#ifdef __SPIN_WHEEL_BOSS__
		else if (name == "spin_wheel_boss")
		{
			if (!value)
				SendNotice("Boss Spin Wheel is now closed.");
			else
				SendNotice("Boss Spin Wheel is now opened.");
			...
			for (auto it = c_ref_set.begin(); it != c_ref_set.end(); ++it)
			{
				...
				ch->ChatPacket(CHAT_TYPE_COMMAND, "SetSpinWheelBoss %d %d", ch->GetQuestFlag("spin_wheel_boss.count"), value);
			}
		}
#endif
	...
	void CQuestManager::RequestSetEventFlagEx(const string& name1, const string& name2, int value)
	{
	...
	int	CQuestManager::GetEventFlagEx(const string& name1, const string& name2)
	{
		const string& name = name1 + name2;
