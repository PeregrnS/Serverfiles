			...
			tmp.type == REGEN_TYPE_ANYWHERE)
		{
			...
			*regen = tmp;
			...
			if (regen->type == REGEN_TYPE_MOB)
			{
				...
				else if (p->m_table.bType == CHAR_TYPE_MONSTER && p->m_table.bRank > 3/*== MOB_RANK_KING*/)
				{
					sys_log(0, "InsertBossPosition %lu", regen->vnum);
					SECTREE_MANAGER::instance().InsertBossPosition(lMapIndex,
							p->m_table.szLocaleName,
							(regen->sx+regen->ex) / 2 - base_x,
							(regen->sy+regen->ey) / 2 - base_y,
							regen->time);
