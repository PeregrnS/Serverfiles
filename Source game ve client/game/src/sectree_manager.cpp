...
int SECTREE_MANAGER::Build(const char * c_pszListFileName, const char* c_pszMapBasePath)
{
	...
	for (auto & line : lines)
	{
		...
		if (map_allow_find(iIndex))
		{
			...
			filename = fmt::format("{}/{}/", c_pszMapBasePath, szMapName);
			regen_register_map(filename.c_str(), setting.iIndex, setting.iBaseX, setting.iBaseY);
#endif
...
void SECTREE_MANAGER::SendBossPosition(LPCHARACTER ch)
{
	...
	if (m_mapBossPosition[lMapIndex].empty())
		return;

	// TEMP_BUFFER buf{8192*8192, false} ;
	TEMP_BUFFER buf;
	TPacketGCBossPosition p;
	p.header = HEADER_GC_BOSS_POSITION;
	p.count = m_mapBossPosition[lMapIndex].size();
	
	TBossPosition posBoss;
	for (const auto & it : m_mapBossPosition[lMapIndex])
	{
		posBoss.dwTime = it.regenTime;
		strlcpy(posBoss.name, it.name, sizeof(posBoss.name));

		posBoss.x = it.x;
		posBoss.y = it.y;
		buf.write(&posBoss, sizeof(posBoss));
	}
...
void SECTREE_MANAGER::InsertBossPosition(long lMapIndex, const char* szName, long x, long y, DWORD dwTime)
{
	m_mapBossPosition[lMapIndex].push_back(SBossInfo(szName, x, y, dwTime));
}
#endif
