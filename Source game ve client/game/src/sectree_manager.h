...
struct SBossInfo
{
	...
	long x, y;
	DWORD regenTime;
	SBossInfo(const char* name, long xpos, long ypos, DWORD time)
		: name(name), x(xpos), y(ypos), regenTime(time)
		{}
};
#endif
...
#ifdef ENABLE_ATLASS_EXTENDED
		void		SendBossPosition(CHARACTER* ch);
		void		InsertBossPosition(long lMapIndex, const char* szName, long x, long y, DWORD dwTime);
#endif
...
#ifdef ENABLE_ATLASS_EXTENDED
		std::map<DWORD, std::vector<SBossInfo>> m_mapBossPosition;
#endif
