...
// #endif
	...
	if (ch->IsGM() == true && pkShop->IsPCShop())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("GM_BLOCK_BUY"));
		return;
	}
#endif
...
void CShopManager::Sell(LPCHARACTER ch
...
// #endif
	...
	if (ch->IsGM() == true)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("GM_BLOCK_SELL"));
		return;
	}
#endif
...
bool CShopManager::InitializeShopEX(TShopTable* table, int size)
{
	...
	for (int i = 0; i < size; ++i, ++table)
	{
		...
		for (size_t j = 0; j < table->byItemCount; j++)
		{
			...
			grid.Put(iPos, 1, item_table->bSize);
			sys_log(0, "grid put at pos %d %d size %d", iPos, 1, item_table->bSize);
