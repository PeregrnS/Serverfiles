...
class KeyFrame(CamyListBox.Item):
	def __init__(self, _x, _y, _z, _zoom, _pitch, _rotation, _time, _ease, _name=""):
		...
		self.name.Show()
		self.name.OnMouseLeftButtonDown = ui.__mem_func__(self.OnMouseLeftButtonDown)
...
class ScriptManager(ui.ScriptWindow):
	def __init__(self, camyInstance, iSaveLoad = 0):
		...
		self.SetWindowName("ScriptManager")
	...
	def InitializeUI(self, iSaveLoad):
		try:
			...
			if iSaveLoad == 0:
				...
				btnDone.SetEvent(self.Done, self.camyInstance.SaveScript)
			else:
				getObject("board").SetTitleName("Script laden")
				btnDone.SetText("Laden")
				btnDone.SetEvent(self.Done, self.camyInstance.LoadScript)
			
			self.fileName = getObject("editlineScriptName")
		except:
			exception.Abort("Failed to load and/or bind interface")
...
class Interface(ui.ScriptWindow):
	EASE_TYPES_NAMES = ["Linear", "Quadratisch", "Kubisch", "Quartisch", "Quintisch", "Sinus", "Exponentiell", "Zirkular"]
	EASE_TYPES = ["linear", "quadratic", "cubic", "quartic", "quintic", "circular", "exponential", "sinusoidal"]
	def __init__(self, inst=None):
		ui.ScriptWindow.__init__(self)
		
		self.SetWindowName("camModuleInterface")
		...
		if hasattr(app, "CAMY_COMPATIBLE_CLIENT"):
			dbg.TraceError("##############################")
			dbg.TraceError("This client is ready for Camy!")
			dbg.TraceError("##############################")
			self.camyClient = 1
		self.InitializeUI()
	...
	def InitializeUI(self):
		try:
			...
			getObject("btnPlay").SetEvent(self.PlaySequence, 0)
			getObject("btnPlaySmooth").SetEvent(self.PlaySequence, 1)
			getObject("btnSaveScript").SetEvent(self.ShowScriptManager, 0)
			getObject("btnLoadScript").SetEvent(self.ShowScriptManager, 1)
	...
	def DeleteKeyFrame(self):
		...
		for i in xrange(0, self.kfList.GetItemCount()):
			...
			point = KeyFrame(it.X, it.Y, it.Z, it.ZOOM, it.PITCH, it.ROTATION, it.TIME, it.EASE, "KeyFrame " + str(len(newList)))
	...
	def AddNewKeyFrame(self):
		if self.kfList.GetItemCount() < 1:
			...
			with open("camy/camSettings.default") as f:
				lines = f.readlines()
			pos = lines[0].split(" ")
			point = KeyFrame(int(round(float(pos[1]))), int(round(float(pos[2]))), int(round(float(pos[3]))), 2494, 31, -7, 1, 0, "KeyFrame " + str(self.kfList.GetItemCount()))
			point.SetParent(self.kfList)
			self.kfList.AppendItem(point)
		else:
			a = self.kfList.GetItemAtIndex(self.kfList.GetItemCount() - 1)
			point = KeyFrame(a.X, a.Y, a.Z, a.ZOOM, a.PITCH, a.ROTATION, a.TIME, a.EASE, "KeyFrame " + str(self.kfList.GetItemCount()))
			point.SetParent(self.kfList)
			self.kfList.AppendItem(point)
	...
	def SaveScript(self, fileName):
		...
		with open("camy/" + fileName + ".script", "wb") as fw:
			for i in xrange(0, c):
				...
				fw.write(kf.name.GetText() + "#" + str(kf.X) + "#" + str(kf.Y) + "#" + str(kf.Z) + "#" + str(kf.ZOOM) + "#" + str(kf.PITCH) + "#" + str(kf.ROTATION) + "#" + str(kf.TIME) + "#" + str(kf.EASE) + "\n")
		...
		try:
			...
			if fileContentSize > 0:
				...
				for i in xrange(0, fileContentSize):
					...
					if len(splitted) == 9:
						point = KeyFrame(int(round(float(splitted[1]))), int(round(float(splitted[2]))), int(round(float(splitted[3]))), int(round(float(splitted[4]))), int(round(float(splitted[5]))), int(round(float(splitted[6]))), int(round(float(splitted[7]))), int(round(float(splitted[8]))), splitted[0])
						point.SetParent(self.kfList)
						self.kfList.AppendItem(point)
		except:
			dbg.TraceError("Failed to read file " + fileName)
	...
	def PlaySequence(self, state):
		...
		if state == 0:
			...
			for i in xrange(0, len(playKeyFrames) - 1):
				...
				self.calcNormalPlay(playKeyFrames[i].X, playKeyFrames[i+1].X, f, self.EASE_TYPES[playKeyFrames[i].EASE], self.xCoords)
				self.calcNormalPlay(playKeyFrames[i].Y, playKeyFrames[i+1].Y, f, self.EASE_TYPES[playKeyFrames[i].EASE], self.yCoords)
				self.calcNormalPlay(playKeyFrames[i].Z, playKeyFrames[i+1].Z, f, self.EASE_TYPES[playKeyFrames[i].EASE], self.zCoords)
				self.calcNormalPlay(playKeyFrames[i].ZOOM, playKeyFrames[i+1].ZOOM, f, self.EASE_TYPES[playKeyFrames[i].EASE], self.zoomCoords)
				self.calcNormalPlay(playKeyFrames[i].PITCH, playKeyFrames[i+1].PITCH, f, self.EASE_TYPES[playKeyFrames[i].EASE], self.pitchCoords)
				self.calcNormalPlay(playKeyFrames[i].ROTATION, playKeyFrames[i+1].ROTATION, f, self.EASE_TYPES[playKeyFrames[i].EASE], self.rotationCoords)
			self.act = 0
			self.SetPlayState(1)
		elif state == 1:
			with open("camy/spl/tmp.camyin", "wb") as fw:
				for i in xrange(0, self.kfList.GetItemCount()):
					...
					fw.write(str(item.X) + "|" + str(item.Y) + "|" + str(item.Z) + "|" + str(item.ZOOM) + "|" + str(item.PITCH) + "|" + str(item.ROTATION) + "|" + str(item.TIME) + "|" + self.EASE_TYPES[item.EASE] + "|\n")
			...
			for i in xrange(0, len(lines)):
				...
				self.playKeyFrames.append(KeyFrame(float(x), float(y), float(z), float(zoom), float(pitch), float(rotation), 0.0, 0, ""))
	...
	def SelectKeyFrame(self, selectedKeyFrame):
		...
		self.currentlySelectedKeyFrame = KeyFrame(selectedKeyFrame.X, selectedKeyFrame.Y, selectedKeyFrame.Z, selectedKeyFrame.ZOOM, selectedKeyFrame.PITCH, selectedKeyFrame.ROTATION, selectedKeyFrame.TIME, selectedKeyFrame.EASE, selectedKeyFrame.name.GetText())
		self.currentlySelectedKeyFrameHelp = KeyFrame(selectedKeyFrame.X, selectedKeyFrame.Y, selectedKeyFrame.Z, selectedKeyFrame.ZOOM, selectedKeyFrame.PITCH, selectedKeyFrame.ROTATION, selectedKeyFrame.TIME, selectedKeyFrame.EASE, selectedKeyFrame.name.GetText())
	...
	def SliderChangeX(self, adjust=0):
		...
		if adjust == 0:
			if self.currentlySelectedKeyFrame != None and self.currentlySelectedKeyFrameHelp != None:
				self.currentlySelectedKeyFrame.X = self.currentlySelectedKeyFrameHelp.X + int((self.kfPosXSlider.GetSliderPos() - 0.5) * 10000.0)
		else:
			self.kfPosXSlider.SetSliderPos(max(0.0, min(1.0, (float(self.currentlySelectedKeyFrameHelp.X) - float(self.currentlySelectedKeyFrameHelp.X) + 10000.0) / 10000.0 - 0.5)))

	def SliderChangeY(self, adjust=0):
		self.lastChange = "Y"
		if adjust == 0:
			if self.currentlySelectedKeyFrame != None and self.currentlySelectedKeyFrameHelp != None:
				self.currentlySelectedKeyFrame.Y = self.currentlySelectedKeyFrameHelp.Y + int((self.kfPosYSlider.GetSliderPos() - 0.5) * 10000.0)
		else:
			self.kfPosYSlider.SetSliderPos(max(0.0, min(1.0, (float(self.currentlySelectedKeyFrame.Y) - float(self.currentlySelectedKeyFrameHelp.Y) + 10000.0) / 10000.0 - 0.5)))

	def SliderChangeZ(self, adjust=0):
		self.lastChange = "Z"
		if adjust == 0:
			if self.currentlySelectedKeyFrame != None and self.currentlySelectedKeyFrameHelp != None:
				self.currentlySelectedKeyFrame.Z = self.currentlySelectedKeyFrameHelp.Z + int((self.kfPosZSlider.GetSliderPos() - 0.5) * 10000.0)
		else:
			self.kfPosZSlider.SetSliderPos(max(0.0, min(1.0, (float(self.currentlySelectedKeyFrame.Z) - float(self.currentlySelectedKeyFrameHelp.Z) + 10000.0) / 10000.0 - 0.5)))

	def SliderChangeZoom(self, adjust=0):
		self.lastChange = "D"
		if adjust == 0:
			if self.currentlySelectedKeyFrame != None and self.currentlySelectedKeyFrameHelp != None:
				self.currentlySelectedKeyFrame.ZOOM = self.currentlySelectedKeyFrameHelp.ZOOM + int((self.kfPosZoomSlider.GetSliderPos() - 0.5) * 2000.0)
		else:
			self.kfPosZoomSlider.SetSliderPos(max(0.0, min(1.0, (float(self.currentlySelectedKeyFrame.ZOOM) - float(self.currentlySelectedKeyFrameHelp.ZOOM) + 2000.0) / 2000.0 - 0.5)))

	def SliderChangePitch(self, adjust=0):
		self.lastChange = "P"
		if adjust == 0:
			if self.currentlySelectedKeyFrame != None and self.currentlySelectedKeyFrameHelp != None:
				self.currentlySelectedKeyFrame.PITCH = self.currentlySelectedKeyFrameHelp.PITCH + int((self.kfPosPitchSlider.GetSliderPos() - 0.5) * 40.0)
		else:
			self.kfPosPitchSlider.SetSliderPos(max(0.0, min(1.0, (float(self.currentlySelectedKeyFrame.PITCH) - float(self.currentlySelectedKeyFrameHelp.PITCH) + 40.0) / 40.0 - 0.5)))

	def SliderChangeRotation(self, adjust=0):
		self.lastChange = "R"
		if adjust == 0:
			if self.currentlySelectedKeyFrame != None and self.currentlySelectedKeyFrameHelp != None:
				self.currentlySelectedKeyFrame.ROTATION = self.currentlySelectedKeyFrameHelp.ROTATION + int((self.kfPosRotationSlider.GetSliderPos() - 0.5) * 180.0)
		else:
			self.kfPosRotationSlider.SetSliderPos(max(0.0, min(1.0, (float(self.currentlySelectedKeyFrame.ROTATION) - float(self.currentlySelectedKeyFrameHelp.ROTATION) + 180.0) / 180.0 - 0.5)))

	def SliderChangeTime(self, adjust=0):
		self.lastChange = "T"
		if adjust == 0:
			if self.currentlySelectedKeyFrame != None and self.currentlySelectedKeyFrameHelp != None:
				self.currentlySelectedKeyFrame.TIME = self.currentlySelectedKeyFrameHelp.TIME + int((self.kfSettingsTimeSlider.GetSliderPos() - 0.5) * 8.0)
		else:
			self.kfPosTimeSlider.SetSliderPos(max(0.0, min(1.0, (float(self.currentlySelectedKeyFrame.TIME) - float(self.currentlySelectedKeyFrameHelp.TIME) + 8.0) / 8.0 - 0.5)))
	...
	def DownKey(self, key):
		curTime = app.GetTime()
		if self.downKey == key:
			if self.nextUpdateTime < curTime:
				self.PressedKey()
		else:
			self.downKey = key
			self.nextUpdateTime = curTime + 0.25
			self.PressedKey()
	...
	def ApplyChanges(self):
		if self.currentlySelectedKeyFrameIndex != None and self.currentlySelectedKeyFrame != None:
			self.kfList.ReplaceItemAtIndex(self.currentlySelectedKeyFrameIndex, KeyFrame(self.currentlySelectedKeyFrame.X, self.currentlySelectedKeyFrame.Y, self.currentlySelectedKeyFrame.Z, self.currentlySelectedKeyFrame.ZOOM, self.currentlySelectedKeyFrame.PITCH, self.currentlySelectedKeyFrame.ROTATION, self.currentlySelectedKeyFrame.TIME, self.currentlySelectedKeyFrame.EASE, self.currentlySelectedKeyFrame.name.GetText()))
	...
	def OnUpdate(self):
		if self.isPlaySequence == 0:
			...
			if not self.board.IsShow():
				self.board.Show()
			if cSKF != None:
				app.SetCameraSetting(cSKF.X, cSKF.Y, cSKF.Z, cSKF.ZOOM, cSKF.ROTATION, cSKF.PITCH)
				app.SetCamera(cSKF.ZOOM, cSKF.PITCH, cSKF.ROTATION, 0.0)
			else:
				app.SetCamera(1.0, 0.0, 0.0, 0.0)
		elif self.isPlaySequence == 1:
			app.SetCameraSetting(self.xCoords[self.act], self.yCoords[self.act], self.zCoords[self.act], self.zoomCoords[self.act], self.rotationCoords[self.act], self.pitchCoords[self.act])
			...
			app.SetCameraSetting(k.X, k.Y, k.Z, k.ZOOM, k.PITCH, k.ROTATION)
