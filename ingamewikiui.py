...
categoryPeakWindowSize = [141, 339]

mainBoardPos = [177, 34]
mainBoardSize = [555, 438]

monsterBonusInfoPageSize = [539, 223]
...
class WikiRenderTarget(ui.WikiRenderTarget):
	...
	def SetModel(self, model_vnum):
		if self.IsValidModuleID():
			wiki.RenderTargetSetBackground(self.moduleID, "d:/ymir work/ui/game/myshop_deco/render_target.tga")
	...
	if app.ENABLE_SHINING_ITEM_SYSTEM:
		def SetModelShining(self, shining_vnum):
			if self.IsValidModuleID():
				wiki.SetShiningModel(self.moduleID, shining_vnum)
...
class WikiBuildGrid(ui.ScriptWindow):
	...
	AUTO_BUILD_GRID_IMAGES = [ "d:/ymir work/ui/systems/wiki/grid/detail_item_3.tga", "d:/ymir work/ui/systems/wiki/grid/detail_item.tga"]
	...
	def LoadGrid(self, size):
		...
		if size <= self.AUTO_BUILD_GRID_COUNT:
			...
			self.grid_elements[0].AddFlag("attach")
			self.grid_elements[0].AddFlag("not_pick")
			self.grid_elements[0].LoadImage("d:/ymir work/ui/systems/wiki/grid/initial_grid.tga")
			self.grid_elements[0].SetPosition(0,0)
			...
			for i in xrange(REFACT_COUNT):
				...
				tmpGrid.LoadImage("d:/ymir work/ui/systems/wiki/grid/grid_line.tga")
			...
			self.grid_elements[-1].LoadImage("d:/ymir work/ui/systems/wiki/grid/last_grid.tga")
...
class SubCategObject(ui.Window):
	...
	def __init__(self, text):
		...
		self.renderImg = ui.ExpandedImageBox()
		self.renderImg.SetParent(self)
		self.renderImg.AddFlag("attach")
		self.renderImg.AddFlag("not_pick")
		self.renderImg.SetPosition(0, 0)
		self.renderImg.LoadImage(PRIVATESHOP_PATH + "category_small_button_default.sub")
		self.renderImg.Show()

		self.textWindow = ui.Window()
		self.textWindow.SetParent(self.renderImg)
		self.textWindow.AddFlag("attach")
		self.textWindow.AddFlag("not_pick")
		self.textWindow.SetPosition(0, 0)
		...
		self.cutTextLine.SetPosition(10, 2)
	...
	def OnRender(self):
		if self.parent.GetSelectedItem()==self:
			grp.SetColor(ui.INVISIBLE_COLOR)
	...
	def OnUpdate(self):
		if self.parent.GetSelectedItem()==self:
			self.renderImg.LoadImage(PRIVATESHOP_PATH + "category_small_button_down.sub")
		elif self.IsIn():
			self.renderImg.LoadImage(PRIVATESHOP_PATH + "category_small_button_over.sub")
		else:
			self.renderImg.LoadImage(PRIVATESHOP_PATH + "category_small_button_default.sub")
...
class WikiCategory(ui.Window):
	...
	TICK_COUNT = 6
	MIN_HEIGHT = 20
	ARROW_IMG = ["d:/ymir work/ui/systems/wiki/arrow.tga", "d:/ymir work/ui/systems/wiki/arrow_up.tga"]
	CATEGORY_IMG = [PRIVATESHOP_PATH + "category_big_button_default.sub", PRIVATESHOP_PATH + "category_big_button_down.sub"]

	LINES_COLOR = 0xff4E0000
	CORNER_COLOR = 0xff4E0000
	BASE_COLOR = 0xff4E0000
	...
	def __init__(self, owner = None):
		...
		self.titleImg.LoadImage(self.CATEGORY_IMG[0])
		self.titleImg.SetStringEvent("mouse_left_down",ui.__mem_func__( self.ClickExpand))
		self.titleImg.SetStringEvent("mouse_over_in",ui.__mem_func__(self.OnCategoryOverIn))
		self.titleImg.SetStringEvent("mouse_over_out",ui.__mem_func__(self.OnCategoryOverOut))
		self.titleImg.Show()
		...
		self.titleText.SetPosition(28, self.titleImg.GetHeight() / 2)
		self.titleText.Show()
		...
		self.expandWnd.SetPosition(16, self.titleImg.GetHeight())
		...
		self.categList.SetItemSize(self.titleImg.GetWidth() - 6, 19)
		self.categList.SetItemStep(20)
		self.categList.SetSize(self.titleImg.GetWidth() - 6, self.MIN_HEIGHT)
		self.categList.SetPosition(0, 2)
	...
	def AddSubCategory(self, key, text):
		self.currHeight = ((17 + 3) * (self.categList.GetItemCount() + 1) + 3)
	...
	def ClickExpand(self):
		...
		self.titleImg.LoadImage(self.CATEGORY_IMG[(0 if self.isOpening else 1)])
	...
	def OnCategoryOverIn(self):
		self.titleImg.LoadImage(PRIVATESHOP_PATH + "category_big_button_over.sub")

	def OnCategoryOverOut(self):
		self.titleImg.LoadImage(self.CATEGORY_IMG[(1 if self.isOpening else 0)])
	...
	def SetTitleName(self, text):
		...
		self.titleText.SetPosition(20, pos_y)
...
class WikiMainWeaponWindow(ui.Window):
	class WikiItem(ui.Window):
		...
		def GetMaterialCount(self, retInfo):
			maxMat = self.STARTING_MAT_COUNT

			for i in xrange(len(retInfo)):
				curr = 0
		...
		def NoticeMe(self, noticeType = [False, 0]):
			...
			maxMat = self.GetMaterialCount(retInfo)
			moneyRow = (maxMat + 1)
	...
	classButtonsPath = "d:/ymir work/ui/systems/wiki/class_buttons/"
	...
	def __init__(self):
		...
		self.wikiRenderTarget = WikiRenderTarget(225, 225)
	...
	def OnOverIn(self, vnum):
		...
		if (itemType == item.ITEM_TYPE_WEAPON and subType != 6) or (itemType == item.ITEM_TYPE_ARMOR and subType == 0):
			self.toolTip.SetThinBoardSize(self.wikiRenderTarget.GetWidth() + (self.toolTip.toolTipWidth/4), self.wikiRenderTarget.GetHeight() + self.toolTip.toolTipHeight)
			...
			if (itemType == item.ITEM_TYPE_WEAPON or itemType == item.ITEM_TYPE_WEAPON and subType != 6):
				...
				# Zoom
				(V3Eye, V3Target) = constInfo.GetCharTypeWeaponCamera(char_type, subType)
				...
				# Zoom
				(V3Eye, V3Target) = constInfo.GetCharTypeCostumeCamera(char_type)
				if len(V3Eye) and len(V3Target):
					self.wikiRenderTarget.SetModelV3Eye(*V3Eye)
					self.wikiRenderTarget.SetModelV3Target(*V3Target)
	...
	def RegisterScrollBar(self):
		...
		self.scrollBar.SetPosition(self.peekWindow.GetLocalPosition()[0] + self.peekWindow.GetWidth() + 5, self.peekWindow.GetLocalPosition()[1])
		self.scrollBar.SetSize(6, self.peekWindow.GetHeight())
		self.scrollBar.SetScrollEvent(self.OnScrollBar)
		self.scrollBar.SetScrollSpeed(self.SCROLL_SPEED)
		self.scrollBar.Show()
...
class ChestPeekWindow(ui.Window):
	...
	def RegisterScrollBar(self):
		self.scrollBar = WikiScrollBar()
		self.scrollBar.SetParent(self)
		self.scrollBar.SetPosition(self.peekWindow.GetLocalPosition()[0] + self.peekWindow.GetWidth() + 5,\
...
class WikiMainChestWindow(ui.Window):
	class WikiItem(ui.Window):
		def __init__(self, vnum, parent):
			...
			self.base.LoadImage("d:/ymir work/ui/systems/wiki/grid/detail_chest.tga")
			self.base.Show()
...
class WikiMainBossWindow(ui.Window):
	class WikiItem(ui.Window):
		def __init__(self, vnum, parent):
			...
			self.modelView = WikiRenderTarget(47, 87) ## class WikiMainBossWindow
...
class WikiMonsterBonusInfoWindow(ui.Window):
	...
	def __init__(self, vnum):
		...
		damageMin = localeInfo.DottedNumber(_dmin)
		damageMax = localeInfo.DottedNumber(_dmax)
		goldMin = localeInfo.DottedNumber(_gMin)
		goldMax = localeInfo.DottedNumber(_gMax)
		...
		d_page_content = [
			...
			((localeInfo.WIKI_MONSTERINFO_DMG_HP % (damageMin, damageMax, localeInfo.DottedNumber(nonplayer.GetMonsterMaxHP(self.vnum)))), 0),
			((localeInfo.WIKI_MONSTERINFO_GOLD_EXP % (goldMin, goldMax, localeInfo.DottedNumber(nonplayer.GetMonsterExp(self.vnum)))), 0),
	...
	def AddItem(self, text, padding = 0):
		...
		tmp.img.LoadImage("d:/ymir work/ui/systems/wiki/arrow_right.tga")
...
class WikiItemOriginInfo(ui.Window):
	...
	def ParseTextlines(self):
		...
		for (vnum, isMonster) in lst:
			...
			# UNIQUE_GROUP
			if vnum == 10001 or vnum == 10501 or vnum == 10502:
				continue
...
class SpecialPageWindow(ui.Window):
	...
	def __init__(self, vnum, isMonster):
		...
		self.bg.LoadImage("d:/ymir work/ui/systems/wiki/grid/detail_monster.tga")
		self.bg.Show()
		...
		if isMonster:
			...
			self.modelView = WikiRenderTarget(163, 163) ## class SpecialPageWindow(
			...
			self.bonusInfo = WikiMainChestWindow.WikiItem(self.vnum, self)
...
class SimpleTextLoader(ui.Window):
	...
	def GetPharserAffect(self, affect):
		if affect == 53:
			return localeInfo.DETAILS_12
		if affect == 63:
			return localeInfo.DETAILS_5
		if affect == 8:
			return localeInfo.STAT_TOOLTIP_MOVE_SPEED
		if affect == 87:
			return localeInfo.DETAILS_0
		if affect == 28:
			return localeInfo.DETAILS_64
		if affect == 27:
			return localeInfo.DETAILS_63
		if affect == 89:
			return localeInfo.DETAILS_26
		if affect == 95:
			return localeInfo.DETAILS_45
		if affect == 73:
			return localeInfo.DETAILS_17
		if affect == 24:
			return localeInfo.DETAILS_62
		if affect == 23:
			return localeInfo.DETAILS_61
		if affect == 26:
			return localeInfo.DETAILS_67
		if affect == 54:
			return localeInfo.PET_TOOLTIP_DEF
		if affect == 66:
			return localeInfo.DETAILS_73
		if affect == 43:
			return localeInfo.DETAILS_68
		if affect == 1:
			return localeInfo.PARTY_BONUS_TANKER_STATIC
		if affect == 79:
			return localeInfo.DETAILS_42
		if affect == 97:
			return localeInfo.PRIVATE_SHOP_SEARCH_SELECT_SASH_ABSORPTION
		if affect == 67:
			return localeInfo.DETAILS_74
		if affect == 2:
			return localeInfo.PARTY_BONUS_SKILL_MASTER_STATIC
		if affect == 37:
			return localeInfo.DETAILS_76
		if affect == 82:
			return localeInfo.BLEND_INFO_ENERGY
		if affect == 7:
			return localeInfo.STAT_TOOLTIP_ATT_SPEED
		if affect == 9:
			return localeInfo.STAT_TOOLTIP_CAST_SPEED
		if affect == 86:
			return localeInfo.DETAILS_18

		return ""

	def GetLocaleText(self, locale):
		if locale == 2:
			return localeInfo.PRIVATESHOPSEARCH_WEAPON
		if locale == 3:
			return localeInfo.PRIVATESHOPSEARCH_BODY
		if locale == 4:
			return localeInfo.PRIVATESHOPSEARCH_HEAD
		if locale == 5:
			return localeInfo.PRIVATESHOPSEARCH_SHIELD
		if locale == 6:
			return localeInfo.PRIVATESHOPSEARCH_EAR

		return ""

	def GetLocaleTextArg(self, locale, locale_arg):
		if locale == 1:
			if locale_arg == 1:
				return localeInfo.WIKI_SUBCATEGORY_UTILS_TITLE % localeInfo.PRIVATESHOPSEARCH_ARMOR
	...
	def LoadFile(self, filename):
		...
		self.scrollBoard.SetSize(0, 0)
		self.UpdateScrollbar()
		open_filename = localeInfo.FN_WIKI + "{}".format(filename)
		...
		for i in loadF.readlines()[1:]:
			...
			if ret:
				...
				if tokenMap.has_key("img"):
					...
					if tokenMap.has_key("center_align"):
						...
						tmp.SetPosition(self.peekWindow.GetWidth() + cimg.GetWidth(), y)
						tokenMap.pop("left_align")
					else:
				...
				if tokenMap.has_key("item_img"):
					item.SelectItem(int(tokenMap["item_img"]))
					cItemImg = ui.ExpandedImageBox()
					cItemImg.SetParent(self.scrollBoard)
					cItemImg.AddFlag("attach")
					cItemImg.LoadImage(item.GetIconImageFileName())
					cItemImg.SAFE_SetEvent(self.OnMouseOverIn, "mouse_over_in", item.GetItemVnum())
					cItemImg.SAFE_SetEvent(self.OnMouseOverOut, "mouse_over_out")
					cItemImg.Show()
					
					tokenMap.pop("item_img")
					...
					if tokenMap.has_key("center_align"):
						cItemImg.SetPosition(self.peekWindow.GetWidth() / 2 - cItemImg.GetWidth() / 2, y)
						tokenMap.pop("center_align")
					elif tokenMap.has_key("right_align"):
						cItemImg.SetPosition(self.peekWindow.GetWidth() - cItemImg.GetWidth(), y)
						tokenMap.pop("right_align")
					elif tokenMap.has_key("left_align"):
						tmp.SetPosition(self.peekWindow.GetWidth() + cItemImg.GetWidth(), y)
						tokenMap.pop("left_align")
					else:
						cItemImg.SetPosition(x, y)
					
					self.item_img.append(cItemImg)
			...
			if len(i) > 0 and i[0] == "*":
				...
				tmp.img.AddFlag("not_pick")
				# if localeInfo.IsARABIC():
				# 	tmp.img.LoadImage("d:/ymir work/ui/wiki/arrow_2ae.tga")
				# else:
			...
			elif tokenMap.has_key("y_padding"):
				tmp.SetPosition(268, 115, True)
				tokenMap.pop("y_padding")
			elif tokenMap.has_key("y_padding1"):
				tmp.SetPosition(186, 145, True)
				tokenMap.pop("y_padding1")
			elif tokenMap.has_key("y_padding2"):
				tmp.SetPosition(157, 175, True)
				tokenMap.pop("y_padding2")
			elif tokenMap.has_key("y_padding3"):
				tmp.SetPosition(251, 205, True)
				tokenMap.pop("y_padding3")
			elif tokenMap.has_key("y_padding4"):
				tmp.SetPosition(215, 235, True)
				tokenMap.pop("y_padding4")
			elif tokenMap.has_key("y_padding5"):
				tmp.SetPosition(235, 265, True)
				tokenMap.pop("y_padding5")
			elif tokenMap.has_key("y_padding6"):
				tmp.SetPosition(268, 295, True)
				tokenMap.pop("y_padding6")
			elif tokenMap.has_key("y_padding7"):
				tmp.SetPosition(268, 325, True)
				tokenMap.pop("y_padding7")
			elif tokenMap.has_key("y_padding8"):
				tmp.SetPosition(272, 355, True)
				tokenMap.pop("y_padding8")
			elif tokenMap.has_key("y_padding9"):
				tmp.SetPosition(291, 385, True)
				tokenMap.pop("y_padding9")
			elif tokenMap.has_key("y_padding10"):
				tmp.SetPosition(293, 415, True)
				tokenMap.pop("y_padding10")
			elif tokenMap.has_key("y_padding11"):
				tmp.SetPosition(250, 445, True)
				tokenMap.pop("y_padding11")
			elif tokenMap.has_key("y_padding12"):
				tmp.SetPosition(260, 475, True)
				tokenMap.pop("y_padding12")
			elif tokenMap.has_key("y_padding13"):
				tmp.SetPosition(247, 505, True)
				tokenMap.pop("y_padding13")
			elif tokenMap.has_key("y_padding14"):
				tmp.SetPosition(259, 535, True)
				tokenMap.pop("y_padding14")
			elif tokenMap.has_key("y_padding15"):
				tmp.SetPosition(216, 565, True)
				tokenMap.pop("y_padding15")
			elif tokenMap.has_key("y_padding16"):
				tmp.SetPosition(168, 595, True)
				tokenMap.pop("y_padding16")
			elif tokenMap.has_key("y_padding17"):
				tmp.SetPosition(144, 625, True)
				tokenMap.pop("y_padding17")
			elif tokenMap.has_key("y_padding18"):
				tmp.SetPosition(139, 655, True)
				tokenMap.pop("y_padding18")
			elif tokenMap.has_key("y_padding19"):
				tmp.SetPosition(171, 685, True)
				tokenMap.pop("y_padding19")
			elif tokenMap.has_key("y_padding20"):
				tmp.SetPosition(195, 715, True)
				tokenMap.pop("y_padding20")
			elif tokenMap.has_key("y_padding21"):
				tmp.SetPosition(125, 745, True)
				tokenMap.pop("y_padding21")
			elif tokenMap.has_key("y_padding22"):
				tmp.SetPosition(127, 775, True)
				tokenMap.pop("y_padding22")
			elif tokenMap.has_key("y_padding23"):
				tmp.SetPosition(100, 805, True)
				tokenMap.pop("y_padding23")
			elif tokenMap.has_key("y_padding24"):
				tmp.SetPosition(81, 835, True)
				tokenMap.pop("y_padding24")
			elif tokenMap.has_key("y_padding25"):
				tmp.SetPosition(174, 865, True)
				tokenMap.pop("y_padding25")
			elif tokenMap.has_key("y_padding26"):
				tmp.SetPosition(205, 895, True)
				tokenMap.pop("y_padding26")
			elif tokenMap.has_key("y_padding27"):
				tmp.SetPosition(220, 925, True)
				tokenMap.pop("y_padding27")
			elif tokenMap.has_key("y_padding28"):
				tmp.SetPosition(225, 955, True)
				tokenMap.pop("y_padding28")
			elif tokenMap.has_key("y_padding29"):
				tmp.SetPosition(265, 985, True)
				tokenMap.pop("y_padding29")
			elif tokenMap.has_key("y_padding30"):
				tmp.SetPosition(268, 1015, True)
				tokenMap.pop("y_padding30")
		...
		for j in self.item_img:
			mxSize = j.GetLocalPosition()[1] + j.GetHeight()
			if mxSize > self.scrollBoard.GetHeight():
				self.scrollBoard.SetSize(self.peekWindow.GetWidth(), mxSize)
...
class WikiMainCostumeWindow(ui.Window):
	class WikiItem(ui.Window):
		def __init__(self, vnum, parent):
			...
			self.base.AddFlag("attach")
			self.base.AddFlag("not_pick")
			self.base.LoadImage("d:/ymir work/ui/systems/wiki/grid/detail_item_small_no_title.tga")
			self.base.Show()
			...
			self.costumeImage.SetPosition(1 + (125 - 55) / 2 - self.costumeImage.GetWidth() / 2,\
			...
			self.contentText.SetPosition(1 + (125 - 55) / 2 - self.contentText.GetTextSize()[0] / 2, 122 + 18 / 2 - self.contentText.GetTextSize()[1] / 2 - 1)
			self.contentText.Hide()
	...
	def __init__(self):
		...
		self.wikiRenderTarget = WikiRenderTarget(225, 225) ## class WikiMainCostumeWindow(
		self.wikiRenderTarget.SetParent(self.toolTip)
		self.wikiRenderTarget.AddFlag("not_pick")
		self.wikiRenderTarget.SetPosition(5, self.toolTip.toolTipHeight)
		self.wikiRenderTarget.Hide()
	...
	def OnOverIn(self, vnum, metinSlot = [0 for i in xrange(player.METIN_SOCKET_MAX_NUM)]):
		...
		self.toolTip.ClearToolTip()
		self.toolTip.SetThinBoardSize(self.wikiRenderTarget.GetWidth() + 10,
									  self.wikiRenderTarget.GetHeight() + self.toolTip.toolTipHeight)
		self.toolTip.childrenList.append(self.wikiRenderTarget)
		self.toolTip.ResizeToolTip()
		...
		if itemType == item.ITEM_TYPE_WEAPON or itemType == item.ITEM_TYPE_COSTUME and subType == item.COSTUME_TYPE_WEAPON:
			...
			# Zoom
			(V3Eye, V3Target) = constInfo.GetCharTypeWeaponCamera(char_type, item.GetValue(3))
			if len(V3Eye) and len(V3Target):
				self.wikiRenderTarget.SetModelV3Eye(*V3Eye)
				self.wikiRenderTarget.SetModelV3Target(*V3Target)
		...
		# Costume hair
			...
			# Zoom
			(V3Eye, V3Target) = constInfo.GetCharTypeHairCamera(char_type)
			if len(V3Eye) and len(V3Target):
				self.wikiRenderTarget.SetModelV3Eye(*V3Eye)
				self.wikiRenderTarget.SetModelV3Target(*V3Target)

		elif itemType == item.ITEM_TYPE_PET and subType == item.PET_PAY:
			self.wikiRenderTarget.SetModel(item.GetValue(0))

		elif itemType == item.ITEM_TYPE_MOUNT:
			self.wikiRenderTarget.SetModel(item.GetValue(4))
...
def InitTitleBar(self):
	...
	self.titleBar = ui.BoardWithFinalCoreTitleBar7(withButton=True)
	self.titleBar.SetParent(self.baseBoard)
	self.titleBar.SetPosition(0, 0)
	self.titleBar.SetSize(self.baseBoard.GetWidth(), 0)
	self.titleBar.SetTitleName(localeInfo.WIKI_TITLENAME)
...
def BuildSearchWindow(self):
	...
	self.searchBG.LoadImage("d:/ymir work/ui/systems/wiki/searchfield.tga")
	self.searchBG.SetPosition(8, 50)
	self.searchBG.Show()

	self.searchButton = ui.Button()
	self.searchButton.SetParent(self.baseBoard)
	self.searchButton.SetUpVisual("d:/ymir work/ui/systems/wiki/button_search_normal.tga")
	self.searchButton.SetOverVisual("d:/ymir work/ui/systems/wiki/button_search_hover.tga")
	self.searchButton.SetDownVisual("d:/ymir work/ui/systems/wiki/button_search_down.tga")
	self.searchButton.SetPosition(9 + self.searchBG.GetWidth(), 50)
	self.searchButton.SAFE_SetEvent(self.StartSearch)
	self.searchButton.Show()

	self.searchEdit = ui.EditLine()
	self.searchEdit.SetParent(self.searchBG)
	self.searchEdit.SetMax(item.ITEM_NAME_MAX_LEN)
	self.searchEdit.SetSize(139, 19)
	self.searchEdit.SetPosition(4, 3)
	self.searchEdit.SetBackgroundText(localeInfo.WIKI_SEARCH_OVERLAY_TEXT)
	...
	self.searchEdit.SetUpdateEvent(ui.__mem_func__(self.OnNameInputUpdate))
	self.searchEdit.SetTabEvent(ui.__mem_func__(self.SetNameInputResult))
	...
	if INGAME_WIKI_DROPDOWN_SEARCH:
		self.searchEditHintDropDown = ui.DropDownList(self, -13, 69)
		self.searchEditHintDropDown.SetSize(width = 200)
		self.searchEditHintDropDown.SetEvent(self.SelectItem)
		self.searchEditHintDropDown.Show()

def BuildBaseMain(self):
	self.baseBoard = ui.ExpandedImageBox()
	self.baseBoard.AddFlag("attach")
	self.baseBoard.SetParent(self)
	self.baseBoard.LoadImage("D:/Ymir Work/ui/systems/wiki/bg.tga")
	self.baseBoard.SetWindowName("InGameWikiBoard")
	self.baseBoard.Show()

	self.SetSize(self.baseBoard.GetWidth(), self.baseBoard.GetHeight())

	self.tab_title_search = ui.ExpandedImageBox()
	self.tab_title_search.LoadImage("d:/ymir work/ui/tab_menu_02.tga")
	self.tab_title_search.SetParent(self.baseBoard)
	self.tab_title_search.SetPosition(9, 28)
	self.tab_title_search.Show()

	self.tab_title_search_text = ui.TextLine()
	self.tab_title_search_text.SetParent(self.tab_title_search)
	self.tab_title_search_text.SetPackedFontColor(0xFFFEE3AE)
	self.tab_title_search_text.SetWindowHorizontalAlignCenter()
	self.tab_title_search_text.SetWindowVerticalAlignCenter()
	self.tab_title_search_text.SetHorizontalAlignCenter()
	self.tab_title_search_text.SetVerticalAlignCenter()
	self.tab_title_search_text.SetText(uiScriptLocale.PRIVATESHOPSEARCH_ITEMNAME)
	self.tab_title_search_text.Show()

	if INGAME_WIKI_DROPDOWN_SEARCH:
		import uiCheckbox
		self.monsterSearch = uiCheckbox.CheckBox(self.baseBoard, 88, 73)
		self.monsterSearch.SetText(localeInfo.WIKIPEDIA_SEARCH_ONLY_MOBS)
		self.monsterSearch.SetEvent(self.ClearNameInput)
		self.monsterSearch.SetWindowHorizontalAlignCenterFromPos()
		self.monsterSearch.Show()

	self.tab_title_category = ui.ExpandedImageBox()
	self.tab_title_category.LoadImage("d:/ymir work/ui/tab_menu_02.tga")
	self.tab_title_category.SetParent(self.baseBoard)
	self.tab_title_category.SetPosition(9, 100)
	self.tab_title_category.Show()

	self.tab_title_category_text = ui.TextLine()
	self.tab_title_category_text.SetParent(self.tab_title_category)
	self.tab_title_category_text.SetPackedFontColor(0xFFFEE3AE)
	self.tab_title_category_text.SetWindowHorizontalAlignCenter()
	self.tab_title_category_text.SetWindowVerticalAlignCenter()
	self.tab_title_category_text.SetHorizontalAlignCenter()
	self.tab_title_category_text.SetVerticalAlignCenter()
	self.tab_title_category_text.SetText(uiScriptLocale.WIKI_CONTENT_INFO)
	self.tab_title_category_text.Show()
	...
	self.scrollBar.SetSize(6, categoryPeakWindowSize[1] - 10)
	self.scrollBar.Show()
