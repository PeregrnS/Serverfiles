...
import titles_helper as TitleHelper

# True = no loading-screen images / locale loading*.sub (pack-only, no C++ define).
ROOT_PACK_MINIMAL_LOADING_NO_IMAGES = True

def _use_minimal_loading():
	return ROOT_PACK_MINIMAL_LOADING_NO_IMAGES

_g_perfFratelloSelectPreloadSlot = -1
_g_perfFratelloSelectCommonDone = False

def PerfFratelloResetSelectPreload():
	global _g_perfFratelloSelectPreloadSlot, _g_perfFratelloSelectCommonDone
	if not app.ENABLE_PERFORMANCE_FRATELLO:
		return
	_g_perfFratelloSelectPreloadSlot = -1
	_g_perfFratelloSelectCommonDone = False

def PerfFratelloPreloadSelectAssets(slot):
	global _g_perfFratelloSelectPreloadSlot, _g_perfFratelloSelectCommonDone
	if not app.ENABLE_PERFORMANCE_FRATELLO:
		return
	cid = net.GetAccountCharacterSlotDataInteger(slot, net.ACCOUNT_CHARACTER_SLOT_ID)
	if not cid:
		return
	if not _g_perfFratelloSelectCommonDone:
		_g_perfFratelloSelectCommonDone = True
	...
	if _g_perfFratelloSelectPreloadSlot == slot:
		return
	_g_perfFratelloSelectPreloadSlot = slot
	race = net.GetAccountCharacterSlotDataInteger(slot, net.ACCOUNT_CHARACTER_SLOT_RACE)
	if hasattr(net, "ACCOUNT_CHARACTER_SLOT_SKILL_GROUP"):
		group = net.GetAccountCharacterSlotDataInteger(slot, net.ACCOUNT_CHARACTER_SLOT_SKILL_GROUP)
	else:
		group = 0
...
class LoadingWindow(ui.ScriptWindow):
	...
	def Open(self):
		...
		if _use_minimal_loading():
			self._perfMinimalLoad = True
			self.loadingGage = 0
			self.errMsg = 0
			self.loadStepList = []
			self.loadingImage = 0
	...
	def Close(self):
		...
		self.loadingGage=0
		self.errMsg=0
		if not getattr(self, "_perfMinimalLoad", False):
	...
	def __RegisterSpecialTitles(self):
		for i in xrange(TitleHelper.GetTitlesLength()):
			chrmgr.RegisterSpecialTitle(i + 1, TitleHelper.GetSpecialTitle(i + 1), 0)

	def __LoadDataImmediate(self):
		self.__RegisterDungeonMapName()
		self.__RegisterSkill()
		self.__RegisterTitleName()
		self.__RegisterColor()
		self.__RegisterSpecialTitles()
	...
	def LoadData(self, playerX, playerY):
		...
		if _use_minimal_loading():
			try:
				self.__LoadDataImmediate()
			except:
				if self.errMsg:
					self.errMsg.Show()
				self.loadStepList=[]
				import dbg
				dbg.TraceError(" !!! Failed to load game data : minimal/immediate path")
				app.Exit()
				return
	...
	def __RegisterColor(self):
		...
		if app.ENABLE_ATLASS_EXTENDED:
			...
			NAME_COLOR_DICT[chrmgr.NAMECOLOR_STONES] = colorInfo.CHR_NAME_RGB_STONES

		if app.ENABLE_GAME_MASTER_CHARNAME_COLOR:
			NAME_COLOR_DICT.update( {chrmgr.NAMECOLOR_GM : (238,232,170)} )
