...
class LoginWindow(ui.ScriptWindow):
	...
	def Open(self):
		...
		if app.ENABLE_PIN_LOGIN:
			self.loginFailureMsgDict["WRONGPIN"] = localeInfo.LOGIN_FAILURE_WRONG_PIN_CODE

		if app.ENABLE_HWID_BAN:
			self.loginFailureMsgDict["HWIDBAN"] = localeInfo.LOGIN_FAILURE_BLOCK_ID
			# self.loginFailureFuncDict["HWIDBAN"] = app.Exit

		if app.ENABLE_MAINTENANCE_SYSTEM:
			self.loginFailureMsgDict["NOSERVER"] = "Server under maintenance!"
	...
	if constInfo.ENABLE_SAVE_ACCOUNT:
		def SAB_LoadAccountData(self):
			...
			for idx in xrange(constInfo.SAB.slotCount):
				if constInfo.SAB.storeType == constInfo.SAB.ST_REGISTRY:
					...
					pin = constInfo.GetWinRegKeyValue(constInfo.SAB.regPath, constInfo.SAB.regName % (idx, constInfo.SAB.regValuePin))
					if id and pwd and pin:
						self.SAB_SetAccountData(idx, (id, pwd, pin))
				elif constInfo.SAB.storeType == constInfo.SAB.ST_FILE:
					(id, pwd, pin) = constInfo.GetJsonSABData(idx)
					if id and pwd and pin:
						self.SAB_SetAccountData(idx, (id, pwd, pin))
			...
			for idx in xrange(constInfo.SAB.slotCount):
				if constInfo.SAB.storeType == constInfo.SAB.ST_REGISTRY:
					...
					if _tSlot:
						...
						constInfo.SetWinRegKeyValue(constInfo.SAB.regPath, constInfo.SAB.regName % (idx, constInfo.SAB.regValuePin), pin)
					else:
						...
						constInfo.DelWinRegKeyValue(constInfo.SAB.regPath, constInfo.SAB.regName % (idx, constInfo.SAB.regValuePin))
				elif constInfo.SAB.storeType == constInfo.SAB.ST_FILE:
					_tSlot = self.SAB_GetAccountData(idx)
					if _tSlot:
		...
		def SAB_Click_Save(self, slot):
			...
			try:
				...
				return
			self.SAB_SetAccountData(slot, (id,pwd,pin))
	...
	def __OpenLoginBoard(self):
		...
		if app.loggined:
			if app.ENABLE_PIN_LOGIN:
				self.Connect(self.id, self.pwd, self.pin)
			else:
