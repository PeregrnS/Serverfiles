...
APP_TITLE = "Hardcore © | The World is ours..."
...
FN_WIKI = "%s/wiki/" % app.GetLocalePath()
...
if app.__BL_MULTI_LANGUAGE__:
	...
	def ReloadLocaleFile():
		...
		LOCALE_GAME_FILE_NAME = "%s/locale_game.txt" % app.GetLocalePath()
		LOCALE_GAME_EX_FILE_NAME = "%s/locale_game_ex.txt" % app.GetLocalePath()
		...
		LoadLocaleFile(LOCALE_GAME_FILE_NAME)
		LoadLocaleFile(LOCALE_GAME_EX_FILE_NAME)
...
if app.__BL_MULTI_LANGUAGE__:
	...
	LoadLocaleFile(LOCALE_GAME_FILE_NAME, locals())
	LoadLocaleFile(LOCALE_GAME_EX_FILE_NAME, locals())
...
if app.BL_PARTY_UPDATE or app.ENABLE_DUNGEON_INFO_SYSTEM:
	MINIMAP_ZONE_NAME_DICT_BY_IDX = {
		...
		60 : GM_MAP_S3,
		61 : GM_MAP_S2,
		62 : MAP_WHITEDRAGONCAVE_BOSS,
		63 : MAP_N_DESERT_01,
		64 : MAP_ELEMENTAL_04_REWORK,
		65 : MAP_MILGYO,
		66 : MAP_DEVILTOWER1,
		67 : MAP_TRENT,
		68 : MAP_TRENT02,
		...
		132 : GUILD_WAR3,
		133 : GUILD_WAR4,
		160 : RIDACK_FIREDUNGEON,
		170 : MAP_RIDACKFRAMMAP2,
		174 : PLECHITO_RAMADAN2024_DUNGEON,
		176 : PLECHITO_EGYPT_DUNGEON_01,
		180 : METIN_TEST,
		181 : MAP_EMPIREWAR01,
		182 : MAP_EMPIREWAR02,
		183 : MAP_EMPIREWAR03,
...
if app.ENABLE_RENEWAL_SHOPEX:
	...
	if app.ENABLE_GEM_SYSTEM:
		SHOP_ERROR_DICT["NOT_ENOUGH_MONEY_GEM"] = SHOP_NOT_ENOUGH_GEM
...
def GetTitleAchievement(uniqueId):
	return globals()["NAME_TITLE_%d" % (uniqueId)]
...
if app.ENABLE_RENEWAL_SHOPEX:
	def NumberToShopGem(n):
		if n <= 0 :
			return "0 %s" % (MONETARY_UNIT_GEM)
		n = str(n)
		return "%s %s" % ('.'.join([ i-3<0 and str(n)[:i] or str(n)[i-3:i] for i in range(len(str(n))%3, len(str(n))+1, 3) if i ]), MONETARY_UNIT_GEM)
