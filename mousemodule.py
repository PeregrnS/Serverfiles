...
class CMouseController(object):
	...
	def AttachObject(self, Owner, Type, SlotNumber, ItemIndex, count = 0):
		...
		try:
			...
			if Type == player.SLOT_TYPE_INVENTORY or\
				...
				Type == player.SLOT_TYPE_GUILDSTORAGE or\
			...
			elif app.ENABLE_SWITCHBOT_SYSTEM and Type == player.SLOT_TYPE_SWITCHBOT:
				item.SelectItem(self.AttachedItemIndex)
				self.AttachedIconHandle = item.GetIconInstance()
			...
			elif app.ENABLE_GUILD_SAFEBOX and Type == player.SLOT_TYPE_GUILD_SAFEBOX:
				item.SelectItem(self.AttachedItemIndex)
				self.AttachedIconHandle = item.GetIconInstance()
	...
	def DeattachObject(self):
		...
		if self.AttachedIconHandle != 0:
			...
			elif app.ENABLE_SWITCHBOT_SYSTEM and self.AttachedType == player.SLOT_TYPE_SWITCHBOT:
				item.DeleteIconInstance(self.AttachedIconHandle)

			elif app.ENABLE_GUILD_SAFEBOX and self.AttachedType == player.SLOT_TYPE_GUILD_SAFEBOX:
				item.DeleteIconInstance(self.AttachedIconHandle)
