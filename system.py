...
if app.ENABLE_SYSERR_VIEWER and not getattr(sys, "_FW_STD_HOOKED", False):
	import traceback

	def __force_open_syserr_on_boot():
		try:
			import uisyserr
			import wndMgr
	...
	sys._FW_STD_HOOKED = True

	_ORIG_STDOUT = sys.stdout
	_ORIG_STDERR = sys.stderr

	class FWTraceStdout(object):
		def write(self, msg):
			try:
	...
	class FWTraceStderr(object):
		def write(self, msg):
			...
			__force_open_syserr_on_boot()

	sys.stdout = FWTraceStdout()
	sys.stderr = FWTraceStderr()

	_orig_excepthook = sys.excepthook
	def _fw_excepthook(exctype, value, tb):
		try:
