...
class CombineWindow(ui.ScriptWindow):
	...
	if app.WJ_ENABLE_TRADABLE_ICON:
		...
		def CantCombineItem(self, slotIndex):
			ItemVNum = player.GetItemIndex(slotIndex)
			if ItemVNum == 0:
				return False
				
			item.SelectItem(ItemVNum)
			if not (item.GetItemType() == item.ITEM_TYPE_COSTUME and item.GetItemSubType() == item.COSTUME_TYPE_ACCE):
				return True
...
class AbsorbWindow(ui.ScriptWindow):
	...
	def Open(self):
		if app.WJ_ENABLE_TRADABLE_ICON:
			self.interface.SetOnTopWindow(player.ON_TOP_WND_ABSORB_PHASE1)
			self.interface.RefreshMarkInventoryBag()
		self.PositionOut = 0
		(self.PositionStartX, self.PositionStartY, z) = player.GetMainCharacterPosition()
		for i in xrange(acce.WINDOW_MAX_MATERIALS + 1):
	...
	def Refresh(self, iAct):
		...
		if app.WJ_ENABLE_TRADABLE_ICON:
			(isHere1, iCell1) = acce.GetAttachedItem(0)
			if isHere1:
				self.interface.SetOnTopWindow(player.ON_TOP_WND_ABSORB_PHASE2)
			else:
				self.interface.SetOnTopWindow(player.ON_TOP_WND_ABSORB_PHASE1)
			# self.interface.RefreshMarkInventoryBag()
	...
	if app.WJ_ENABLE_TRADABLE_ICON:
		...
		def CantAbsorbPhaseOne(self, slotIndex):
			ItemVNum = player.GetItemIndex(slotIndex)
			if ItemVNum == 0:
				return False
		...
		def CantAbsorbPhaseTwo(self, slotIndex):
			(isHere1, iCell1) = acce.GetAttachedItem(0)
			ItemVNum = player.GetItemIndex(slotIndex)
			if ItemVNum == 0:
				return False
