...
class Attr67AddWindow(ui.ScriptWindow):
	...
	def CantAttachToAttrSlot(inven_slot, bAdd):
		...
		if (attrCount < 5):
			chat.AppendChat(chat.CHAT_TYPE_INFO, "The item must have at least 5 bonuses to be able to add a 6/7.")
	...
	def GetMaterialVnum(self, item_level):
		if item_level >= 0 and item_level <= 150:
			return 39070
	...
	def AutoSetItem(self, inven, itemCount):
		...
		if player.IsEquipmentSlot(invenPos):
			return
		...
		if itemVnum == 72064 or itemVnum == 72065 or itemVnum == 72066 or itemVnum == 72067:
			if self.RegistSlotIndex == -1:
				return
			if itemVnum not in Attr67AddWindow.SUPPORT_DICT:
				return

			self.SupportCountText.SetText("1")
			self.SupportSlot.SetItemSlot(0, itemVnum)
			self.SupportSlot.RefreshSlot()
			self.SupportSlotIndex = invenPos

			self.__CalculateTotalSuccessPer()
		else:
			if not Attr67AddWindow.CantAttachToAttrSlot(invenPos, True):
				return
			MaterialVnum = self.GetMaterialVnum(level_limit)
			self.MaterialSlot.SetItemSlot(0, MaterialVnum)
			self.MaterialSlot.RefreshSlot()
			self.MaterialCountText.SetText("1")
			self.MaterialSlotIndex = MaterialVnum

			self.RegistSlot.SetItemSlot(0, itemVnum)
			self.RegistSlot.RefreshSlot()
			self.RegistSlotIndex = invenPos
