...
class DungeonInfoWindow(ui.ScriptWindow):
	class ItemGrid(object):
		...
		def Put(self, pos, width, height):
			# import dbg
			# dbg.TraceError("Put on pos {} width {} height {}".format(pos, width, height))
	...
	def Initialize(self):
		...
		if dungeonInfo.GetCount() > 0:
			...
			# chat.AppendChat(7, "DungeonI Count %d" % dungeonInfo.GetCount())
	...
	def OnUpdate(self):
		...
		if self.previewTools:
			renderTarget.MouseZoom(RENDER_BACKGROUND_INDEX, self.previewZoom)
	...
	def __OnLoadInformation(self, key, reloadRender = False):
		self.vnum = dungeonInfo.GetBossVnum(key)
	...
	def __BindObject(self):
		...
		#self.shopButton = Child("ShopButton")
	...
	def __BindEvent(self):
		...
		# 	self.elementalImg.SAFE_SetStringEvent("mouse_over_in", self.__OnOverElementImg)
		# 	self.elementalImg.SAFE_SetStringEvent("mouse_over_out", self.__OnOverOutElementImg)

		# Rank Buttons.
		# if self.rankScoreButton:
		# 	self.rankScoreButton.SetEvent(lambda arg = RANK_SCORE : self.__OnClickRankingButton(arg))

		# if self.rankTimeButton:
		# 	self.rankTimeButton.SetEvent(lambda arg = RANK_TIME : self.__OnClickRankingButton(arg))

		# if self.rankDamageButton:
		# 	self.rankDamageButton.SetEvent(lambda arg = RANK_DAMAGE : self.__OnClickRankingButton(arg))
		...
		# Box icon button.
		if self.boxButton:
			self.boxButton.SetEvent(ui.__mem_func__(self.__OnWikiBossPage))

		#if self.shopButton:
		#	self.shopButton.SetEvent(ui.__mem_func__(self.__OnOpenShop))
	...
	def __OnWarpButton(self):
		...
		self.questionDialog.Open(5)
		self.questionDialog.SetText1(localeInfo.DUNGEON_INFO_DO_YOU_TELEPORT % mapName)
		...
		# self.questionDialog.SetTimeOverEvent()
	...
	def __OnWikiBossPage(self):
		try:
			import constInfo
			if not constInfo._interface_instance.wndWiki.IsShow():
				constInfo._interface_instance.OpenWikiWindow()

			constInfo._interface_instance.wndWiki.CloseBaseWindows()
			constInfo._interface_instance.wndWiki.OpenSpecialPage(None, self.vnum, True)
			constInfo._interface_instance.wndWiki.SetTop()
			return
	...
	def RefreshBoxInventory(self):
		...
		if bossDropCount != 0:
			for slotPos in xrange(self.boxItemSlot.GetSlotCount()):
				...
				itemVnum = dungeonInfo.GetBossDropItemVnum(key, slotPos)
				itemCount = dungeonInfo.GetBossDropItemCount(key, slotPos)
				if itemCount <= 1:
					itemCount = 0

				if itemVnum != 0:
					# import dbg
					# dbg.TraceError("itemVnum %d" % itemVnum)
					...
					# dbg.TraceError("update pos {} itemVnum {} itemCount {}".format(pos, itemVnum, itemCount))
					self.boxItemSlot.SetSlot(pos, 0, width, height, itemIcon, (1.0, 1.0, 1.0, 0.5))
					self.boxItemSlot.SetSlotCount(pos, itemCount)
					self.boxItemSlot.HideSlotBaseImage(pos)
				else:
					self.boxItemSlot.ClearSlot(pos)
					self.boxItemSlot.ShowSlotBaseImage(pos)
