...
class GuildWindow(ui.ScriptWindow):
	...
	if app.ENABLE_ALLOW_GUILD_SAFEBOX:
		GRADE_SAFEBOX_AUTHORITY = 5
	...
	def __MakeInfoPage(self):
		...
		try:
			...
			page.gsButton = page.GetChild("GuildStorageButton")
			...
			page.gsButton.SetEvent(ui.__mem_func__(self.__OnClickSelectGuildStorageButton))
	...
	def __MakeGradePage(self):
		...
		for i in xrange(15):
			...
			if app.ENABLE_ALLOW_GUILD_SAFEBOX:
				...
				if localeInfo.IsARABIC():
					safeboxAuthorityCheckBox = CheckBox(page, 14+57, yPos, event)
				else:
					safeboxAuthorityCheckBox = CheckBox(page, 352, yPos, event)
	...
	def RefreshGuildGradePage(self):
		...
		for key, slotList in page.gradeDict.items():
			...
			if app.ENABLE_ALLOW_GUILD_SAFEBOX:
				slotList[self.GRADE_SAFEBOX_AUTHORITY].SetCheck(authority & guild.AUTH_SAFEBOX)
	...
	def __OnClickSelectGuildStorageButton(self):
		net.SendChatPacket("/open_gs")
