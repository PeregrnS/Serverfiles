...
class ItemshopWindow(ui.BoardWithFinalCoreTitleBar5):
	BOARD_WIDTH = 800
	BOARD_HEIGHT = 553
	...
	def __init__(self):
		ui.BoardWithFinalCoreTitleBar5.__init__(self)
	...
	def __del__(self):
		ui.BoardWithFinalCoreTitleBar5.__del__(self)
	...
	# 	self.SetCoins(player.GetCoins())

	def SetCoins(self):
		# self.coinText.SetText('< %s >' % localeInfo.DottedNumber(coins))
		self.coinText.SetText('< %s >' % localeInfo.DottedNumber(player.GetCoins()))
...
class ItemSlot(ui.ImageBox):
	...
	def OnMouseRightButtonDown(self):
		if app.IsPressed(app.DIK_LCONTROL):
			if item.HasDropInfo(self.itemVnum) and self.interface:
				self.interface.OpenChestDropWindow(self.itemVnum)
			return

		if app.ENABLE_RENDER_TARGET_PREVIEW and app.IsPressed(app.DIK_LSHIFT) and self.interface:
			self.interface.OpenRenderTarget(self.itemVnum)
			return
	...
	def ShowToolTip(self):
		if self.toolTip:
			...
			if app.ENABLE_RENDER_TARGET_PREVIEW:
				self.toolTip.AppendRenderingTooltip(self.itemVnum)
			if app.ENABLE_CHEST_DROP_INFO:
				self.toolTip.AppendChestDropInfo(self.itemVnum)
	...
	def UpdatePriceValue(self):
		...
		self.limitedCountText.SetText(ITEMSHOP_INFO['strings']['ITEMS_LEFT'] % localeInfo.DottedNumber(self.limitCount))
...
class MiddleBar(ui.DragButton):
	def __init__(self,
		...
		self.SetOnRunMouseWheelEvent(wheel_event)
		self.Show()
	...
	def SetOnRunMouseWheelEvent(self, onMouseWheel):
		self.SetOnRunMouseWheelEvent = onMouseWheel
...
class ListBox(ui.Window):
	...
	def AppendItem(self, newItem):
		...
		newItem.OnRunMouseWheel = self.OnRunMouseWheel
