...
def GetServerID():
	...
	for i, server_data in enumerate(serverInfo.SRV_LIST[0]):
...
class MoveChannelWindow(ui.ScriptWindow):
	...
	def GetChannelCount(self):
		return len(serverInfo.SRV_LIST[0][0][3])
	...
	def __AddChannelButtons(self):
		...
		for i in xrange(self.GetChannelCount()):
			channel_data = serverInfo.SRV_LIST[0][0][3][i]
			...
			self.channelButtonList[i].SetText(channel_name)
