...
if app.ENABLE_RENDER_TARGET and app.ENABLE_QUEST_RENDER_EVENT:
	import renderTarget
...
class QuestDialog(ui.ScriptWindow):
	...
	if app.ENABLE_RENDER_TARGET and app.ENABLE_QUEST_RENDER_EVENT:
		def OnRenderMob(self, src):
			self.modelRenderer =  ui.RenderTarget()
			self.modelRenderer.SetParent(self.board)
			self.modelRenderer.SetSize(190,210)
			self.modelRenderer.SetWindowHorizontalAlignCenter()
			self.modelRenderer.SetWindowVerticalAlignCenter()
			self.modelRenderer.SetPosition(0, 40)
			...
			renderTarget.SelectModel(self.RENDER_TARGET_INDEX, src)
