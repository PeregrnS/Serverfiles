...
class SafeboxWindow(ui.ScriptWindow):
	...
	if app.ENABLE_SAVE_LAST_WINDOW_POSITION:
		def SetLastPosition(self):
			try:
				file = open("lib/user_data/wnd/" + player.GetName() + "/"+ "safebox" + ".pos", 'r')
		...
		def SaveLastPosition(self):
			...
			file = open("lib/user_data/wnd/" + player.GetName() + "/"+ "safebox" + ".pos","w")
...
if app.ENABLE_GUILD_SAFEBOX:
	class GuildSafeboxWindow(ui.ScriptWindow):
		...
		def OpenPickMoneyDialog(self):
			chat.AppendChat(chat.CHAT_TYPE_INFO, "Option disabled for now.")
			return
			# if mouseModule.mouseController.isAttached():
			...
			# else:
			# 	curMoney = safebox.GetGuildMoney()
		...
		def RefreshGuildSafeboxMoney(self):
			self.wndMoney.SetText(str(safebox.GetGuildMoney()))
