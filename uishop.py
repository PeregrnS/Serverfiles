...
class ShopDialog(ui.ScriptWindow):
	...
	def Open(self, vid, tabCount = 0):
		...
		if isPrivateShop and not tabCount:
			self.__HideMiddleTabs()
			self.__HideSmallTabs()
		else:
	...
	def UnselectItemSlot(self, selectedSlotPos):
		realSelectedSlotPos = self.GetIndexFromSlotPos(selectedSlotPos)
		# import dbg
		# dbg.TraceError("selectedPos %d" % realSelectedSlotPos)
		if app.ENABLE_RENDER_TARGET_PREVIEW or app.ENABLE_CHEST_DROP_INFO:
			itemIndex = shop.GetItemID(realSelectedSlotPos)
		...
		if app.ENABLE_RENDER_TARGET_PREVIEW:
			...
			for i in xrange(player.METIN_SOCKET_MAX_NUM):
				metinSlot.append(shop.GetItemMetinSocket(itemIndex, i))
			if app.IsPressed(app.DIK_LSHIFT):
				self.tooltipItem.ModelPreviewFull(itemIndex, metinSlot)
				return

		# if shop.IsPrivateShop():
		# 	self.AskBuyItem(selectedSlotPos)
		# else:
		net.SendShopBuyPacket(realSelectedSlotPos)
	...
	def OverInItem(self, slotIndex):
		slotIndex = self.GetIndexFromSlotPos(slotIndex)
		...
		if 0 != self.tooltipItem:
			...
			if app.ENABLE_EMOJI_SYSTEM and app.ENABLE_CHEST_DROP_INFO:
				self.tooltipItem.AppendChestDropInfo(itemIndex)
