...
def cfgGet(notused, line):
	config = ConfigParser.RawConfigParser()
	configFile = 'lib/userdata/skill_color/'+chr.GetName()
	config.read(configFile)

	if not config or not config.has_section('SKILL_COLOR'):
		return ""

	if not config or not config.has_option('SKILL_COLOR', line):
		return ''

	return config.get('SKILL_COLOR', line)

def cfgSet(notused, line, text):
	config = ConfigParser.RawConfigParser()
	config.add_section('SKILL_COLOR')

	config.set('SKILL_COLOR', line, text)

	with open('lib/userdata/skill_color/'+chr.GetName(), 'wb') as configfile:
...
class SkillColorWindow(ui.ScriptWindow):
	...
	def GetSavedColorPresets(self, skillSlot):
		skillSlotColorCfg = cfgGet(SAVE_SKILL_COLOR, str(skillSlot))
		if -1 == skillSlotColorCfg:
			return
	...
	def OnClickSaveColorButton(self):
		...
		skillColorCfg = cfgGet(SAVE_SKILL_COLOR, str(self.skillSlot))
		...
		if len(skillColorCfg) < HEX_CODE_LENGTH:
			cfgSet(SAVE_SKILL_COLOR, str(self.skillSlot), hexCode)
		else:
			cfgSet(SAVE_SKILL_COLOR, str(self.skillSlot), skillColorCfg + " " + hexCode)
	...
	def OnClickClearColorButton(self):
		...
		cfgSet(SAVE_SKILL_COLOR, str(self.skillSlot), skillColorCfgStr)
