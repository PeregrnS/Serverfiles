...
ATTR_SLOT_COUNT = 5 + 2
MAIN_PANEL_HEIGHT += INPUT_HEIGHT * 2
...
class SwitchbotItemTab(ui.ThinBoardCircle):
	...
	def OverInItem(self, overSlotPos):
		...
		# self.tooltipItem.AddItemData(itemVnum, metinSlot, attrSlot, 0, 0, player.INVENTORY, self.slot_num)
		...
		if self.IsEmpty():
			...
			if self.IsActive():
				if app.ENABLE_HIGHLIGHT_SLOT_NEW_EFFECT:
					self.itemSlot.ActivateSlotEffect(0)
				elif not app.ENABLE_CHANGE_LOOK_SYSTEM:
					self.itemSlot.ActivateSlot(0)

				self.statusBar.SetColor(COLOR_ACTIVE)
			else:
				if app.ENABLE_HIGHLIGHT_SLOT_NEW_EFFECT:
					self.itemSlot.DeactivateSlotEffect(0)
				elif not app.ENABLE_CHANGE_LOOK_SYSTEM:
...
class SwitchbotAttribute(ui.Window):
	...
	def Create(self, width, index):
		...
		if localeInfo.IsARABIC():
			self.valueEditLine.SetPosition(-4, 7)
		else:
	...
	def __OnEditLineIMEUpdate(self):
		...
		if len(text) > 0 and text.isdigit():
			...
			maxValue = 0
			if not ADDON_TYPES.has_key(curType) and self.index < 5:
				maxValue = switchbot.GetAttributeMaxValue(self.selectedSlot, curType, self.index)
...
class SwitchbotWindow(ui.BoardWithFinalCoreTitleBar2):
	def __init__(self):
		ui.BoardWithFinalCoreTitleBar2.__init__(self)
	...
	def __del__(self):
		ui.BoardWithFinalCoreTitleBar2.__del__(self)
	...
	def __LoadWindow(self):
		...
		if localeInfo.IsARABIC():
			self.alternativeTextLine.SetHorizontalAlignLeft()
			self.alternativeTextLine.SetPosition(210, -1)
		else:
		...
		## Info
		# self.switchBotInfo = ui.Button()
		# self.switchBotInfo.SetParent(self)
		# self.switchBotInfo.SetUpVisual("d:/ymir work/ui/game/mobdrop/question_mark_1.tga")
		# self.switchBotInfo.SetOverVisual("d:/ymir work/ui/game/mobdrop/question_mark_2.tga")
		# self.switchBotInfo.SetDownVisual("d:/ymir work/ui/game/mobdrop/question_mark_3.tga")
		# self.switchBotInfo.SetPosition(352, 11)
		# self.switchBotInfo.Show()
		
		# self.tlInfo = uiToolTip.ItemToolTip()
		# self.tlInfo.Hide()
		# self.tooltipInfo = [self.tlInfo]*7
		# self.InformationText = [localeInfo.SWITCHBOT_T_TITLE,
		# 						localeInfo.SWITCHBOT_T_TEXT1,
		# 						localeInfo.SWITCHBOT_T_TEXT2,
		# 						localeInfo.SWITCHBOT_T_TEXT3,
		# 						localeInfo.SWITCHBOT_T_TEXT4,
		# 						localeInfo.SWITCHBOT_T_TEXT5,
		# 						localeInfo.SWITCHBOT_T_TEXT6
		# ]
	...
	def __OnSearchAttribute(self, attrIdx):
		...
		if firstAttrType in ADDON_TYPES.keys() and self.attrSearchIdx < 5:
		...
		attributes = switchbot.GetAttributesForSet(self.selectedSlot, self.attrSearchIdx)
	...
	def __OnAcceptSearch(self, key, value):
		...
		if not maxValue:
			maxValue = switchbot.GetAttributeMaxValue(self.selectedSlot, key, self.attrSearchIdx)
	...
	class SearchListItem(ui.Window):
		...
		def __init__(self, key, value):
			...
			if localeInfo.IsARABIC():
				...
				self.textLine.SetPosition(221, -1)
			else:
...
class ConfigurationWindow(ui.BoardWithTitleBar):
	...
	class ConfigurationLine(ui.Window):
		...
		def __init__(self, width):
			...
			if localeInfo.IsARABIC():
				self.nameTextLine.SetHorizontalAlignLeft()
				self.nameTextLine.SetPosition(178, -1)
			else:
	...
	def LoadConfigurations(self):
		try:
			self.dict = pickle.load(open("lib/encodings/switchbot.pkl", 'rb'))
		except IOError:
			self.dict = {}
	...
	def SaveConfigurations(self):
		with open("lib/encodings/switchbot.pkl", 'wb') as f:
