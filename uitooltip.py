...
if app.ENABLE_EMOJI_SYSTEM or app.ENABLE_RENDER_TARGET_PREVIEW:
...
class ToolTip(ui.ThinBoard):
	...
	if app.ENABLE_EMOJI_SYSTEM:
		SPECIAL_COLOR_TEXT = grp.GenerateColor(1.0,1.0,0.5,1.0)
	if app.ENABLE_GOLD_MAX_EXTENDED:
		EXPENSIVE_SHIT_PRICE_COLOR = 0xFFDA7C20
	if app.BL_67_ATTR:
	...
	if app.ENABLE_EVENT_MANAGER:
		def AutoAppendNewTextLineResize(self, text, color = FONT_COLOR, centerAlign = True, boldFont = False):
			textLine = ui.TextLine()
			textLine.SetParent(self)
			if boldFont:
				textLine.SetFontName(localeInfo.UI_DEF_BOLD_FONT)
			else:
				textLine.SetFontName(self.defFontName)
			textLine.SetPackedFontColor(color)
			textLine.SetText(text)
...
class ItemToolTip(ToolTip):
	...
	if app.ENABLE_EXTENDED_BLEND:
		AFFECT_RANGE_DICT = {
			...
			item.APPLY_ENERGY : localeInfo.TOOLTIP_RANGE_APPLY_ENERGY,
		}
	...
	if app.ENABLE_RENEWAL_SHOPEX:
		def SetShopItemByShopEx(self, slotIndex, type):
			...
			if type == shop.SHOPEX_ITEM:
				for x in xrange(shop.SHOP_MAX_ITEM_NUM):
					self.AddSHOPEXItem(shop.GetItemVnumItem(slotIndex, x), shop.GetItemVnumPrice(slotIndex, x))
			elif app.ENABLE_GEM_SYSTEM and type == shop.SHOPEX_GEM:
				self.AppendGemPrice(price)
			else:
				self.AppendTextLine(localeInfo.TOOLTIP_BUYPRICE  % (localeInfo.NumberToShopEXP(price)), self.GetPriceColor(price))

		def AddSHOPEXItem(self, vnum, count):
			if 0 == vnum:
				return
			...
			if localeInfo.IsARABIC():
				textLineName.SetHorizontalAlignRight()
			textLineName.Show()
		...
		if app.ENABLE_GEM_SYSTEM:
			def AppendGemPrice(self, price):
				self.AppendSpace(5)
				self.AppendTextLine(localeInfo.CHEQUE_SYSTEM_SELL_PRICE, self.SPECIAL_COLOR_TEXT)
				self.AppendTextLine((localeInfo.NumberToShopGem(price)), self.SPECIAL_COLOR_TEXT)
	...
	if app.ENABLE_RENDER_TARGET_PREVIEW or app.ENABLE_CHEST_DROP_INFO:
		def AddItemRender(self, itemVnum, metinSlot):
			if app.ENABLE_CHEST_DROP_INFO and item.HasDropInfo(itemVnum):
				previewBtn = ui.Button()
				previewBtn.SetParent(self)
				previewBtn.SetUpVisual("d:/ymir work/ui/pattern/q_mark_01.tga")
				previewBtn.SetOverVisual("d:/ymir work/ui/pattern/q_mark_02.tga")
				previewBtn.SetDownVisual("d:/ymir work/ui/pattern/q_mark_01.tga")
				previewBtn.SetPosition(2,2)
				previewBtn.Show()
				previewBtn.SetToolTipText(localeInfo.CHEST_DROP_INFO)
				previewBtn.SetEvent(ui.__mem_func__(self.__ChestDropInfo), itemVnum)
				self.childrenList.append(previewBtn)
			elif systemSetting.IsPreviewModel() and constInfo.IsCanRender(itemVnum):
				previewBtn = ui.Button()
				previewBtn.SetParent(self)
				previewBtn.SetUpVisual("d:/ymir work/ui/pattern/q_mark_01.tga")
				previewBtn.SetOverVisual("d:/ymir work/ui/pattern/q_mark_02.tga")
				previewBtn.SetDownVisual("d:/ymir work/ui/pattern/q_mark_03.tga")
				previewBtn.SetPosition(2,2)
				previewBtn.Show()
				previewBtn.SetToolTipText(localeInfo.INFO_PREVIEW)
				previewBtn.SetEvent(ui.__mem_func__(self.ModelPreviewFull), itemVnum, metinSlot)
				self.childrenList.append(previewBtn)

	if app.ENABLE_CHEST_DROP_INFO:
		def __ChestDropInfo(self, itemVnum):
	...
	def AddItemData(self, itemVnum, metinSlot, attrSlot = 0, flags = 0, unbindTime = 0, window_type = player.INVENTORY, slotIndex = -1):
		...
		myRingVnum = [
			65737, 65736, 71135, 71136, 71300, 71301, 71302, 71303,
			71304, 71305, 71306, 71307, 71308, 71309, 71310, 71311,
			71312, 71313, 71314, 71315, 71316, 71317, 71318, 71319,
			19043, 71410, 71411, 71412, 71413, 71414, 71415, 71416,
			71417, 71418, 71419, 19041, 71158, 99962, 96311, 96312,
			77300, 77301, 77302, 77303, 77316, 77317, 77318, 77319,
			77304, 77305, 77306, 77307, 77308, 77309, 77310, 77311,
			77312, 77313, 77314, 77315, 77922, 77923
		]
		if itemVnum in myRingVnum:
		...
		if app.ENABLE_MOUNT_SYSTEM and item.ITEM_TYPE_MOUNT == itemType:
			isCostumeItem = 1
			isCostumeMount = item.ITEM_TYPE_MOUNT == itemType

		self.__AdjustMaxWidth(attrSlot, itemDesc)
		self.__SetItemTitle(itemVnum, metinSlot, attrSlot)

		# Show item icon at hyperlink
		if flags == "hyperlink":
			if app.ENABLE_RENDER_TARGET_PREVIEW or app.ENABLE_CHEST_DROP_INFO:
				self.AddItemRender(itemVnum, metinSlot)
		...
		## item.ITEM_TYPE_BLEND
			...
			if metinSlot  and metinSlot[0] != 0:
				...
				if time > 0:
					...
					self.AppendTextLine(localeInfo.INFINITE_USE)
				...
				if affectDurMin == affectDurMax and not affectDurMin == 0:
					timeString = localeInfo.TOOLTIP_TIME % localeInfo.SecondToDHM(affectDurMin)
				elif affectDurMin == 0:
					timeString = localeInfo.INFINITE_USE
				else:
					timeString = localeInfo.TOOLTIP_TIME_RANGE % (localeInfo.SecondToDHM(affectDurMin), localeInfo.SecondToDHM(affectDurMax))
		...
		### Use ###
			...
			if item.USE_SPECIAL == itemSubType:
				...
				if app.ENABLE_ITEM_TIME_EXTENDER:
					if itemVnum >= 173010 and itemVnum <= 173014:
						self.AppendTextLine(localeInfo.TOOLTIP_TIME_CHARGER_FIX_DAYS(localeInfo.SecondToDHM(item.GetValue(0))))
					else:
						if metinSlot[2]:
		...
		elif app.ENABLE_SHINING_ITEM_SYSTEM and item.ITEM_TYPE_SHINING == itemType:
			self.__AppendAffectInformation()
			self.__AppendAttributeInformation(attrSlot)
			self.__AppendLimitInformation()
		...
		elif app.ENABLE_BOOSTER_ITEMS and item.ITEM_TYPE_BOOSTER == itemType:
			self.__AppendAffectInformation()
			self.__AppendAttributeInformation(attrSlot)
			self.__AppendLimitInformation()
	...
	def AdditionalTips(self, window_type, itemVnum, metinSlot, slotIndex):
		...
		if self.isHyperLinkItem:
			return

		if chr.IsGameMaster(0):
			self.AppendSpace(5)
			self.AppendTextLine("VNUM: {}".format(itemVnum), self.SPECIAL_COLOR_TEXT)
			self.AppendSpace(3)
			self.AppendTextLine("TYPE: {} SUBTYPE {} SLOT {}".format(itemType, itemSubType, slotIndex), self.SPECIAL_COLOR_TEXT)
			if metinSlot:
				self.AppendSpace(3)
				self.AppendTextLine("SOCKET: {}".format(','.join([str(i) for i in metinSlot])), self.SPECIAL_COLOR_TEXT)
			if app.ENABLE_MOUNT_SYSTEM and (itemType == item.ITEM_TYPE_MOUNT):
				(applyMount, applyVnum) = item.GetAffect(0)
				self.AppendSpace(5)
				self.AppendTextLine("{}".format(applyVnum), self.SPECIAL_COLOR_TEXT)

		if app.ENABLE_EMOJI_SYSTEM and item.IsFlag(item.ITEM_FLAG_STACKABLE) and window_type in (player.INVENTORY,):
			self.AppendSpace(5)
			if itemCount > 1:
				self.AppendTextLine(localeInfo.TOOLTIP_INFO_SPLIT, self.SPECIAL_COLOR_TEXT)
				self.AppendSpace(5)
				if constInfo.IsTreasureBox(itemVnum):
					if itemCount >= 1:
						self.AppendTextLine(localeInfo.TOOLTIP_INFO_OPEN_NUM_CHEST % itemCount, self.SPECIAL_COLOR_TEXT)
				self.AppendSpace(5)
				self.AppendTextLine(localeInfo.TOOLTIP_INFO_STACK_ITEM, self.SPECIAL_COLOR_TEXT)

		if app.ENABLE_INGAME_WIKI:
			if self.interface and self.interface.wndWiki.IsShow():
				self.AppendSpace(5)
				if app.ENABLE_PREMIUM_PRIVATE_SHOP and not self.interface.wndPrivateShopSearch.IsShow():
					self.AppendTextLine("|Eemoji/key_ctrl|e |Eemoji/key_lclick|e " + localeInfo.WIKIPEDIA_ITEM_SEARCH_EMOJI + " ", self.SPECIAL_COLOR_TEXT)
				else:
					self.AppendTextLine("|Eemoji/key_ctrl|e |Eemoji/key_lclick|e " + localeInfo.WIKIPEDIA_ITEM_SEARCH_EMOJI + " ", self.SPECIAL_COLOR_TEXT)

	if app.ENABLE_EMOJI_SYSTEM and app.ENABLE_RENDER_TARGET_PREVIEW:
		def AppendRenderingTooltip(self, itemVnum, isShopRelated = False):
			# if not systemSetting.IsTooltipInfo():
			# 	return

			if systemSetting.IsPreviewModel() and constInfo.IsCanRender(itemVnum):
				self.AppendSpace(5)
				# self.AppendTextLine(localeInfo.TOOLTIP_INFO_OFFLINE_PREVIEW, self.SPECIAL_COLOR_TEXT)
				self.AppendTextLine(localeInfo.TOOLTIP_INFO_PREVIEW, self.SPECIAL_COLOR_TEXT)

	if app.ENABLE_EMOJI_SYSTEM and app.ENABLE_CHEST_DROP_INFO:
		def AppendChestDropInfo(self, itemVnum, isShopRelated = False):
			# if not systemSetting.IsTooltipInfo():
			# 	return
			...
			if hasinfo:
				self.AppendSpace(5)
				# self.AppendTextLine(localeInfo.TOOLTIP_INFO_OFFLINE_CHEST_DROP, self.SPECIAL_COLOR_TEXT)
				self.AppendTextLine(localeInfo.TOOLTIP_INFO_CHEST_DROP, self.SPECIAL_COLOR_TEXT)

	if app.ENABLE_EMOJI_SYSTEM and app.ENABLE_CHECKINOUT_UPDATE:
		def AppendCheckInOutInfo(self, itemVnum):
			# if not systemSetting.IsTooltipInfo():
			# 	return

			self.AppendSpace(5)
			if self.interface and self.interface.wndSafebox and self.interface.wndSafebox.IsShow():
				self.AppendTextLine(localeInfo.TOOLTIP_INFO_STORE_ITEM, self.SPECIAL_COLOR_TEXT)
			elif self.interface and self.interface.dlgExchange and self.interface.dlgExchange.IsShow():
				self.AppendTextLine(localeInfo.TOOLTIP_INFO_EXCHANGE, self.SPECIAL_COLOR_TEXT)

	if app.ENABLE_EMOJI_SYSTEM and app.ENABLE_SWITCHBOT_SYSTEM:
		def AppendCheckInSwitchbot(self, itemVnum):
			# if not systemSetting.IsTooltipInfo():
			# 	return

			self.AppendSpace(5)
			if self.interface and self.interface.wndSwitchbot and self.interface.wndSwitchbot.IsShow():
				self.AppendTextLine(localeInfo.TOOLTIP_INFO_ADD_SWITCH_ITEM, self.SPECIAL_COLOR_TEXT)

	if app.ENABLE_EMOJI_SYSTEM and app.ENABLE_ITEM_MANAGER:
		def AppendItemManagerTooltip(self, itemVnum):
			# if not systemSetting.IsTooltipInfo():
			# 	return

			cansellitem = constInfo.CanSell(itemVnum)
			if cansellitem:
				self.AppendSpace(5)
				self.AppendTextLine(localeInfo.TOOLTIP_INFO_ITEM_MANAGER, self.SPECIAL_COLOR_TEXT)

	if app.ENABLE_EMOJI_SYSTEM and app.ENABLE_GEM_SYSTEM:
		def AppendGemBuyTooltip(self):
			self.AppendSpace(5)
			self.AppendTextLine(localeInfo.TOOLTIP_INFO_GEMSHOP_BUY, self.SPECIAL_COLOR_TEXT)
	...
	if app.ENABLE_RENDER_TARGET_PREVIEW:
		def ModelPreviewFull(self, itemVnum, metinSlot = 0):
			if not systemSetting.IsPreviewModel():
				return

			item.SelectItem(itemVnum)
			itemType = item.GetItemType()
			itemSubType = item.GetItemSubType()
			itemGetRace = self.ItemGetRace(itemVnum)

			IsPetItem = constInfo.IS_PET_SEAL(itemVnum)
			IsArmor = (itemType == item.ITEM_TYPE_ARMOR and itemSubType == item.ARMOR_BODY)
			#if app.ENABLE_QUIVER_SYSTEM:
			#	IsWeapon = (itemType == item.ITEM_TYPE_WEAPON and not itemSubType == item.WEAPON_QUIVER)
			#else:
			IsWeapon = (itemType == item.ITEM_TYPE_WEAPON and not itemSubType == item.WEAPON_ARROW)
			...
			# if app.ENABLE_MOUNT_SYSTEM:
			IsCostumeMount = (itemType == item.ITEM_TYPE_MOUNT)
			if app.ENABLE_WEAPON_COSTUME_SYSTEM:
				IsCostumeWeapon = (itemType == item.ITEM_TYPE_COSTUME and itemSubType == item.COSTUME_TYPE_WEAPON)
			if app.ENABLE_ACCE_COSTUME_SYSTEM:
				IsCostumeAcce = (itemType == item.ITEM_TYPE_COSTUME and itemSubType == item.COSTUME_TYPE_ACCE)
			# if app.ENABLE_AURA_COSTUME_SYSTEM:
			# 	IsCostumeAura = (itemType == item.ITEM_TYPE_COSTUME and itemSubType == item.COSTUME_TYPE_AURA)
			if app.ENABLE_SHINING_ITEM_SYSTEM:
				IsShiningItem = (itemType == item.ITEM_TYPE_SHINING)
			IsPolymorphItem = self.__IsPolymorphItem(itemVnum)
			IsBossScroll = constInfo.IsBossScroll(itemVnum)
			IsETC = constInfo.IsETC(itemVnum)
			IsSkillColor = itemType == 43

			## race=0, weaponVnum=0, armorVnum=0, hairVnum=0, acceVnum=0, auraVnum=0, bChangeMotion = False, bCheckPosition = False
			if IsWeapon or (app.ENABLE_WEAPON_COSTUME_SYSTEM and IsCostumeWeapon) or IsETC:
				self.interface.wndTargetRender.DisplayUser(itemGetRace, itemVnum)
				self.interface.wndTargetRender.Open()
			elif IsArmor or IsCostumeBody:
				self.interface.wndTargetRender.DisplayUser(itemGetRace, 0, itemVnum)
				self.interface.wndTargetRender.Open()
			elif IsSkillColor:
				self.interface.wndTargetRender.DisplayUser(itemGetRace, skillColor = itemVnum)
				self.interface.wndTargetRender.Open()
			elif IsCostumeHair:
				self.interface.wndTargetRender.DisplayUser(itemGetRace, 0, 0, item.GetValue(3))
				self.interface.wndTargetRender.Open()
			elif app.ENABLE_ACCE_COSTUME_SYSTEM and IsCostumeAcce:
				self.interface.wndTargetRender.DisplayUser(itemGetRace, 0, 0, 0, itemVnum)
				self.interface.wndTargetRender.Open()
			# elif app.ENABLE_AURA_COSTUME_SYSTEM and IsCostumeAura:
			# 	self.interface.wndTargetRender.DisplayUser(itemGetRace, 0, 0, 0, 0, itemVnum)
			# 	self.interface.wndTargetRender.Open()
			elif app.ENABLE_SHINING_ITEM_SYSTEM and IsShiningItem:
				self.interface.wndTargetRender.DisplayUser(itemGetRace, 0, 0, 0, 0, 0, itemVnum)
				self.interface.wndTargetRender.Open()
			elif IsCostumeMount:
				applyVnum = item.GetValue(4)
				if applyVnum != 0:
					self.interface.wndTargetRender.DisplayUser(applyVnum, bCheckPosition = True)
					self.interface.wndTargetRender.Open()
			elif IsPetItem:
				petVnum = item.GetValue(0)
				if petVnum != 0:
					self.interface.wndTargetRender.DisplayUser(petVnum, bCheckPosition = True)
					self.interface.wndTargetRender.Open()
			elif IsPolymorphItem:
				if metinSlot[0] != 0:
					self.interface.wndTargetRender.DisplayUser(metinSlot[0])
					self.interface.wndTargetRender.Open()
			elif IsBossScroll:
				bossVnum = item.GetValue(0)
				if bossVnum != 0:
					self.interface.wndTargetRender.DisplayUser(bossVnum)
					self.interface.wndTargetRender.Open()

		def ModelPreviewPolyMorph(self, socketValue):
			if not systemSetting.IsPreviewModel():
				return

			if socketValue != 0:
				self.interface.wndTargetRender.DisplayUser(socketValue, bCheckPosition = True)
				self.interface.wndTargetRender.Open()
		...
		def ItemGetRace(self, itemVnum = 0):
			...
			if app.ENABLE_WOLFMAN_CHARACTER and not item.IsAntiFlag(item.ITEM_ANTIFLAG_WOLFMAN):
				races_m.append(MALES[4])
				races_f.append(MALES[4])
	...
	def GetAffectString(self, affectType, affectValue):
		return self.__GetAffectString(affectType, affectValue)
		
	def GetAttributeColor(self, index, value):
		return self.__GetAttributeColor(index, value)
	...
	def __AppendAffectInformation(self):
		for i in xrange(item.ITEM_APPLY_MAX_NUM):
			...
			if app.ENABLE_ACCE_COSTUME_SYSTEM and affectType==item.APPLY_ACCEDRAIN_RATE:
				continue
			if app.ENABLE_MOUNT_SYSTEM and affectType==item.APPLY_MOUNT:
				continue
			affectString = self.__GetAffectString(affectType, affectValue)
			if affectString:
				self.AppendTextLine(affectString, self.GetChangeTextLineColor(affectValue))
	...
	def GetPriceColor(self, price):
		if app.ENABLE_GOLD_MAX_EXTENDED and price>=constInfo.EXPENSIVE_SHIT:
			return self.EXPENSIVE_SHIT_PRICE_COLOR
	...
	def __AppendMetinSlotInfo_AppendMetinSocketData(self, index, metinSlotData, custumAffectString="", custumAffectString2="", custumAffectString3="", leftTime=0):
		...
		if player.METIN_SOCKET_TYPE_SILVER == slotType or itemIndex == 0:
			slotImage.LoadImage("d:/ymir work/ui/game/windows/metin_slot_silver.sub")
		if itemIndex != constInfo.ERROR_METIN_STONE and itemIndex > 0:
		#elif player.METIN_SOCKET_TYPE_GOLD == slotType:
			slotImage.LoadImage("d:/ymir work/ui/game/windows/metin_slot_gold.sub")
...
class HyperlinkItemToolTip(ItemToolTip):
	...
	def SetHyperlinkItem(self, tokens):
		...
		if tokens and len(tokens) >= minTokenCount and len(tokens) <= maxTokenCount:
			...
			self.ClearToolTip()
			self.isHyperLinkItem = True
			self.AddItemData(itemVnum, metinSlot, attrSlot, "hyperlink")
...
class SkillToolTip(ToolTip):
	...
	def __SetSkillTitle(self, skillIndex, skillGrade):
		if chr.IsGameMaster(0):
			self.AppendTextLine("%d" % skillIndex, self.SPECIAL_COLOR_TEXT)
			self.AppendSpace(5)
	...
	def AppendSkillDataNew(self, slotIndex, skillIndex, skillGrade, skillLevel, skillCurrentPercentage, skillNextPercentage):
		...
		if app.ENABLE_SKILL_DESCRIPTION_RENEWAL:
			need_bookcount = player.GetSkillLevel(slotIndex);
