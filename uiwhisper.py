...
if app.ENABLE_ADD_FRIEND_BUTTON_IN_WHISPERDIALOG:
	import messenger
...
class WhisperDialog(ui.ScriptWindow):
	...
	def LoadDialog(self):
		...
		self.btnWarpFriendShop = ui.MakeButton(self, 200, 9, uiScriptLocale.MC_WARP_BUTTON, "d:/ymir work/ui/game/windows/", "messenger_warp_01.sub", "messenger_warp_02.sub", "messenger_warp_03.sub")
		self.btnWarpFriendShop.SetEvent(ui.__mem_func__(self.OnWarpTarget))
		self.btnWarpFriendShop.Hide()
	...
	def OnWarpTarget(self):
		if self.targetName:
			net.SendChatPacket("/request_warp_at_character %s" % (self.targetName))
	...
	def OpenWithTarget(self, targetName):
		...
		if app.ENABLE_ADD_FRIEND_BUTTON_IN_WHISPERDIALOG:
			if not messenger.IsFriendByName(self.targetName):
				...
				self.friendButton.SetEvent(ui.__mem_func__(self.AddFriend))
				self.friendButton.Show()

		if not targetName == player.GetName():
			if targetName.find("[") == -1:
				self.btnWarpFriendShop.Show()
	...
	def SendWhisper(self):
		...
		if textLength > 0:
			...
			if app.__BL_MULTI_LANGUAGE_PREMIUM__:
				...
				chat.AppendWhisper(chat.WHISPER_TYPE_CHAT, self.targetName, player.GetName() + " : " + whisper_message, empire, country)
			else:
